...
public abstract class Participant {
                ...
                try {
                    if (!field.getType().equals(Map.class)) {
                        throw new ParticipantRegistrationException(
                                "Unable to inject data management Map into to field '" + field.getName() + "' on '"
                                        + serviceImpl.getClass().getName() + "': Field is not of type '" + Map.class
                                        + "'");
                    }
                    field.setAccessible(true);
                    field.set(serviceImpl, txDataMap);
                } catch (IllegalAccessException e) {
                    throw new ParticipantRegistrationException("Unable to inject data management map impl to field '"
                            + field.getName() + "' on '" + serviceImpl.getClass().getName() + "'", e);
                }
            }
        }
    ...
    protected Object invoke(Class<? extends Annotation> lifecycleEvent, Object... args) {
        ...
        try {
            ...
            // todo: detect parameters better. Maybe have a different
            // participant per interface.
            ...
            throw new RuntimeException(
                    "Unable to invoke method '" + method.getName() + "' on '" + serviceImpl.getClass().getName() + "'",
                    e);
        }
    }
