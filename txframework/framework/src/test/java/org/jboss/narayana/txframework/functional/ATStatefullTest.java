...
public class ATStatefullTest extends BaseFunctionalTest {
    ...
    /*
     * @Test public void testClientDrivenRollback() throws Exception {
     * ut.begin(); client.invoke(); ut.rollback();
     * 
     * //todo: should rollback be called twice? once for volatile and once for
     * durable assertOrder(Rollback.class, Rollback.class);
     * 
     * }
     * 
     * @Test(expected = TransactionRolledBackException.class) public void
     * testParticipantDrivenRollback() throws Exception { try { ut.begin();
     * client.invoke(VOTE_ROLLBACK); ut.commit(); } catch
     * (TransactionRolledBackException e) { //todo: should rollback be called
     * twice? once for volatile and once for durable assertOrder(Prepare.class,
     * Rollback.class); throw e; } }
     * 
     * @Test public void testApplicationException() throws Exception { try {
     * ut.begin(); client.invoke(THROW_APPLICATION_EXCEPTION);
     * Assert.fail("Exception should have been thrown by now"); } catch
     * (SOAPFaultException e) { //todo: can we pass application exceptions over
     * SOAP when using an EJB exposed as a JAX-WS ws?
     * System.out.println("Caught exception"); } finally { ut.rollback(); }
     * //todo: should this cause Rollback? assertOrder(Rollback.class,
     * Rollback.class); }
     * 
     */
