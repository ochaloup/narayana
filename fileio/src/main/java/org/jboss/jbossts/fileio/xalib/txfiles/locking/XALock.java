 ...
 * which of the bytes within the byte-range needs to change mode. This will
 * allow more concurrent operations acting on the same lock.
 * <p>
...
public class XALock implements Serializable, Comparable<XALock> {
     ...
     * managed by the {@link XALockManager} and are stored in a <em>locks
     * file</em>.
     *
     * @param xid
     *            the global Transaction id which was used when the lock was
     *            acquired
     * @param mode
     *            the mode (<code>LockMode.READ</code or <code>LockMode.WRITE)
     *            in which the bytes in the byte-range will be initialised
     * @param startPosition
     *            the position in the file of the first byte in the byte-range
     * @param lockLength
     ...
     *
     * @param mode
     *            the mode in which the bytes will be initialised
     */
     ...
     *
     * @return the global Transaction id used when the lock was acquired
     */
     ...
     *
     * @param position
     *            the position at which the new mode will be set
     * @param newMode
     *            the new mode to set to the byte at <code>position</code>
     */
     ...
     *
     * @param position
     *            the position of the first byte in the byte range
     * @param len
     *            the number of bytes that will change mode
     * @param newMode
     *            the new mode to set to a range of bytes
     */
    ...
    /**
     * This method sets a new start position of this lock. The method is
     * especially useful when the bounds of the lock have to be moved.
     *
     * @param startPosition
     *            the new start position of this lock
     */
     ...
     * Transactional file.
     *
     * @return a <code>long</code> which specifies how many bytes have been
     *         locked by this lock
     */
     ...
     *
     * @return a <code>long</code> which represents the end of this lock within
    ...
    /**
     * This method returns true if the <code>mode</code> is contained in the
     * byte-range of this lock; otherwise returns false.
     *
     * @param mode
     *            the mode to check if exists
     * @return true if the <code>mode</code> exists in the byte-range; false
     *         otherwise
     */
     ...
     *
     * @param sp
     *            the start position of the sub-range to search
     * @param ep
     *            the end position of the sub-range
     * @param mode
     *            the mode to search if exists
     ...
     * @return true if there is a conflict between <code>this</code> lock and
     *         the <code>newLock</code>; false otherwise
     */
    private boolean conflictsWith(XALock newLock) {
        return newLock == null || this.wraps(newLock) || this.isWrappedBy(newLock) || this.equals(newLock)
                || this.intersectedFromLeft(newLock) || this.intersectedFromRight(newLock);
    }

    /**
     * This method performs some calculations to return a result whether a
     * conflict exists or not. If the conflict exists then it tries to check the
     * byte-range to see if the lock can be granted or not. If needed the method
     * will modify the bounds of existing locks so the new one can fit next to
     * it.
     *
     * @param neighLock
     *            the lock located next to this lock
     * @return <code>Globals.UPDATE_OLD_LOCK</code>,
     *         <code>Globals.ADD_NEW_LOCK</code>
     *         <code>Globals.MOVE_LOCK_BOUNDS</code>,
     *         <code>Globals.REFUSE_LOCK</code>,
     *         <code>Globals.NO_MOD_LOCK</code>, depending on the outcome of
     *         computations
     */
        ...
        if (neighLock != null && neighLock.conflictsWith(this)) {
            if (neighLock.getXid() == this.getXid()) {
                if (this.equals(neighLock) || neighLock.wraps(this)) {
                    ...
                    this.setStartPosition(neighEP); // lock length will be
    ...
    /**
     * The method returns true if an existing lock surrounds the new lock; false
     * otherwise.
     *
     * @param newLock
     *            the lock trying to add
    ...
    /**
     * The method returns true if the existing lock is surrounded by the new
     * lock; false otherwise.
     *
     * @param newLock
     *            the lock trying to add
     * @return true if the existing lock is surrounded by the new lock; false
     *         otherwise
     */
    ...
    /**
     * Returns true if the existing lock conflicts from the left hand side with
     * the new lock; false otherwise.
     *
     * @param newLock
     *            the lock trying to add
     * @return true if the existing lock conflicts from the left hand side with
     *         the new lock; false otherwise
     */
    ...
    /**
     * Returns true if the existing lock conflicts from the right hand side with
     * the new lock; false otherwise.
     *
     * @param newLock
     *            the lock trying to add
     * @return true if the existing lock conflicts from the right hand side with
     *         the new lock; false otherwise
     */
     ...
     * <p>
     * It will return: <code>-1</code> if the new lock is on the right side of
     * the existing lock
     * <p>
     ...
     * @param newXALock
     *            the lock trying to add
     * @return <code>-1</code> if the new lock is on the right side of the
     *         existing lock <code>0</code> if both of the locks are starting at
     *         the same position <code>1</code> if the new lock is on the left
     *         side of the existing lock
     */
    ...
    /**
     * This method checks the equality of two locks. It will return true if both
     * of the locks start at the same point in the Transactional file and they
    ...
    private boolean equals(XALock newLock) {
        return (this.compareTo(newLock) == 0 && getLockLength() == newLock.getLockLength());
    }
}
