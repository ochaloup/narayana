...
/**
 * Instances of this class support both reading/writing to a random access file
 * and using the random access file with the support of Transactions. If
 * <code>newTransaction</code> is invoked after a
 * <code>TransactionManager</code> has been created and begun, the XAFile will
 * automatically act as a Transactional File, otherwise it will act as a normal
 * random access file with operations acting directly on the source file. If the
 * XAFile is in Transactional mode then any changes (writes) will not affect
 * directly the file, but will only take effect right after the
 * <code>TransactionManager</code> invokes its own </em>commit</em> method which
 * will cause the <code>commitUpdates</code> apply (write) the updated bytes to
 * the source file.
 * <p>
 * Calling the <code>newTransaction</code> method twice from the same thread
 * will cause a <code>DuplicateTransactionsException</code> to be thrown. As
 * this XAFile class implements the <code>DataInput, DataOutput</code>
 * interfaces all of their implemented read or write methods will throw an
 * <code>IOException</code> like specified in the above interfaces and the
 * read/write methods of the random access file used.
 *
...
public class XAFile implements DataInput, DataOutput, Serializable, Closeable {
    ...
    /**
     * Constructor to create objects that represent a Transactional File on
     * which read and write operations can be applied with the presence of ACID
     * semantics.
     * 
     * @param filename
     *            the name of the source file
     * @param mode
     *            the access mode (as specified in the
     *            {@link java.io.RandomAccessFile})
     * @param transactionsEnabled
     *            if true the XAFile will behave Transactionally and will allow
     *            commit/rollback operations, otherwise will behave as a normal
     *            random access file with read/write operations
     *
    ...
    /**
     * Method to create folders that are needed by the library
     */
     ...
     * <p>
     * The transaction is enlisted in a new {@link XAResourceManager} and
     * appropriate log files are created each time the method is invoked. It is
     * important that this method is called after the
     * {@link javax.transaction.TransactionManager} has began, in order to
     * benefit from the Transactional effects.
     * <p>
     * This method can be used in a multi-threaded environemnt but each thread
     * must call it once, otherwise a
     * <code>DuplicateTransactionsException</code> will be thrown to prevent
     * this.
     *
     * @param txnMngr
     *            the <code>TransactionManager</code> used to commit or rollback
     *            any attempted modifications to the source file
     *
     ...
     *                the <code>XAResourceManager</code> or the
     *                <code>RecordsLogger</code>
     * @exception javax.transaction.SystemException
     *                Will be thrown if there is a problem enlisting the
     *                XAResource created within this method or when the
     *                <code>getTransaction</code> in the
     *                <code>TransactionManager</code> fails or when the
     *                TransactionManager is not in ready
     *                (<code>Status.ACTIVE</code>) mode
     * @exception javax.transaction.RollbackException
     *                Will be thrown if there is a problem while enlisting the
     *                resource to the transaction
     * @exception IllegalStateException
     *                if the <code>transactionsEnabled</code> in the constructor
     *                of the XAFile has been chosen to be false
     */
        ...
        if (transactionsEnabled) {
            if (txnMngr.getStatus() == Status.STATUS_ACTIVE) {
                ...
                if (!xares.containsKey(th_id)) {
                    ...
                    throw new DuplicateTransactionsException(
                            "Cannot create a new Transaction. " + "There is already a thread with id=<" + th_id
     ...
     * 
     * @return the <code>RandomAccessFile</code> object associated with this
     *         class to apply the read/write operations
     */
     ...
     * Closes this XAFile.
     * <p>
     * The method actually closes the random access file stream and releases any
     * system resources associated with the stream and the Transactions. If this
     * file has an associated channel then the channel is closed as well.
     *
     * @exception IOException
     *                if an I/O error occurs or if there are incomplete
     *                Transactions.
     */
    public void close() throws IOException {
        if (!xares.isEmpty())
            throw new IOException("Failed to close the file. There are incomplete Transactions."); // todo
                                                                                                    // better
                                                                                                    // rollback??
        raf.close();
        if (xaLockManager.obtainHeldLocksWith(null).isEmpty()) // if the file is
                                                                // not empty,
            xaLockManager.deleteFile(); // possibly another VM has written to
                                        // it, so
    } // do not delete it

    /**
     * Gets the <code>FileDescriptor</code> of the random access file used and
    ...
    /**
     * Forces data to be written to disk by instantly closing and re-opening the
     * random access file. The file pointer returns to the correct position
     ...
     * @exception IOException
     *                if <code>position</code> is less than <code>0</code> or if
     *                an I/O error occurs.
     */
     ...
     * outside the scope of begin-commit/rollback an exception will be thrown as
     * specified in the <code>newTransaction</code> method. To use these
     * operations in that way set transaction support to false.
     * 
     * @param transactionsEnabled
     *            if true, the XAFile will create transactions and will allow
     *            commit/rollback operations in the
     ...
     * 
     * @param th_id
     *            the thread id participating in the generated message
     * @return a standard error message based on the given <code>th_id</code>
     */
    private String getErrMsg(long th_id) {
        return "Failed to update source file. " + "The thread with id=<" + th_id
                + "> is not associated with any Transaction";
    }

    /**
     * Method to update a series of bytes in the file starting from a given
     * <code>position</code>.
     * <p>
     * The method is called after an {@link XAResourceManager} is ready to
     * commit. It copies <code>data</code> of exact <code>recordLength</code>
     * and starting from position <code>position</code> in the source file.
     *
     * @param position
     *            the position in the file to start copying <code>data</code>
     * @param recordLength
     *            the length of bytes to be copied
     * @param data
     *            the actual data/updates
     * @param th_id
     *            the thread associated with the Transaction trying to commit
     * @exception IOException
     *                if an I/O error occurs
     * @exception IllegalStateException
     ...
     *                Transaction
     */
    protected synchronized void commitUpdates(long position, int recordLength, byte[] data, long th_id)
            throws IOException {
     ...
     * <p>
     * After a <code>TransactionManager</code>'s commit/rollback operations this
     * method is invoked to remove the thread registered by the
     * <code>newTransaction</code> method.
     * 
     * @param xid
     *            the thread as a key to remove the corresponding
     *            <code>XAResourceManager</code> from the list.
     * @param recovers
     *            true if the system has crashed and tries to recover; otherwise
     *            false
     * @exception IOException
    ...
    /**
     * Fill the list of locks held while trying to read/write to the file.
     *
     * @exception IOException
     *                if an I/O error occurs
     */
    ...
    /**
     * Internal method to ask from
     * {@link org.jboss.jbossts.fileio.xalib.txfiles.locking.XALockManager} to
     * apply a read/write lock on a given <code>DataRecord</code>.
     * <p>
     * The method is invoked by read/write operations within the XAFile to lock
     * on a specific range of bytes in the file.
     * 
     * @param dr
     *            the record to lock on, either in read or write mode
     * @param xid
     *            the {@link javax.transaction.xa.Xid} of the current
     *            {@link XAResourceManager}
     * @param mode
     *            read/write modes as specified in the
     *            {@link com.arjuna.ats.txoj.LockMode} class
     * @return the {@link com.arjuna.ats.txoj.LockResult} depending on whether
     *         the lock can be <em>GRANTED or REFUSED</em>
     * @exception IOException
     *                if an I/O error occurs
     * @exception org.jboss.jbossts.fileio.xalib.txfiles.exceptions.LockRefusedException
     *                if lock cannot be <em>GRANTED</em>
     */
    ...
    /**
     * Returns the access mode in which the <code>XAFile</code> was created
     * 
    ...
    /**
     * Internal method used by read operations in this <code>XAFile</code>. If
     * the class is used in the Transactional mode (Transactions are enabled)
     ...
     * <p>
     * The method starts reading bytes at the current file pointer. If data are
     * retrieved from the memory instead of the actual file, the file pointer
     * progresses by <code>len</code> steps, if successful.
     * 
     * @param len
     ...
     *                if an I/O error occurs
     * @exception org.jboss.jbossts.fileio.xalib.txfiles.exceptions.LockRefusedException
     *                if a lock cannot be <code>GRANTED</code>
     */
    ...
    /**
     * Method to read exactly <code>len</code> bytes directly from the file and
     * starting at the current file pointer.
     *
     * @param len
     *            the length of bytes to read from the file
     ...
     * byte from the <code>readByte</code> method, starting at the current file
     * pointer. A value of <code>0</code> represents <code>false</code>. Any
     * other value represents <code>true</code>. This method blocks until the
    ...
    /**
     * Reads an unsigned 16-bit number from this file. This method gets the two
     * bytes return from the <code>readRecord</code> method, starting at the
     * current file pointer. If the bytes read, in order, are <code>b1</code>
     ...
     *         uncommitted changes), interpreted as an unsigned 16-bit integer.
     * @exception EOFException
     ...
     * are uncommitted updated bytes) into the byte array, starting at the
     * current file pointer. This method reads repeatedly until the requested
     * number of bytes are read. This method blocks until the requested number
     * of bytes are read, the end of the stream is detected, or an exception is
     * thrown.
     *
     * @param bytes
     ...
     *                if offset is negative, or len is negative, or offset+len
     *                is greater than the size of the <code>bytes</code> array
     */
     ...
     * from this file (or memory if there are uncommitted byte updates),
     ...
     * between <code>0</code> and <code>255</code>, inclusive, then the result
    ...
    /**
     * Reads in a string from this file (or memory if there are uncommitted byte
     * updates). The string has been encoded using a
     ...
     * uncommitted byte updates). This method reads an <code>int</code> value,
     * starting at the current file pointer, as if by the <code>readInt</code>
     * method and then converts that <code>int</code> to a <code>float</code>
     * using the <code>intBitsToFloat</code> method in class <code>Float</code>.
     * <p>
    ...
    /**
     * Reads a signed 32-bit integer from this file (or memory if there are
     * uncommitted byte updates). This method reads 4 bytes from the file,
     * starting at the current file pointer. If the bytes read, in order, are
     * <code>b1</code>, <code>b2</code>, <code>b3</code>, and <code>b4</code>,
     * where <code>0&nbsp;&lt;=&nbsp;b1, b2, b3, b4&nbsp;&lt;=&nbsp;255</code>,
    ...
    /**
     * Reads an unsigned eight-bit number from this file (or memory if there are
     * uncommitted byte updates). This method reads a byte, starting at the
     * current file pointer, and returns that byte.
     * <p>
    ...
    /**
     * Reads a signed eight-bit value from this file (or memory if there are
     * uncommitted byte updates). This method reads a byte, starting from the
     * current file pointer. If the byte read is <code>b</code>, where
     ...
     * reads bytes from the file (or memory if there are uncommitted byte
     * updates), starting at the current file pointer, until it reaches a line
     * terminator or the end of the file. Each byte is converted into a
    ...
    /**
     * Reads up to <code>bytes.length</code> bytes of data from this file (or
     * memory if there are uncommitted byte updates) into an array of bytes.
    ...
    /**
     * Reads exactly <code>len</code> bytes of data from this file (or memory if
     * there are uncommitted byte updates) into an array of bytes. This method
     ...
     *            the buffer into which the data is read.
     * @param off
     *            the start offset in array <code>bytes</code> at which the data
     *            is written.
     * @param len
    ...
    public int read(byte[] bytes, int off, int len) throws IOException {
        ...
        if (bytes.length == 0)
            return 0;
        byte[] bs = getBytesFromInts(readRecord(len)); // todo readRecord may
                                                        // return null
        System.arraycopy(bs, 0, bytes, off, len);
        // int b;
    ...
    /**
     * Reads up to <code>chars.length</code> characters from this file (or
     * memory if there are uncommitted byte updates) into an array of
     * characters.
     * <p>
     * This method blocks until the char is read, the end of the stream is
     * detected, or an exception is thrown.
     *
     * @param chars
     *            the array into which the characters are read
     ...
     * file (or memory if there are uncommitted byte updates), starting at the
     * current file pointer. If the bytes read, in order, are <code>b1</code>
     * and <code>b2</code>, where
     * <code>0&nbsp;&lt;=&nbsp;b1,&nbsp;b2&nbsp;&lt;=&nbsp;255</code>, then the
     * result is equal to: <blockquote>
    ...
    /**
     * Attempts to write a <code>long</code> to the file as eight bytes, high
    ...
    public void writeLong(long l) throws IOException {
        int[] ls = new int[]{((int) (l >>> 56) & 0xFF), ((int) (l >>> 48) & 0xFF), ((int) (l >>> 40) & 0xFF),
                ((int) (l >>> 32) & 0xFF), ((int) (l >>> 24) & 0xFF), ((int) (l >>> 16) & 0xFF),
     ...
     * byte first. The write starts at the current position of the file pointer.
     * <p>
     * The <code>int</code> will be written to the file only after a commit
     * operation call
     * 
     * @param n
     ...
     * then attempts to write that <code>long</code> value to the file as an
     * eight-byte quantity, high byte first. The write starts at the current
     * position of the file pointer.
     * <p>
     * The <code>double</code> will be written to the file only after a commit
     * operation call
     *
     * @param d
     ...
     * attempts to write that <code>int</code> value to the file as a four-byte
     * quantity, high byte first. The write starts at the current position of
     * the file pointer.
     * <p>
     * The <code>float</code> will be written to the file only after a commit
     * operation call
     *
     * @param f
    ...
    /**
     * Attempts to write a <code>boolean</code> to the file as a one-byte value.
     * The value <code>true</code> is written out as the value
     * <code>(byte)1</code>; the value <code>false</code> is written out as the
     * value <code>(byte)0</code>. The write starts at the current position of
     * the file pointer.
     * <p>
     * The <code>boolean</code> will be written to the file only after a commit
     * operation call
     *
     * @param b
     ...
     * <p>
     * The <code>byte</code> will be written to the file only after a commit
     * operation call
     *
     * @param b
    ...
    /**
     * Internal method used by write oprations in the <code>XAFile</code>. If
     * Transactions are disabled, bytes are written directly to the file. If
     * Transactions are enabled the given <code>bytes</code> are written to the
     * memory (added in a hashtable).
     * <p>
     * The method may be called by different threads. To prevent different
     * Transactions to modify an already modified record by another Transaction
     * a lock in <code>WRITE</code> mode is acquired. If the lock is finally
     * <code>GRANTED</code> the <code>bytes<code> are added
     ...
     * byte first. The write starts at the current position of the file pointer.
     * <p>
     * The <code>short</code> will be written to the file only after a commit
     * operation call
     *
     * @param s
    ...
    /**
     * Attempts to write <code>len</code> bytes from the specified byte array
     * starting at offset <code>off</code> to this file.
     * <p>
     * The bytes will be written to the file only after a commit operation call
     *
     * @param bytes
     *            the data.
     * @param off
    ...
    /**
     * Attempts to write a string to the file as a sequence of characters. Each
     * character is written to the data output stream as if by the
     * <code>writeChar</code> method. The write starts at the current position
     * of the file pointer.
     * <p>
     * The <code>String</code> will be written to the file only after a commit
     * operation call
     *
     * @param s
    ...
    /**
     * Attempts to write a <code>char</code> to the file as a two-byte value,
     * high byte first. The write starts at the current position of the file
     * pointer.
     * <p>
     * The <code>char</code> will be written to the file only after a commit
     * operation call
     *
     * @param ch
     ...
     * high eight bits. The write starts at the current position of the file
     * pointer.
     * <p>
     * The <code>String</code> will be written to the file only after a commit
     * operation call
    ...
    /**
     * Attempts to write <code>bytes.length</code> bytes from the specified byte
     * array to this file, starting at current file pointer.
     * <p>
     * The bytes will be written to the file only after a commit operation call
     *
     * @param bytes
     ...
     * <p>
     * First, two bytes are written to the memory, starting at the current file
     ...
     *         next attempt of read or write occurs.
     * @exception IOException
     *                if an I/O error occurs.
     */
    ...
    /**
     * Returns the current thread id. If this id was not used to register a
     * Transaction before an <code>IOException</code> will be thrown.
     * 
     * @return the current thread's id registered with a Transaction
     * @exception IOException
     *                if the current thread is not associated with any
     *                Transaction
     */
    ...
    /**
     * Used by <code>XAResourceManager</code> after starting-up recovery
     * procedure
     * 
     * @exception FileNotFoundException
    ...
    /**
     * Converts an array of type <code>int</code> to an array of type
     * <code>byte</code> and returns that.
     *
     * @param ints
     *            the array to convert into a <code>byte</code> array
    ...
    /**
     * Converts an array of type <code>byte</code> to an array of type
     * <code>int</code> and returns that.
     *
     * @param bytes
     *            the array to convert into an <code>int</code> array
