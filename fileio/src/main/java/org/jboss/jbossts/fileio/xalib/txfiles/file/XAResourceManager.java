...
/**
 * This class implements methods of the standard
 * {@link javax.transaction.xa.XAResource} interface which acts like a contract
 * between a Resource Manager and a Transaction Manager.
 * <p>
 * Each instance of an <code>XAResourceManager</code> has its own
 * {@link javax.transaction.xa.Xid} which distinguishes by the other objects of
 * this class and is associated with a Transaction. Each
 * <code>XAResourceManager</code> instance has also a <code>log</code>
 * ({@link RecordsLogger} object) to write information about the bytes that have
 * been updated by <code>XAFile</code>. The updates are also added into a
 * hashtable (depending on the starting position of the bytes that make up a
 * {@link DataRecord} each time. If no crash has occured the Resource manager
 * will retrieve information about the requested records using its hashtable in
 * memory. In a situation where a system failure occured and the system tries to
 * recover, the logs will be used to obtain the requested information.
 * <p>
...
public class XAResourceManager implements XAResource, Serializable {
    ...
    /**
     * Constructor to create Resource Manager objects. Each of these objects, at
     * transaction time, are informed by the Transaction Manager to prepare,
     * commit or rollback (depending on the outcome of the 2PC protocol). Each
     * of these Resource Manager objects are kept in a hashtable in the XAFile
     * and are associated with the thread id that created the corresponding
     * Transaction.
     * 
     * @param xaFile
     *            the file instance on which updates take place
     * @param log
     *            the logger object which keeps update-relative information
     * @param th_id
     *            the thread id that created the transaction in which this
     *            <code>XAResourceManager</code> object has been enlisted.
     * @throws IOException
     ...
     * commit.
     * <p>
     * The method forces system-memory buffers to write their data to the log
     * file to ensure all the updates that are to be applied to the file are
     * included in the log. The log file is then closed.
     *
     * @param xid
     *            a global Transaction id
     * @return <code>XA_OK</code> after synchronizing the log
     * @exception XAException
     *                if an error occured while synchronizing the log
     */
    public int prepare(Xid xid) throws XAException {
        ...
        try {
            ...
            throw new XAException("Unable to flush data to the log file <" + log.getFilename() + ">.");
        }
     ...
     * failure, the same procedure will be followed but the updated bytes will
     * be read from the log file instead of the memory, as memory will contain
     * no relative information during recovery.
     *
     * @param xid
     *            a global Transaction id
     * @param onePhase
     ...
     * @exception XAException
     *                if an I/O error occurs when calling the
    ...
    public void commit(Xid xid, boolean onePhase) throws XAException {
        ...
        try {
            ...
            ioe.printStackTrace();
            throw new XAException("Commit failed! Possibly an I/O error occurd while " + "using the log file <"
                    + log.getFilename() + ">");
        }
    ...
    /**
     * Forget about a heuristically completed transaction branch. The
     * <code>currentXid</code> is set to null
     * 
     * @param xid
     *            a global Transaction id
     */
     ...
     * <code>XAResourceManager.setTransactionTimeout</code> was not used prior
     * to invoking this method, the return value is the default timeout set for
     ...
     * <p>
     * The method closes the log file and deletes it as it is not useful
     * anymore. It also deletes the bytes from the hashtable and disassociates
     * the transaction with the given <code>xid</code> from the thread that
     * initiated it, if not called by the Recovery Manager.
     *
     * @param xid
     *            a global Transaction id
     * @exception XAException
     *                if an error occured while trying to disassociate the the
     *                given <code>xid</code> from its initiator thread
     */
    public void rollback(Xid xid) throws XAException {
        ...
        try {
            ...
            e.printStackTrace();
            throw new XAException("Rollback failed. Could not disassociate " + "the current thread " + th_id
                    + "from the transaction with xid=<" + xid);
        }
        log.close();
        log.delete();
        currentXid = null;
     ...
     * a different value. To reset the timeout value to the default value used
     * by the resource manager, set the value to zero. If the timeout operation
     * is performed successfully, the method returns true; otherwise false. If a
     ...
     * 
     * @param xid
     *            a global Transaction id to be associated with this Resource
     *            Manager instance
     * @param flags
     *            (can be anything)
     * @throws XAException
    ...
    public void start(Xid xid, int flags) throws XAException {
        System.out.println("XAResourceManager.start(Xid=" + xid + ", flags=" + flags + "), th_id=" + th_id);
        if (currentXid != null) {
    ...
    /**
     * The method is invoked by the <code>commit</code> method of the
     * <code>XAResourceManager</code> and performs the work as specified in that
     * method.
     *
     * @exception IOException
     *                If an I/O error occurs while reading the log
     */
    private void commitChanges() throws IOException {
        ...
        for (DataRecord dr : records) {
            xaFile.commitUpdates(dr.getStartPosition(), dr.getRecordLength(), dr.getRecordBytes(), th_id);
        }
    ...
    /**
     * Returns a list with the records (updates) written by some write operation
     * in the <code>XAFile</code>. It either reads the records from the
     * hashtable in memory (if not in recover phase), or from the
     * <code>log</code> file which contains all the updates (if the system tries
     * to recover).
     *
     * @return a list with <code>DataRecord</code> objects that contain the
     *         updated data
     * @exception IOException
     *                if an I/O error occurs
     */
    ...
    /**
     * Adds adequate information to the <code>log</code>. Each entry consists of
     * the start position of the record in the file, its length and the data
     * (updates)
     * 
     * @param dr
     *            the record by which information will be written to the log
     * @exception IOException
     *                if an I/O error occurs
     */
     ...
     * 
     * @return the global Transaction id associated with this XAResourceManager
     */
    ...
    /**
     * Returns all the updated bytes that have been modified by the transaction
     * with <code>currentXid</code>.
     * 
     * @return all the updated bytes that have been stored in the table by
     *         previous <code>write</code> operations.
     */
    ...
    /**
     * Adds the given <code>bytes</code> to the table that keeps all the
     * <code>updatedBytes</code>. The array is treated as a group of modified
     * bytes (record) with its first byte located at <code>startPosition</code>
     * in the Transactional File.
     *
     * @param startPosition
     *            the position in the file where the record starts
     * @param bytes
     *            an array that includes all the updated bytes to be stored
     */
    ...
    /**
     * This method is invoked when trying to deserialize an
     * <code>XAResourceManager</code> object.
     * <p>
     * The method will read the non-static and non-transient fields of the
     * current class and will set the <code>recovers</code> value to
     * <code>true</code>. This will allow the rest of the methods in this class
     * to be aware that the system is in its recovery phase and aact properly.
     * <p>
     * The method also initializes the <code>RandomAccessFile</code> object used
     * in the <Code>XAFile</code> as well as the <code>XALockManager</code>
     * objects to obtain the list of held locks.
     *
     * @param in
