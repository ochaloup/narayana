...
/**
 * Instances of this class represent a transactional directory on which file
 * operations such as create, rename and delete can be applied. The contents
 * (files) of this directory are represented by {@link XADirFile} objects.
 * Through these objects the application programmer can access the file
 * operations.
 * <p>
...
public class XADir implements Serializable, Closeable {
     ...
     * <p>
     * The constructor checks if the <code>storeDir</code> is a directory or
     * not. If it is not then
     * {@link org.jboss.jbossts.fileio.xalib.txdirs.exceptions.NotDirectoryException}
     * exception is thrown. Otherwise, a new <code>FileResourceManager</code>
     * object is created to allow access to transactional methods (e.g. start,
    ...
    public XADir(File storeDir) throws IOException, ResourceManagerException {
        ...
        freMngr.start(); // start the FileResourceManager service, must be
                            // started
    } // before using any of its methods

    /**
     * This method lists all the files under the transactional directory. It
     * lists only files and not <code>File</code>s as in Java this may also mean
     * directories.
     * 
     * @return a list of <code>XADirFile</code> objects which represent the
    ...
    /**
     * This method must be used after a <code>TransactionManager</code> has
     * begun and within the boundaries of a transaction (<code>begin,
     * commit/rollback</code>).
     * <p>
     * The method also creates a new {@link XAFileResourceManager} and enlists
     * it to the transaction obtained by the <code>txnMngr</code>.
     *
     * @param txnMngr
     *            the <code>TransactionManager</code> used to commit or rollback
     *            the file operations
     * @exception javax.transaction.RollbackException
     *                if an error occurs while enlisting the
     *                <code>XAResource</code>
     * @exception javax.transaction.SystemException
     *                if there is a problem enlisting the XAResource created
     *                within this method or when the <code>getTransaction</code>
     *                in the <code>TransactionManager</code> fails or when the
    ...
    /**
     * This is the name of the "shadow" folder which keeps the changes whilst a
    ...
    /**
     * Returns the number of files (not directories) under this transactional
     * directory.
     *
     * @return a <code>long</code> representing the number of files within the
     ...
     * The method will release and remove any of the "shadow" files/folders used
     * whilist the transaction was in progress.
     * <p>
