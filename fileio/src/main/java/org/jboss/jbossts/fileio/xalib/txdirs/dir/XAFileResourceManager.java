 ...
 * <p>
 * Each instance of an <code>XAFileResourceManager</code> has its own
 * {@link javax.transaction.xa.Xid} which distinguishes by the other objects of
 * this class and is associated with a Transaction. It also has a unique
 * transaction id which is represented by a <code>String</code> and is used by
 * {@link org.apache.commons.transaction.file.FileResourceManager} to know which
 * transaction to start, commit/rollback etc.
 * <p>
 * Any changes made within a transaction specified by the <code>Xid</code>
 * mentioned above will be applied to the transactional directory only after
 * this <code>XAFileResourceManager</code> has commited. If the decision is to
 * rollback then all the modifications made within the transaction are removed.
 *
...
public class XAFileResourceManager implements XAResource, Serializable {
     ...
     * commit or rollback (depending on the outcome of the 2PC protocol).
     *
     * @param freMngr
     *            the Resource Manager on which methods like start, commit or
     *            rollback a transaction will be applied
     * @param curTxId
     *            the unique transaction id used by <code>freMngr</code> to know
     *            on which transaction to work
     */
    ...
    /**
     * Acts like the
     * {@link org.apache.commons.transaction.file.FileResourceManager#prepareTransaction(Object)}
     * method does.
     *
     * @param xid
     ...
     * {@link org.apache.commons.transaction.file.FileResourceManager#commitTransaction(Object)}
     * does.
     *
     * @param xid
     ...
     * <code>XAFileResourceManager.setTransactionTimeout</code> was not used
     * prior to invoking this method, the return value is the default timeout
     * set for the resource manager; otherwise, the value used in the previous
    ...
    public int getTransactionTimeout() throws XAException {
        try {
            int timeout = (int) freMngr.getTransactionTimeout(curTxId) / 1000; // ms
                                                                                // ->
                                                                                // secs
     ...
     * <code>XAFileResourceManager</code> instance. Once set, this timeout value
     * is effective until <code>setTransactionTimeout</code> is invoked again
     * with a different value. The method acts like the
     ...
     * <code>xid</code>. The method act like the
    ...
    /**
     * After a system crash and upon recovery phase the
     * <code>FileResourceManager</code> object needs to be re-initialised.
     */
