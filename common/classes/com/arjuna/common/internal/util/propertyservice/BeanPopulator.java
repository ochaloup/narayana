...
/**
 * Utility class that configures *EnvironmentBean objects using a
 * PropertyManager, which is usually backed by a -properties.xml file.
 *
...
public class BeanPopulator {
    ...
    /**
     * Examine the properties of the provided bean and update them to match the
     * values of the corresponding properties in the Properties. This will
     * normally be used at startup to configure a freshly created default bean
     * to match the configuration read from a properties file.
     *
     * The algorithm is as follows: for each field in the bean, which must have
     * a getter and setter method matching according to the JavaBeans naming
     * conventions, determine the corresponding property key.
     *
     * Several key names are tried, with the first match being used: For scalar
     * properties: The FQN of the bean class, optionally followed by the bean
     * name, followed by the field name, e.g. com.arjuna.FooBean.theField or
     * com.arjuna.FooBean.theName.theField the short class name of the bean,
     * optionally followed by the bean name, followed by the field name, e.g.
     * FooBean.theField or FooBean.theName.theField and finally the bean
     * classes' PropertyPrefix annotation value followed by the name of the
     * field, the last being except in cases where the field has a
     * FullPropertyName annotation, in which case its value is used instead. For
     * vector (in the math sense - the type is actually normally List/Map)
     * properties, a single property key matched according to the prior rules
     * will be treated as having a compound value which will be tokenized into
     * elements based on whitespace and inserted into the list in token order or
     * further tokenized on = for Map key/value. If no such key is found, the
     * value of the field's ConcatenationPrefix annotation is treated as a name
     * prefix and the list elements are assembled from the values of any
     * properties having key values starting with the prefix. These are inserted
     * into the list in order of the key's name sort position.
     *
     * This allows for the convention that all properties in a given bean will
     * share the same prefix e.g. com.arjuna.ats.arjuna.foo. whilst still
     * allowing for changing of the property name in cases where this makes for
     * more readable code.
     *
     * Obtain the value of the property from the Properties and if it's not
     * null, type convert it to match the bean's property field type. Obtain the
     * value of the property from the bean and if it's different from the value
     * read from the Properties, use the setter to update the bean.
     *
     * @param bean
     *            a JavaBean, the target of the property updates and source for
     *            defaults.
     * @param instanceName
     *            the (optional, use null for default) name for the bean
     *            instance.
     * @param properties
     *            a Properties object, the source of the configuration
     *            overrides.
     * @throws Exception
     *             if the configuration of the bean fails.
     */
    public static void configureFromProperties(Object bean, String instanceName, Properties properties)
            throws Exception {
    ...
    private static void handleGroupProperty(Object bean, String instanceName, Properties properties, Field field,
            Method setter, Method getter) throws Exception {
        List<String> lines = new LinkedList<String>();

        String valueFromProperties = getValueFromProperties(bean, instanceName, properties, field,
        ...
        if (valueFromProperties != null) {
            ...
            // for lists, the order we want them in is the order they appear in
            // the string, so we can just add in sequence:
            for (String token : tokens) {
                lines.add(token);
            }
        } else {
            // it's set of values that need gathering together
            // for lists, the order we want them in is the lex sort order of the
            // keys, so we need to buffer and sort them:
            ...
            if (lines.size() == 0) {
                return; // no relevant value in properties file, so leave bean
                        // defaults alone.
            }
        }
        ...
        if (java.util.Map.class.isAssignableFrom(field.getType())) {
            // we have a list but need a map. split each element into key/value
            // pair.
    ...
    private static void handleSimpleProperty(Object bean, String instanceName, Properties properties, Field field,
            Method setter, Method getter) throws Exception {
        String prefix = null;
    ...
    private static String getValueFromProperties(Object bean, String instanceName, Properties properties, Field field,
            String prefix) {
