...
/**
 * Defines that implementations of the interface are to be managed within a
 * transactional container. Unless specified using other annotations, all public
 * methods will be assumed to modify the state of the object, i.e., require
 * write locks. All state variables will be saved/restored by default unless it
 * is marked using the @State annotation or SaveState/RestoreState.
 * 
 ...
 * 
 * NOTE this annotation needs to be provided on the interface so that the
 * container knows to create proxy objects for the derived types once it sees
 * them.
 * 
 * @author marklittle
 *
 ...
 * transaction system. For instance, maybe the implementation class can call
 * into the container that maintains the proxy, passing itself as a reference,
 * and drive setlock, modified etc. implicitly.
 */
