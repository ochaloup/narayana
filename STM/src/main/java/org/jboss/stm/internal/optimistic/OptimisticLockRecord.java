...
class OptimisticLockRecord extends LockRecord {
    ...
    private boolean checkState() {
        ...
        /*
         * If we check the state now, it's possible that some other transactions
         * may be doing the same concurrently. We need to lock the object at
         * this point. Or suffer heuristic by checking during commit - though
         * this still leaves a window of vulnerability.
         */

        /*
         * Assume initially that this will only work if the objects are all in
         * the same address space, since sharing across spaces will impose
         * performance overhead anyway. In that case, we can maintain a list of
         * all objects that are being managed optimistically and check them
         * directly as well as lock them.
         * 
         * Problem is that it's the state that needs to be checked and there may
         * be multiple instances of the state active in memory at the same time.
         * So would need to keep each instance per Uid.
         */

        /*
         * Could even make this specific to STM and in which case we have even
         * more control.
         */
        ...
        if (objectType == ObjectType.ANDPERSISTENT) {
            ...
            try {
                ...
                if (s != null) {
                    ...
                    return true; // no state, so we are first! (check state type
                                    // instead?)
