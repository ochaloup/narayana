...
public class InvocationHandler<T> implements java.lang.reflect.InvocationHandler {
    ...
    public InvocationHandler(RecoverableContainer<T> cont, T obj, Uid u) {
        ...
        if (u != null) {
             ...
             * can be used and still be thread-safe, then we could go back to
             * using RECOVERABLE. Make it an annotation option?
             */

            if (_optimistic) {
                /*
                 * At this stage we ignore the type and model settings in the
                 * container. So annotations override the default container
                 * settings. Consider re-visiting the model such that either
                 * this is explicit for all objects where model/type is
                 * different to container, or change instance/container
                 * relationship so the container values continue to match the
                 * instance (even if lazily so container data is updated).
                 * Problem with latter approach is that we can have many
                 * different types of object coming from the same container.
                 */
        ...
        /*
         * Do we need to create (sub-) transactions when each method is called?
         */
    ...
    public Object invoke(Object proxy, java.lang.reflect.Method method, Object[] args) throws Throwable {
        /*
         * Do nothing currently if not inside of a transaction and not asked to
         * create transactions for this type of object.
         * 
         * Automatically create transactions in methods for nested transaction
         * capability, i.e., duplicate what normal Arjuna programmers take for
         * granted.
         */
        ...
        synchronized (_txObject) {
            synchronized (_theObject) {
                ...
                try {
                    ...
                    // todo allow null transaction context - not an issue for
                    // now with STM though!
                    ...
                    if (BasicAction.Current() != null) {
                        ...
                        /*
                         * Look for the corresponding method in the original
                         * object and check the annotations applied there.
                         * 
                        ...
                        if (cachedLock == null) {
                            ...
                            /*
                             * Should we catch common methods, like equals, and
                             * call Object... automatically?
                             */
                        ...
                        if (!lockFree && !transactionFree) {
                            int result = _txObject.setlock((_optimistic
                                    ? new OptimisticLock(cachedLock._lockType)
                                    : new Lock(cachedLock._lockType)), cachedLock._retry, cachedLock._timeout);
        ...
        synchronized (InvocationHandler.class) {
            if (_storeManager == null) {
                try {
                    ...
                    // means store already initialised so check to see if
                    // compatible

                    if (StoreManager.setupStore(null, ObjectModel.SINGLE).getClass()
                            .equals(TwoPhaseVolatileStore.class)) {
