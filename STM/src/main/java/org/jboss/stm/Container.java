...
/**
 * Instances of this class represent the transactional memory within which user
 * objects can be placed and managed.
 * 
 * Objects must implement an interface through which all transactional accesses
 * occur. We don't mandate what the interface is, since that will depend upon
 * the business logic. The interface, or the implementing class, must also use
 * the @Transactional annotation.
 * 
 * Unless either the Nested or NestedTopLevel annotation is used, all method
 * invocations on objects returned from a Container should be done within the
 * context of an active transaction.
 * 
 * @author marklittle
 */
...
public class Container<T> {
     ...
     * 
     * RECOVERABLE cannot be shared between address spaces and cannot tolerate
     * crash failures. PERSISTENT can be shared between address spaces (though
     * don't have to be) and can tolerate crash failures.
     * 
     * @author marklittle
     */
     ...
     * be PERSISTENT too. EXCLUSIVE means that the instance will only be used
     * within a single process. It can be PERSISTENT or RECOVERABLE.
     * 
     * @author marklittle
     */
    ...
    /**
     * Create a container (system assigned name) of the specified type. Objects
     * will be EXCLUSIVE.
     * 
     * @param type
    ...
    /**
     * Create a container (system assigned name) of the specified type and
     * model.
     * 
     * @param type
     *            the TYPE of objects.
     ...
     * that for future use. All accesses on the returned object will be managed
     * according to the rules defined in the various annotations. If the
     * original object instance is used then no transactional manipulation will
     ...
     * useful when using pessimistic concurrency control and we need one object
     * instance per thread to ensure that state is safely managed.
     * 
     * @param member
     *            the instance of type T that you want to be made transactional
     *            and persistent.
    ...
    public synchronized T clone(T member, T proxy) {
        ...
        /*
         * If we are using pessimistic cc then we don't need to do a clone and
         * can return the same instance. No MVCC needed here, so shortcut.
         */
     ...
     * managed.
     * 
     * WARNING: if the Uid is invalid, e.g., points to a state that no longer
     * exists, then a handle will still be returned because checks for validity
     * (other than null parameter) cannot be done until you try to use the
     * state. At that time a lock will be refused (state cannot be activated)
     ...
     * 
     * @param proxy
     *            the instance within the container we're looking for.
    ...
    public static final Container<?> getContainer(Object proxy) {
        /*
         * Rather than maintain a list of Container instances and iterate
         * through them we create a clone of the Container using the real
         * container within the proxy itself. Container is essentially a
         * (almost) stateless wrapper class anyway so hopefully this is a
         * lightweight and faster way of achieving this. Can revisit later if
         * this creates too many instances.
         */
