...
public class DummyRecovery extends RecoverySetup {
        ...
        if (args.length == 1) {
            if (args[0].equals("-f")) {
                BeanPopulator.getDefaultInstance(ObjectStoreEnvironmentBean.class)
                        .setObjectStoreDir(Util.recoveryStoreDir);
    ...
    public void enlistmentFailure() throws NotSupportedException, RollbackException, SystemException,
        ...
        // commit any transactional work that was done on the two dummy XA
        // resources
        System.out.println(
                "Halting VM - next test run will not halt and should pass since there will be transactions to recover");
    ...
    public void waitForRecovery() throws InterruptedException {
        ...
        if (commitRequests >= DummyXAResource.getCommitRequests())
            throw new RuntimeException(
                    "Did you forget to generate a recovery record before testing recovery (use the -f argument)");
