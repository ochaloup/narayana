...
public class JmsRecovery extends RecoverySetup {
    ...
    private static void startHornetq() throws Exception {
        /*
         * Step 1. Decide whether to use inVM or remote communications: clients
         * connect to servers by obtaining connections from a ConnectorFactory
         * servers accept connections from clients by obtaining acceptors from
         * an AcceptorFactory
         */
    ...
    public static void startRecovery() {
        ...
        /*
         * Tell JBossTS how to recover Hornetq resources. To do it via
         * jbossts-properties.xml use <entry
         * key="JTAEnvironmentBean.xaResourceRecoveryClassNames">
         */
    ...
    private static void startHornetqServer(String acceptorName, String connFacName, String queueName) throws Exception {
        ...
        ConnectionFactoryConfiguration cfConfig = new ConnectionFactoryConfigurationImpl("cf", true, connectorNames,
                "/cf");
        ...
        // Step 6. Start the JMS Server using the HornetQ core server and the
        // JMS configuration
    ...
    private static void initialiseHornetqClient(String connFacName, String queueName) {
        xacf = HornetQJMSClient.createConnectionFactoryWithoutHA(JMSFactoryType.XA_CF,
    ...
    private Xid startTx(XAResource xaRes) throws XAException {
        Xid xid = new DummyXid("xa-example1".getBytes(), 1,
    ...
    private void endJTATx(boolean commit)
        ...
        // no need to delist resources since that is done automatically by a JTA
        // compliant TM
        if (commit)
            tm.commit();
        else
    ...
    public int consumeMessages(int cnt, long millis) throws Exception {
        ...
        try {
            ...
            // create an XA JMS session and enlist the corresponding XA resource
            ...
            // roll back the transaction - since we consumed the messages inside
            // a transaction they should still be available
            endJTATx(true);
    ...
    public void testIsJmsWorking() throws Exception {
        ...
        try {
            ...
            // now have another go at consuming the 2 messages but this time
            // commit
            startJTATx(xaRes, new DummyXAResource(DummyXAResource.faultType.NONE));
            if (consumeMessages(xaConsumer, 1000, 2) != 2)
                throw new Exception("missing messages");
            ...
            // since the work was committed we expect there to be no more
            // messages available
