 ...
 *
 * Note: some drivers dont fully support JDBC 3.0, so some of these tests may
 * fail though no fault of our own.
 */
public class JDBC3Test extends JDBC2Test {
    ...
    public void testHoldability() throws Exception {
        ...
        if (!(conn.getHoldability() == ResultSet.HOLD_CURSORS_OVER_COMMIT)) {
            fail("holdability incorrect, set " + ResultSet.HOLD_CURSORS_OVER_COMMIT + " but read back "
                    + conn.getHoldability());
        }
        conn.setHoldability(ResultSet.CLOSE_CURSORS_AT_COMMIT);
        if (!(conn.getHoldability() == ResultSet.CLOSE_CURSORS_AT_COMMIT)) {
            fail("holdability incorrect, set " + ResultSet.CLOSE_CURSORS_AT_COMMIT + " but read back "
                    + conn.getHoldability());
        }
    }
    ...
    public void testStatements() throws SQLException {
        ...
        try {
            ...
            stmt = conn.prepareStatement("SELECT a FROM b", ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY,
                    ResultSet.CLOSE_CURSORS_AT_COMMIT);
            stmt.close();

            stmt = conn.prepareCall("SELECT a FROM b", ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY,
                    ResultSet.CLOSE_CURSORS_AT_COMMIT);
            stmt.close();
