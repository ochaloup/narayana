...
/**
 * Utility factory class to return appropriate implementations of the
 * TransactionImporter and XATerminator interfaces. This will automatically
 * instantiate the correct implementations based on the use of the JTA or JTAX
 * transaction manager configuration and is therefore preferred to instantiating
 * a specific implementation class directly.
 */
public class SubordinationManager {
    ...
    private static void initTransactionImporter() {
        ...
        if (txType == TxType.JTA) {
            ...
            // it's not JTA, so it must be JTAX. However, we are in the JTA
            // module and
            // can't link against the JTS code so we need to do it the hard
            // way...
            try {
    ...
    /**
     * Its rather tricky to figure out if we are running in JTA or JTAX(JTS)
     * mode. We can make a reasonable guess based on the transaction manager
     * implementation that is running, but it's going to break if some unknown
     * or derived impl comes along. It's therefore safer to use setTxType
