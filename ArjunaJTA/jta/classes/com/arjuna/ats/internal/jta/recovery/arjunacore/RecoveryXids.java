...
public class RecoveryXids {
    ...
    public final void nextScan(Xid[] trans) {
        ...
        // gc note: transient errors in distributed RMs may cause values to
        // disappear in one scan and then reappear later.
        // under the current model we'll recover Xids only if they stick around
        // for enough consecutive scans to
        // span the safely interval. In the unlikely event that causes problems,
        // we'll need to postpone gc for a given
        // interval and take care to include only Xids seen in the most recent
        // scan when returning candidates for recovery.
    }
    ...
    /**
     * If supplied xids contains any values seen on prev scans, replace the
     * existing XAResource with the supplied one and return true. Otherwise,
     * return false.
     *
     * @param xaResource
     * @param xids
    ...
    // record when we first saw and most recently saw a given Xid. time in
    // system clock (milliseconds).
    // since we don't trust 3rd party hashcode/equals we convert to our own
    // wrapper class.
    ...
    /**
     * JBTM-1255 this is required to reinstate JBTM-924, see message in @see
     * RecoveryXids#isStale()
     */
    private static final int staleSafetyIntervalMillis; // The advice is that
                                                        // this (if made
                                                        // configurable is twice
                                                        // the safety interval)

    // JBTM-916 removed final so 10000 is not inlined into source code until we
    // make this configurable
    // https://issues.jboss.org/browse/JBTM-842
    private static int safetyIntervalMillis; // may eventually want to make this
                                                // configurable?
