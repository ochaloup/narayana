...
public class XAResourceErrorHandler {
    ...
    public void forget() {
        ...
        /*
         * try { xaResource.forget(xid); } catch (XAException e) { // ignore }
         */
    }
    ...
    public int handleCMRCommitError(boolean onePhase) {
        ...
        jtaLogger.i18NLogger.warn_resources_arjunacore_commitxaerror(XAHelper.xidToString(xid), xaResource.toString(),
        ...
        if (onePhase) {
            switch (e.errorCode) {
                ...
                case XAException.XAER_NOTA :
                    return TwoPhaseOutcome.HEURISTIC_HAZARD; // something
                                                                ...
                                                                // without
                                                                // asking us!
                case XAException.XAER_INVAL : // resource manager failed, did it
                                                // rollback?
                    return TwoPhaseOutcome.HEURISTIC_HAZARD;
                case XAException.XA_RETRY : // XA does not allow this to be
                                            // thrown for 1PC!
                ...
                default :
                    committed = true;
                    return TwoPhaseOutcome.FINISH_ERROR; // ? recovery should
                                                            // retry
            }
            ...
            switch (e.errorCode) {
                ...
                case XAException.XA_HEURCOM : // what about forget?
                    ...
                    break;
                case XAException.XA_HEURRB :
                case XAException.XA_RBROLLBACK : // could really do with an
                                                    // ABORTED status in
                                                    // TwoPhaseOutcome to
                                                    // differentiate
                case XAException.XA_RBCOMMFAIL :
                ...
                case XAException.XAER_RMFAIL :
                    ...
                    /*
                     * Could do timeout retry here, but that could cause other
                     * resources in the list to go down the heuristic path (some
                     * are far too keen to do this). Fail and let recovery
                     * retry. Meanwhile the coordinator will continue to commit
                     * the other resources immediately.
                     */
                    return TwoPhaseOutcome.FINISH_ERROR;
                case XAException.XAER_INVAL : // resource manager failed, did it
                                                // rollback?
    ...
    /**
     * Static block puts all XAResourceMap instances defined in
     * JTAEnvironmentBean to the XAResourceErrorHandler's hash map. They are
     * later used to check if the XAException is a non-error when received in
     * reply to commit or rollback.
     */
    static {
