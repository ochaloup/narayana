...
public class XAResourceRecord extends AbstractRecord implements ExceptionDeferrer {
        ...
        try {
            ...
            switch (e1.errorCode) {
                ...
                case XAException.XAER_NOTA : // resource may have arbitrarily
                                                ...
                                                // ...)
                    return TwoPhaseOutcome.PREPARE_NOTOK; // will not call
                                                            // rollback
                default :
                    return TwoPhaseOutcome.HEURISTIC_HAZARD; // we're not really
                                                                // sure
                                                                // (shouldn't
                                                                // get here
                                                                // though).
                    ...
                    if (notAProblem(e1, false)) {
                        ...
                        switch (e1.errorCode) {
                            ...
                            case XAException.XAER_NOTA :
                                if (_recovered)
                                    break; // rolled back previously and
                                            // recovery completed
    ...
    public int topLevelCommit() {
        ...
        if (_tranID == null) {
            ...
            if (_theXAResource != null) {
                ...
                /*
                 * No need for end call here since we can only get to this point
                 * by going through prepare.
                 */
                ...
                try {
                    ...
                    if (notAProblem(e1, true)) {
                        ...
                        switch (e1.errorCode) {
                            ...
                            case XAException.XA_RBROLLBACK : // could really do
                                                                // with an
                                                                // ABORTED
                                                                // status in
                                                                // TwoPhaseOutcome
                            ...
                            case XAException.XAER_RMFAIL :
                                ...
                                /*
                                 * Could do timeout retry here, but that could
                                 * cause other resources in the list to go down
                                 * the heuristic path (some are far too keen to
                                 * do this). Fail and let recovery retry.
                                 * Meanwhile the coordinator will continue to
                                 * commit the other resources immediately.
                                 */
                                return TwoPhaseOutcome.FINISH_ERROR;
        ...
        if (_tranID == null) {
            ...
            if (_theXAResource != null) {
                ...
                try {
                     ...
                     * (http://jira.jboss.com/jira/browse/JBTM-343) Since this
                     * is 1PC we can call forget: the outcome of the transaction
                     * is the outcome of the participant.
                     */
                    ...
                    switch (e1.errorCode) {
                        ...
                        case XAException.XA_RBTRANSIENT :
                            /*
                             * Has been marked as rollback-only. We still need
                             * to call rollback.
                             */
                ...
                try {
                    /*
                     * Not strictly necessary since calling commit will do the
                     * rollback if end failed as above.
                     */

                    if (endHeuristic != null) // catch those RMs that terminate
                                                // in end rather than follow the
                                                // spec
                        throw endHeuristic;
                    ...
                    switch (e1.errorCode) {
                        ...
                        case XAException.XA_RETRY : // XA does not allow this to
                                                    // be thrown for 1PC!
                        case XAException.XAER_PROTO :
                            return TwoPhaseOutcome.ONE_PHASE_ERROR; // assume
                                                                    // rollback
                        ...
                        default :
                            ...
                            return TwoPhaseOutcome.FINISH_ERROR; // recovery
                                                                    // should
                                                                    // retry
                    }
                } catch (Exception e2) {
    ...
    public boolean forgetHeuristic() {
        ...
        // remove the connection regardless of whether or not the forget
        // operation failed
        removeConnection();
    ...
    protected int recover() {
        ...
        if (_committed) {
            /*
             * A previous commit attempt failed, but we know the intention was
             * to commit. So let's try again.
             */
            ...
            if (os.unpackInt() == RecoverableXAConnection.OBJECT_RECOVERY) {
                    ...
                    try {
                        ...
                        if (!deserialized) {
                            try {
                                ...
                                // JBTM-2550 if we fail to deserialize the
                                // object, we treat it as haveXAResource is
                                // false
                ...
                if (!haveXAResource) {
                    ...
                    if (_theXAResource == null) {
                         ...
                         * other participants that can still recover. Plus, we
                         * will try to get a new XAResource later for this
                         * instance.
                         */
    ...
    /**
     * Returns the resource manager JNDI name for e.g. xa datasource. Note this
     * is not used for lookup, only for information.
     * 
    ...
    /**
     * Sets the resource manager JNDI name. Note this is not used for lookup,
     ...
     * after recovery, then we failed after successfully committing the
     * transaction but before updating the log. In which case we just need to
     * ignore this resource and remove the entry from the log.
     *
     * BUT if not all XAResourceRecovery instances are correctly implemented (or
     * present) we may end up removing participants that have not been dealt
