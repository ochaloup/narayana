...
public class ProductComparison extends Product {
    ...
    public static void afterClass() {
        Map<String, Float> newMetrics = PerformanceProfileStore
        ...
        for (Map.Entry<String, Float> entry : oldVariances.entrySet()) {
            ...
            if (!metricName.equals(narayanaMetricName) && newVariances.containsKey(metricName)) {
                ...
                Float headRoom = Math.abs(oldVariance * (variance - 1)); // the
                                                                            // leeway
                                                                            // either
                                                                            // side
                                                                            // of
        ...
        /*
         * for (Map.Entry<String, Float> entry : newMetrics.entrySet()) { String
         * metricName = entry.getKey();
         * 
         * if (!oldMetrics.containsKey(metricName)) continue; // there is no
         * previous value to use for regression checks
         * 
         * Float canonicalValue = oldMetrics.get(metricName); // the previous
         * (best) value Float headRoom = Math.abs(oldMetrics.get(metricName) *
         * (variance - 1)); // the leeway either side of the best value Float
         * difference = (entry.getValue() - canonicalValue) / canonicalValue *
         * 100; boolean withinTolerance = (entry.getValue() >= canonicalValue -
         * headRoom);
         * 
         * if (!withinTolerance) { String s = String.
         * format("%s: %f%% performance %s (%f versus %f) (variance=%f headroom=%f)%n"
         * , metricName, difference, "regression", entry.getValue(),
         * canonicalValue, variance, headRoom);
         * 
         * failures.put(metricName, s); sb.append(s); }
         * 
         * assertEquals(sb.toString(), 0, failures.size()); }
         */
    }
