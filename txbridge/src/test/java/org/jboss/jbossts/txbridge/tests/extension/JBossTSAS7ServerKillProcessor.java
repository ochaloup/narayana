...
public class JBossTSAS7ServerKillProcessor extends JBossTSBaseServerKillProcessor {
    ...
    private static final String CHECK_JBOSS_ALIVE_CMD = "if [ \"x`" + PROCESSES_CMD
            + " | grep 'jboss-module[s]'`\" = \"x\" ]; then exit 1; fi";
    private static final String SHUTDOWN_JBOSS_CMD = PROCESSES_CMD + " | grep jboss-module[s] | awk '"
            + JBossTSServerExtension.OSType.getOSType().getPSIDIndex() + "' | xargs kill";
    private static final String CHECK_FOR_DEFUNCT_JAVA_CMD = "if [ \"x`" + PS_AUX_CMD
            + " | grep '\\[java\\] <defunct>'`\" = \"x\" ]; then exit 1; fi";
    ...
    protected boolean isDefunctJavaProcess() throws Exception {
        // Command will 'exit 1' if a defunct java process is not running and
        // 'exit 0' if there is.
