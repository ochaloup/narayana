...
public class BridgeXAResource implements XAResource, Serializable {
    // Design note: Given the way JBossTS is designed, we could subclass
    // AbstractRecord rather than
    // implementing XAResource, but this design is more standards friendly and
    // thus portable.

    private static final List<BridgeXAResource> xaResourcesAwaitingRecovery = Collections
            .synchronizedList(new LinkedList<BridgeXAResource>());
     ...
     *
     * @param externalTxId
     *            the parent JTA transaction identifier.
     * @param bridgeWrapper
     *            the control for the subordinate WS-AT transaction.
     */
    public BridgeXAResource(Uid externalTxId, BridgeWrapper bridgeWrapper) {
        txbridgeLogger.logger
                .trace("BridgeXARresource.<ctor>(TxId=" + externalTxId + ", BridgeWrapper=" + bridgeWrapper + ")");
    ...
    /**
     * Serialization hook. Gathers and writes information needed for transaction
     * recovery.
     *
     * @param out
     *            the stream to which the object state is serialized.
    ...
    /**
     * Deserialization hook. Unpacks transaction recovery information and uses
     * it to recover the subordinate transaction.
     *
     * @param in
     *            the stream from which to unpack the object state.
     * @throws IOException
    ...
    private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException {
        ...
        // this readObject method executes only when a log is being read at
        // recovery time:
        ...
        try {
            ...
            // rollback and commit will deal with bridgeWrapper == null cases
            // via ensureRecoveryIsDoneIfNeeded
    ...
    /**
     * Attempts to reconnect the BridgeWrapper if it's not already connected. If
     * XTS is not ready yet, throws the specified Exception.
     *
     * @param exceptionCode
     *            a XAException status code.
     * @throws XAException
     *             if XTS recovery is not complete.
     */
    private void ensureRecoveryIsDoneIfNeeded(int exceptionCode) throws XAException {
        if (bridgeWrapper == null) {
            // XTS recovery was not ready when deserialization ran. maybe it's
            // ready now, so try again...
            try {
                bridgeWrapper = BridgeWrapper.recover(bridgeWrapperId);
                if (bridgeWrapper == null) {
    ...
    public int prepare(Xid xid) throws XAException {
        ...
        txbridgeLogger.logger.trace(
                "prepare TwoPhaseOutcome is " + twoPhaseOutcome + "/" + TwoPhaseOutcome.stringForm(twoPhaseOutcome));
        ...
        switch (twoPhaseOutcome) {
            ...
            default :
                ...
                XAException xaException = new XAException(
                        "unexpected oucome: " + TwoPhaseOutcome.stringForm(twoPhaseOutcome));
    ...
    /**
     * Release any BridgeXAResource instances that have been driven through to
     * completion by their parent JTA transaction.
     */
     ...
     *
     * @param bridgeWrapperId
     *            the Id to search for.
     * @return true if the BridgeWrapper is known to be awaiting recovery, false
     *         otherwise.
     */
    ...
    public void commit(Xid xid, boolean onePhase) throws XAException {
        ...
        // if XTS has not finished recovery yet we wont be able to complete
        // until it does.
