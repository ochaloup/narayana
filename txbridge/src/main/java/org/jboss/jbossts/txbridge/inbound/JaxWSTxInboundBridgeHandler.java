...
/**
 * A handler that sits in the server side JAX-WS processing pipeline between the
 * XTS header context processor and the web service. Takes the WS transaction
 * context provided by the former and maps it to a JTA transaction context for
 * use by the latter. Handles Thread association of the JTA context.
 *
...
public class JaxWSTxInboundBridgeHandler implements Handler {
    ...
    /**
     * Process inbound messages by mapping the WS transaction context to a JTA
     * one and associating the latter to the current Thread.
     *
     * @param msgContext
     *            unused
     * @return true on success, false on error
    ...
    /**
     * Break the association between the JTA transaction context and the calling
     * Thread.
     *
     * @return true on success, false on error
     */
