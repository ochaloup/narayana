...
/**
 * Maintains the mapping data that relates JTA transactions to WS-AT subordinate
...
public class OutboundBridgeManager {
    ...
    /**
     * Return an OutboundBridge instance that maps the current Thread's JTA
     * transaction context to a WS-AT transaction context. Control of the latter
     * is provided by the returned instance.
     *
     * @return as OutboundBridge corresponding to the calling Thread's current
     *         JTA transaction context.
     */
    ...
    /**
     * Create a WS-AT transaction mapping and support objects for a given JTA
     * transaction context.
     *
     * @param externalTxId
     *            The JTA transaction identifier.
    ...
    private static synchronized void createMapping(Transaction transaction, Uid externalTxId) throws SystemException {
        ...
        org.jboss.jbossts.txbridge.outbound.OutboundBridge outboundBridge = new org.jboss.jbossts.txbridge.outbound.OutboundBridge(
                bridgeWrapper);
