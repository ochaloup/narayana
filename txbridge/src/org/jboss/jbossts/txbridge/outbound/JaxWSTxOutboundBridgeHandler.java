...
/**
 * A handler that sits in the client side JAX-WS processing pipeline between the
 * application and the XTS header context processor. Takes the JTA transaction
 * context provided by the former and maps it to a WS-AT transaction context for
 * use by the latter. Handles Thread association of the WS-AT context.
 *
 * Note: we assume that there is a JTA transaction context present and that the
 * service needs a WS-AT context. The handler should not be registered on
...
public class JaxWSTxOutboundBridgeHandler implements Handler {
    ...
    /**
     * Tidy up the Transaction/Thread association before returning a message to
     * the client.
     *
     * @param msgContext
     *            unused
    ...
    /**
     * Process outbound messages by mapping the JTA transaction context to a
     * WS-AT one and associating the latter to the current Thread.
     *
     * @param msgContext
     *            unused
     * @return true on success, false on error
    ...
    /**
     * Break the association between the WS-AT transaction context and the
     * calling Thread.
     *
     * @return true on success, false on error
     */
