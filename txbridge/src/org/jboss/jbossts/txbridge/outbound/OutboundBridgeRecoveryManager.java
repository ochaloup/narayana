...
public class OutboundBridgeRecoveryManager implements RecoveryModule {
    ...
    public void periodicWorkSecondPass() {
        ...
        // the JTA top level tx recovery module is registered and hence run
        // before us. Therefore by the time
        // we get here we know readObject has been called for any
        // BridgeXAResource for which a log exists.
        // thus if it's not in our xaResourcesAwaitingRecovery list by now, it's
        // presumed rollback.
        ...
        for (BridgeWrapper bridgeWrapper : bridgeWrappers) {
            if (!BridgeXAResource.isAwaitingRecovery(bridgeWrapper.getIdentifier())) {
                txbridgeLogger.logger
                        .trace("rolling back orphaned subordinate BridgeWrapper " + bridgeWrapper.getIdentifier());
