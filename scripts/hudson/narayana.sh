...
function build_as {
  ...
  (cd .. && ./build.sh -f jboss-as/pom.xml -B clean install -DskipTests -Dts.smoke=false $IPV6_OPTS -Drelease=true -Dversion.org.jboss.jboss-transaction-spi=7.1.0.SP2 -Dversion.org.jboss.jbossts=4.17.44.Final-SNAPSHOT $AS_XARGS)
...
function xts_as_tests {
  ...
  (cd ../ && ./build.sh -f ./jboss-as/testsuite/integration/xts/pom.xml -B -Pxts.integration.tests.profile -Dversion.org.jboss.jboss-transaction-spi=7.1.0.SP2 -Dversion.org.jboss.jbossts=4.17.44.Final-SNAPSHOT  "$@" test $AS_XARGS)
