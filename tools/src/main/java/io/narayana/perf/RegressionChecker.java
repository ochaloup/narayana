...
public class RegressionChecker {
    ...
    public final static Double DEFAULT_VARIANCE = 1.1; // percentage _variance
    ...
    public RegressionChecker() throws IOException {
        this(BASE_DIR + File.separator + PERF_ARGS_FILENAME, BASE_DIR + File.separator + PERF_DATA_FILENAME,
                BASE_DIR + File.separator + PERF_VAR_FILENAME);
    }

    public RegressionChecker(String perfArgsFileName, String perfHistoryFileName, String perfVarFileName)
            throws IOException {
    ...
    /**
     * If isFailOnRegression is false then new metrics will not be persisted to
     * the metrics store
     * 
     ...
     *            {@link RegressionChecker#getTestArgs(String)}
     * @param index
    ...
    public Map<String, Double> getMatchingMetrics(String pattern) {
        ...
        while (e.hasMoreElements()) {
            ...
            if (key.matches(pattern))
                matches.put(key, Double.valueOf(testHistory.getProperty(key))); // data
                                                                                // must
                                                                                // contain
                                                                                // numbers
        }
    ...
    boolean isWithinTolerance(StringBuilder info, String metricName, double metricValue, double canonicalValue,
            double variance, boolean largerIsBetter) {
        ...
        String s = String.format("%s %s: %f%% performance %s (%f versus %f) (variance=%f headroom=%f)", metricName,
                within ? "Passed" : "Failed", difference, within ? "difference" : "regression", metricValue,
                canonicalValue, variance, headRoom);
    ...
    boolean updateMetric(StringBuilder info, double variance, String metricName, double metricValue,
