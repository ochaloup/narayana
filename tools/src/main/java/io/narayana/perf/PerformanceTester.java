 ...
 *
 *         Workload runner for measuring the maximum throughput of an instance
 *         of @see Worker
 * @deprecated replaced by
 *             {@link Result#measure(WorkerLifecycle, WorkerWorkload)}}
 */
@Deprecated
public class PerformanceTester<T> {
     ...
     *
     * @param maxThreads
     *            the number of threads to use to complete the workload
     * @param batchSize
     *            the size of each batch of work to pass to @see Worker tasks
     */
    public PerformanceTester(int maxThreads, int batchSize) {
        THREAD_POOL_SIZE = maxThreads > 0 ? maxThreads : DEF_THREAD_POOL_SZ; // must
                                                                                // be
                                                                                // >=
     ...
     *
     * @param worker
     *            a multithreaded worker for running batches of work
     * @param config
     *            config parameters for the run
     * @return
     */
    ...
    public Result<T> measureThroughput(WorkerLifecycle<T> lifecycle, WorkerWorkload<T> workload, int callCount,
    ...
    private Result<T> doWork(final WorkerWorkload<T> workload, final Result<T> opts) {
        ...
        final CyclicBarrier cyclicBarrier = new CyclicBarrier(opts.getThreadCount() + 1); // workers
                                                                                            // +
                                                                                            // self
