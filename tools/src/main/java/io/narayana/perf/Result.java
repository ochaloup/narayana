...
public class Result<T> implements Serializable {
    ...
    private long maxTestTime = 0; // TODO terminate measurement if test takes
    ...
    Result(long maxTestTime, int threadCount, int numberOfCalls, int batchSize) {
        ...
        if (numberOfBatches < this.threadCount) {
            System.err.printf("Too few batches - reducing thread count (%d %d %d)%n", this.threadCount, numberOfBatches,
                    this.numberOfCalls);
    ...
    public String toString() {
        return String.format("%d calls / second (%d calls in %d ms using %d threads. %d errors)", getThroughput(),
                getNumberOfCalls(), getTotalMillis(), getThreadCount(), getErrorCount());
    }
    ...
    public Result<T> measure(final WorkerLifecycle<T> lifecycle, final WorkerWorkload<T> workload,
            int warmUpCallCount) {
