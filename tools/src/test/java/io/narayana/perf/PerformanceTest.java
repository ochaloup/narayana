...
public class PerformanceTest {
    ...
    public void testPerformanceTester() {
        ...
        try {
            Result<BigInteger> measurement = tester.measureThroughput(worker, worker, numberOfCalls, threadCount,
                    batchSize);
            ...
            assertTrue("init method called after work method",
                    worker.getInitTimemillis() <= worker.getWorkTimeMillis());
            assertTrue("work method called after fini method",
                    worker.getWorkTimeMillis() <= worker.getFiniTimeMillis());
    ...
    public void testPerformanceTester2() {
        ...
        try {
            Result<BigInteger> measurement = new Result<BigInteger>(threadCount, numberOfCalls, batchSize)
