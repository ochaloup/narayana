...
/**
 * The <CODE>BankClient</CODE> application is an interactive CLI that allows the
 * user of the JBoss Transactions product to manipulate a database backed bank
...
public class BankClient {
    ...
    /**
     * Whether the bank object's underlying database should be dropped (if
    ...
    /**
     * Create a new BankClient indicating the Bank to execute updates and
     * queries upon.
     *
     * @param bank
     *            A reference to the bank to update.
     */
    ...
    private static int menu() {
        ...
        String choice = input();
        // Can only be null if there was a problem reading the input from the
        // user.
        if (choice == null)
            return 0;
        ...
        try {
            ...
            // If the user input was not a number return as if the user asked to
            // redisplay the menu.
            return 6;
        }
    }
     ...
     *
     * @return The input from the keyboard, or null if there is a problem
     *         reading the data.
     */
    ...
    /**
     * This will display the menu in a loop asking for user input to create
    ...
    /**
     * Create a new bank account. This will reserve space in the database for
     * the account and it's initial amount of cash.
     */
    private void newAccount() {
        ...
        try {
            // Obtain a reference to the user transaction object inorder to
            // transactionally perform this work
            ...
            // Begin the transaction so all work now will be executed under
            ...
            // Commit the transaction and disassociate this thread from the
            // transaction
    ...
    /**
     * Get the current balance of a required bank account. This will look at the
     * database record for the account specified.
     */
    private void getInfo() {
        ...
        try {
            ...
            try {
                ...
                // Quite an artifical example as rolling back the read operation
                // is not too useful but
    ...
    /**
     * This will allow the bank to transactionally move money from one account
     * to another.
     */
    ...
    /**
     * Credit a specified bank account with the required amount. This method
     * will transactionally update the underlying datastore with the amount of
     * cash to add.
     */
    ...
    /**
     * Debit the specified account by the required value. This will
     * transactionally remove funds from the account by executing an update on
     * the underlying JDBC data.
     */
    ...
    /**
     * This is the entry point into the JBoss Transaction product trailmap
     * sample that uses a JDBC database to store the bank accounts repository.
     * At minimum it should be invoked with the "-host [db_host] -port
     * [db_port]" flags.
     *
     * @param args
     *            The arguments to configure this run of the JDBC backed
     *            trailmap example
     */
    public static void main(String[] args) {
        // Parse the command line options to determine any required
        // configuration for the bank application
        for (int i = 0; i < args.length; i++) {
            ...
            if (args[i].compareTo("-create") == 0)
                create = true;
            // Hack our check to ensure that the argument length is OK
            // (indicates host and port
            if (args.length < 4 || args[i].compareTo("-help") == 0) {
                // Display help information about the parameters supported by
                // this trailmap example
                System.out.println(
                        "Usage: java BankClient [-host] [-port] [-dbName] [-username] [-password] [-clean] [-create] [-help]");
                System.exit(0);
            }
        }
        try {
            // Create a new oracle XA DataSource, this datasource is used to
            // indicate the location of the database to
            // store bank account details within
            // Use dynamic class loading to remove any trailmap build time
            // dependency on the Oracle driver
            ...
            // Use reflection as the datasource API does not provide a setURL
            // method
            ...
            // The datasource must be able to be resolved by the transaction
            // manager so place it in a file system
            // based JNDI, this implies that other machines can see this file
            // system location
            Hashtable env = new Hashtable();
            env.put(Context.INITIAL_CONTEXT_FACTORY, "com.sun.jndi.fscontext.RefFSContextFactory");
            env.put(Context.PROVIDER_URL, "file:/tmp/JNDI");
            InitialContext ctx = new InitialContext(env);
            // This is the object to use so rebind rather than bind, some JNDI
            // implementations would protect this
            // with the requirement to add
            // <CODE>Context.SECURITY_CREDENTIALS</CODE> or the like
        ...
        // Initialize the property manager with the correct JNDI credentials to
        // use to log in to JNDI
        ...
        // Create a local representation of a bank and then create a bank client
        // to interact with it
        Bank bank = new Bank();
