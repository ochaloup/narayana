...
/**
 * This class shows a simple implementation of XAResourceRecovery. It shows how
 * even though NonSerializableExampleXAResource is not <CODE>Serializable</CODE>
 * the resource is recovered via this recovery module that is used by the
 * recovery manager to periodically scan the resource for indoubt transaction
 * XIDs.
 *
 * An <CODE>XAResourceRecovery</CODE> class is analagous to an
 * <CODE>Iterator</CODE> or <CODE>Enumeration</CODE>. Implementations will have
 * their getXAResource method invoked each time they return true to
 * hasMoreResources.
 */
public class NonSerializableExampleXAResourceRecovery implements XAResourceRecovery {
    ...
    /**
     * Used to count the times the resource has been returned to the transaction
     * manager.
     */
     ...
     *
     * @param initializer
     *            An arbitrary string from which initialization data is
     *            obtained.
     *
    ...
    public boolean hasMoreResources() {
        ...
        // We know there is only one resource to recover, this method will
        // return true twice
        if (m_count < 2) {
            toReturn = true;
            m_count++;
    ...
    /**
     * Get an XAResource. This implementation will create the next XA resource
     * to recover.
     *
