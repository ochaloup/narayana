...
/**
 * This is a trivial implementation of a non-serializable XAResource. It will
 * crash (<CODE>System.exit</CODE>) when commit is called on it for the first
 * time and will then complete normally when it is recovered.
 *
...
public class NonSerializableExampleXAResource implements XAResource {
    ...
    /**
     * Create a new NonSerializableExampleXAResource. This resource is used to
     * crash the VM.
     *
     * @param name
     *            The name to associate with the XA resource.
     * @param recover
     *            If this XA resource is recovering.
     */
    ...
    /**
     * Either crashes the resource (if it wasn't recovered) or commits the
     * resource normally (if it was recovered).
     *
     * @param xid
     *            A global transaction identifier
     * @param onePhase
     *            If true, use a one-phase commit protocol to commit the work
     *            done on behalf of <code>Xid</code>.
     *
     * @throws XAException
    ...
    public void commit(Xid xid, boolean onePhase) throws XAException {
        ...
        if (!m_recovered) {
            // If the commit is one phase we will not have prepared yet so write
            // out the XID now
            if (onePhase) {
                persistXid(xid);
     ...
     * is ready to commit the transaction. The XID is saved so that it could be
     * recovered
     *
     * @param xid
     ...
     * <p>
     * This implementation returns the XID that was saved during prepare This
     * saved XID will be commited, the newly created XID will be rolled back.
     *
     * <p>
     * Note: It is the responsibility of the Transaction Manager to ignore
     * transactions that do not belong to it. Therefore this method could return
     * non-Arjuna XIDs which should be ignored.
     *
     * @param flag
     *            One of TMSTARTRSCAN, TMENDRSCAN, TMNOFLAGS. TMNOFLAGS must be
     *            used when no other flags are set in the parameter.
     *
     * @return zero or more XIDs of the transaction branches that are currently
    ...
    public Xid[] recover(int flag) throws XAException {
        ...
        switch (flag) {
            case XAResource.TMSTARTRSCAN :
                ...
                // If the example application detects that there is an XID to
                // commit it will return it
                if (commitXID != null) {
                    xids = new Xid[1];
                    xids[0] = commitXID;
     ...
     * TMRESUME specified.
     * <li>If TMFAIL is specified, the portion of work has failed. The resource
     * manager may mark the transaction as rollback-only
     * <li>If TMSUCCESS is specified, the portion of work has completed
     * successfully. /ul>
     *
     * @param xid
     *            A global transaction identifier that is the same as the
     ...
     *
     * @throws XAException
     *             This implementation does not throw exception
     *
     * @see XAResource
     */
     ...
     * default value used by the resource manager, set the value to zero.
     *
     * @param seconds
     ...
     * <li>If TMRESUME is specified, the start applies to resuming a suspended
     * transaction specified in the parameter xid.
     * <li>If neither TMJOIN nor TMRESUME is specified and the transaction
     * specified by xid has previously been seen by the resource manager, the
     * resource manager throws the XAException exception with XAER_DUPID error
     * code.
     * </ul>
     *
     * @param xid
     ...
     *
     * @param recovered
     *            The recovery mode for this resource.
     */
     ...
     *
     * @param msg
     *            The message to append to the console and a file
     *            "./Resource[count].log"
     */
     ...
     *
     * @param toPersist
     *            The XID to persist in a file "./Resource[count].xid"
     */
    private void persistXid(Xid toPersist) {
        ...
        try {
            ...
            System.err.println(
                    "Trailmap IO problem: Could not persist XID to file \"" + fileName + "\", check stack trace");
            ioe.printStackTrace();
        }
    }
    ...
    private Xid readXid() {
        ...
        if (file.exists()) {
            try {
                ...
                System.err.println("Trailmap IO problem: Could not read XID persisted in file \"" + fileName
                        + "\", check stack trace");
                ioe.printStackTrace();
            } catch (ClassNotFoundException cnfe) {
                System.err.println("Trailmap IO problem: Class not found for XID persisted in file \"" + fileName
                        + "\", check stack trace");
