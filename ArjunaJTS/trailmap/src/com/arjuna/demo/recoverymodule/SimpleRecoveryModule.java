...
/**
 * BEWARE: Implementing a <CODE>RecoveryModule</CODE> is a very advanced feature
 * of the transaction service. It should only be performed by users familiar
 * with the all the concepts used in the JBoss Transactions product. Please see
 * the ArjunaCore guide for more information about <CODE>AbstractRecord</CODE>s.
 *
 * This implementation of the recovery module indicates how to write a recovery
 * module to detect if an object is in an inconsistent state. This
 * implementation also shows how the recovery module implementation requires
 * deep knowledge of the specific type that it is responsible for recovering.
 */
public class SimpleRecoveryModule implements RecoveryModule {
    /**
     * The name of the file which contains the current state of the current
     * <CODE>SimpleRecord</CODE>.
     */
    private static String filename = SimpleRecord.filename;

    /**
     * Only used to display information to the user when the recovery manager is
     * loaded.
     */
    ...
    /**
     * This method is called by the recovery subsystem to allow this module to
     * check for uncommited <CODE>SimpleRecord</CODE> states.
     *
     * This implementation is a simple system that only allows one outstanding
     * <CODE>SimpleRecord</CODE> and so we check the state of this record.
     */
    public void periodicWorkFirstPass() {
        ...
        if (simpleRecord.exists()) {
            try {
                ...
                // We know with this implementation that it would be an error if
                // an empty file exists as SimpleRecord
                ...
                // NOTE: If this recovery module could handle more than one
                // outstanding SimpleRecord it would now be
                // possible to remember that the record exists in a prepared
                // state and during the second pass of
        ...
        if (simpleRecord.exists()) {
            try {
                ...
                if (stringState.compareTo("I'm prepared") == 0) {
                    ...
                    // NOTE: A full implementation of recovery manager could now
                    // try to recover the the
                    // <CODE>SimpleRecord</CODE>
                }
                // The recovery module is only really interested in finding
                // prepared objects, this implementation
