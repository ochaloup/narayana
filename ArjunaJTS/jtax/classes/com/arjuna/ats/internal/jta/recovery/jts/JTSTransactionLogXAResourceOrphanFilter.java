 ...
 * <p>
 * See the JTA filter at {@link JTATransactionLogXAResourceOrphanFilter}.
 */
public class JTSTransactionLogXAResourceOrphanFilter implements XAResourceOrphanFilter {
    ...
    private boolean isLogRecordForTheXid(Xid xid) throws ObjectStoreException, IOException {
        ...
        String transactionType = com.arjuna.ats.internal.jts.orbspecific.coordinator.ArjunaTransactionImple.typeName();
        ...
        if (!u.equals(Uid.nullUid())) {
            if (jtaxLogger.logger.isDebugEnabled())
                jtaxLogger.logger.debugf("Looking for uid '%s' of transaction type %s", u, transactionType);
