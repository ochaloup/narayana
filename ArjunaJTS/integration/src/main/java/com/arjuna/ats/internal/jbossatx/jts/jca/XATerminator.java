...
public class XATerminator extends XATerminatorImple implements ExtendedJBossXATerminator {
        ...
        try {
            ...
            switch (tx.getStatus()) {
                ...
                case Status.STATUS_UNKNOWN :
                    throw new WorkCompletedException(jbossatxLogger.i18NLogger.get_jts_jca_inactive(),
                            WorkException.TX_RECREATE_FAILED);
                case Status.STATUS_ACTIVE :
                    break;
                default :
                    throw new WorkCompletedException(jbossatxLogger.i18NLogger.get_jts_jca_completing(),
    ...
    public Transaction getTransaction(Xid xid) throws XAException {
        ...
        if (txn == null) {
            /*
             * If it wasn't created locally. Check to see if it has been
             * imported from another server. Note that: - this call may reload
             * the transaction from disk - will throw exceptions if it has
