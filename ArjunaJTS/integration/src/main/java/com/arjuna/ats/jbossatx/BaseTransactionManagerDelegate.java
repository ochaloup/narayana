...
public abstract class BaseTransactionManagerDelegate
    ...
    /**
     * register a listener for transaction related events that effect the
     * current thread
     * 
     * @param listener
     ...
     */
    @Override
    public void addListener(Transaction transaction, TransactionListener listener, EnumSet<EventType> types)
            throws TransactionTypeNotSupported {
        if (transaction == null)
            throw new NullPointerException(); // we could interpret this as
                                                // meaning register for all
                                                // transactions
        ...
        if (listeners != null) {
            ...
            // if transaction is already associated with the current thread
            ...
            try {
                ...
                // no transaction associated so do not trigger the ASSOCIATED
                // callback
            }
        }
    }
    ...
    private Collection<TransactionListener> getListeners(Transaction transaction, boolean create) {
        ...
        // protect against two concurrent listener registrations both trying to
        // create the initial resource entry
    ...
    // stateless, we just delegate down to the object storage on the
    // TransactionImple

    // Note this has some interesting effects around Transaction termination.
    // The TransactionImple instance
    // lifetime is up to tx termination only. After that getTransaction() on the
    // tm returns a new instance
    // representing the same tx. Hence TransactionLocal state goes away
    // magically at tx termination time
    // since it's part of the TransactionImple instance. That's what we want and
    // saves writing cleanup code.
    // On the down side, since locks use the same storage they go away too. This
    // causes us to have to
    // jump through some hoops to deal with locks vanishing and maybe never
    // getting unlocked, see below.

    /**
     * Does the specified transaction contain a value for the transaction local.
     *
     * @param transactionLocal
     *            The associated transaction local.
     * @param transaction
     *            The associated transaction.
     * @return true if a value exists within the specified transaction, false
     *         otherwise.
     */
     ...
     */
    public void storeValue(final TransactionLocal transactionLocal, final Transaction transaction, final Object value) {
        com.arjuna.ats.jta.transaction.Transaction transactionImple = (com.arjuna.ats.jta.transaction.Transaction) transaction;
        if (transactionImple.isAlive()) {
     ...
     */
    public void lock(final TransactionLocal local, final Transaction transaction) throws InterruptedException {
        com.arjuna.ats.jta.transaction.Transaction transactionImple = (com.arjuna.ats.jta.transaction.Transaction) transaction;
        if (transactionImple.isAlive()) {
            // There is a race here, as the transaction may terminate between
            // the check
            // and the lock attempt. See lock() implementation below.
            // It's still possible to lock a dead transaction, but that does no
            // real harm.
    ...
    // Lock implementation: This used to use a Synchronization for lock storage,
    // but
    // we need to be able to lock things in some states where registration of
    // Synchronizations is not permitted. Besides, the JTA 1.1 work gives us a
    // nice
    // general object storage mechanism on a TransactionImple, so we use that.

    // This is the key under which the map of TransactionLocals to locks is
    // stored.
    // Bad things will probably happen if users ever use this key themselves
    private final String LOCKS_MAP = "__LOCKS_MAP";

    // Using string LOCKS_MAP directly as object of synchronization is not
    // recommended
    ...
    // locate and return the lock for a given TransactionLocal+Transaction
    // tuple.
    ...
    private TransactionLocalLock findLock(final TransactionLocal local, final Transaction transaction) {
        ...
        // ideally for performance we should sync on the tx instance itself but
        // that may have nasty
        // side effects so we use something else as the lock object for the sync
        // block
        ...
        if (locks == null) {
            synchronized (locksMapLock) {
                // ensure there is a holder for lock storage on the given tx
                // instance.
                locks = (Map) transactionImple.getTxLocalResource(LOCKS_MAP);
                if (locks == null) {
                    locks = new HashMap(); // <TransactionLocal,
                                            // TransactionLocalLock>
    ...
    private class TransactionLocalLock {
        ...
        /**
         * Lock the transaction local within the current thread context. true on
         * lock acquired, false otherwise
         */
        public boolean lock(com.arjuna.ats.jta.transaction.Transaction tx) {
            // The current code in the app server locks the transaction for all,
            // we follow that practice
            synchronized (lock) {
                ...
                while (lockingThread != null) {
                    try {
                        // lock object instances get thrown away at Transaction
                        // termination. That makes it impossible
                        // to call unlock() on them. Searching through them and
                        // unlocking them from the transaction
                        // termination code is a pain and finalizers suck.
                        // Hence we need to make sure we don't wait forever for
                        // a notify
                        // that will never come. Probably the only thing that
                        // will terminate a Transaction in another
                        // Thread is the transaction reaper, so we wait not
                        // longer than the tx timeout plus a fudge factor.
                        long timeout = 0;
                        try {
        ...
        public void unlock() {
            synchronized (lock) {
                if (lockCount == 0 && lockingThread == null) {
                    ...
                    // we fail silent to save the caller having to deal with
                    // race condition.
                    return;
                }
