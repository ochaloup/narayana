...
/**
 * This class is a wrapper around a PropagationContext object allowing it to be
 * serialized.
 *
 * @author Richard A. Begg (richard.begg@arjuna.com)
 * @version $Id: PropagationContextWrapper.java,v 1.5 2004/10/04 09:48:19 nmcl
 *          Exp $
 */
    ...
    public PropagationContext getPropagationContext() {
        ...
        if (_tpc == null) {
            if (_propagateFullContext) {
                ...
                _tpc = new PropagationContext(_timeout, _current.getTransIdentity(), parents, null);
            } else {
                _tpc = new PropagationContext(_timeout, _current.getTransIdentity(), new TransIdentity[0], // no
                                                                                                            // parents,
                                                                                                            // but
                                                                                                            // not
                                                                                                            // null
    ...
    private class TransIdentityWrapper implements Serializable {
        ...
        public TransIdentity getTransIdentity() {
            return new TransIdentity(CoordinatorHelper.narrow(ORBManager.getORB().orb().string_to_object(_coordinator)),
                    _terminator == null
                            ? null
                            : TerminatorHelper.narrow(ORBManager.getORB().orb().string_to_object(_terminator)),
                    _otid);
        }
         ...
         * and arrays.
         *
         * @serialData Overriding methods should use this tag to describe the
         *             data layout of this Externalizable object. List the
         *             sequence of element types and, if possible, relate the
         *             element to a public/protected field and/or method of this
