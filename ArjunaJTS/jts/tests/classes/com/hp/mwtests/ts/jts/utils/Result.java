...
public class Result implements Serializable {
    ...
    public Result(boolean local, int threadCount, int numberOfCalls, int enlist, boolean cmt, boolean transactional,
            long prepareDelay, boolean verbose, boolean showHeader, boolean useHtml, String storeType) {
        this.local = local;
    ...
    public Result(Result result) {
        this(result.local, result.threadCount, result.numberOfCalls, result.enlist, result.cmt, result.transactional,
                result.prepareDelay, result.verbose, true, true, result.storeType);
    ...
    public String toString() {
        if (useHtml)
            ...
            return String.format("%12s %12tT %16s %11d %6d %9d %9s %11d %13b %6d %6b %12s%n", hostName, calendar,
                    productVersion, getThroughputBMT(), getNumberOfCalls(), getErrorCount(), patchedJacorb, threadCount,
                    transactional, enlist, !isLocal(), storeType);
    }

    public static StringBuilder getHeaderAsText(StringBuilder sb) {
        return sb.append(String.format("%12s %12s %16s %11s %6s %9s %9s %11s %13s %6s %6s %12s%n", "Hostname",
                "Time of Day", "Version", "Throughput", "Calls", "Errors", "Patched", "Threads", "Transaction",
                "Enlist", "Remote", "StoreType"));
    }
    ...
    public String toHtml() {
        if (verbose) {
            return String.format(
                    "<tr>\n<td>%d</td>\n<td>%d</td>\n<td>%d</td>\n<td>%b (%b)</td>\n<td>%b</td>\n<td>%d</td>\n<td>%d (%d)</td>\n</tr>\n",
                    getNumberOfCalls(), getErrorCount(), getThreadCount(), isTransactional(), getEnlist() > 0,
                    !isLocal(), getTotalMillis(), getThroughputBMT(), getThroughputCMT());
        } else {
    ...
    public static Result toResult(Map<String, String[]> opts) {
        Result res = validateOpts(new Result(getBooleanParameter(opts, "local", false),
                getIntegerParameter(opts, "threads", 1), getIntegerParameter(opts, "count", 100),
                getIntegerParameter(opts, "enlist", 1), getBooleanParameter(opts, "cmt", true),
                getBooleanParameter(opts, "transactional", true), getIntegerParameter(opts, "prepareDelay", 0),
                getBooleanParameter(opts, "verbose", true), getBooleanParameter(opts, "show_header", true),
                getBooleanParameter(opts, "html", true), getStringParameter(opts, "store_type", "Unknown")));
