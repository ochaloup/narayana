...
public class PerfHammer {
    private static int getArg(String[] args, int index, int defaultValue) {
        try {
            ...
            throw new NullPointerException(new PerfHammer().getClass().getName()
                    + "test arguments in the PerformanceProfileStore invalid: " + e.getMessage());
        }
    ...
    public static void main(String[] args) throws Exception {
        ...
        String metricName = "JTSRemote_PerfTest_PerfHammer_"
                + System.getProperty("org.omg.CORBA.ORBClass", myORB.orb().getClass().getName());

        System.out.printf("%s: %d iterations using %d threads with a batch size of %d%n", metricName, numberOfCalls,
                threadCount, batchSize);

        GridWorker worker = new GridWorker(myORB, gridReference);

        Measurement measurement = PerformanceProfileStore.regressionCheck(worker, metricName, true, 0, numberOfCalls,
                threadCount, batchSize);
