...
/**
 * This class is responsible for managing the thread-to-transaction context
 * mappings.
 *
...
public class ContextManager {
    public ContextManager() {
        ...
        try {
            _piCurrent = org.omg.PortableInterceptor.CurrentHelper
                    .narrow(ORBManager.getORB().orb().resolve_initial_references("PICurrent"));
    ...
    /**
     * Get the current transaction associated with the invoking thread. Do not
     * look in the PI thread data.
     *
    ...
    /**
     * Get the transaction for the invoking thread. If there isn't one in the
     * normal thread associated data then look in the PI implementation.
     *
    ...
    public ControlWrapper current() throws SystemException {
        ...
        /*
         * If we do not have a context currently, then check to see if we have
         * just been spawned to handle a transactional invocation. If so, there
         * may be a context handle associated with this thread in piCurrent.
         *
         * We only do this for the current thread, hence the difference between
         * the two versions of ContextManager.current.
         */
    ...
    public final ControlWrapper popAction(String threadId) {
        ...
        if (arg != null) {
            ...
            try {
                /*
                 * When we pushed the action we did the check for whether it was
                 * local to save time now.
                 */
            ...
            /*
             * If size now zero we can delete from thread specific data.
             */
        ...
        /*
         * Now update action in thread's notion of current if this action is
         * local.
         */
        ...
        if (action != null) {
            ...
            if (action.isLocal()) {
                /*
                 * If transaction is terminated by another thread then our
                 * thread-action information may have already been removed from
                 * the action.
                 */
    ...
    public final void purgeActions(String threadId) {
        ...
        /*
         * Don't do anything with these actions, i.e., do not commit/abort them.
         * Just because this thread is finished with them does not mean other
         * threads are!
         */
    ...
    /**
     * We could maintain a list of suspended action hierarchies and resume the
     * right one (and the right place!) given the control. However, this can
     * lead to memory leaks, since we never know when to remove this hierarchy
     * information. So, for now we simply rely on the propagation context.
     */
            ...
            try {
                ...
                if (ctx != null) {
                    ...
                    for (int i = depth - 1; i >= 0; i--) {
                        /*
                         * No memory leak as we delete either when suspend is
                         * called, or the transaction is terminated.
                         */
                        ...
                        pushAction(new ControlWrapper(theControl)); // takes
                                                                    // care of
                                                                    // thread/BasicAction
                                                                    // for us.
                    }
    ...
    /*
     * Given a ControlWrapper we can create the hierarchy quickly, since we have
     * the implementation information to hand.
     */
     ...
     * registered with BasicAction. Also, they will either all be remote
     * references to controls, or all but the current action will be
     * proxy/wrapper controls, i.e., controls which contain references to the
     * remote coordinator/terminator.
     */
    ...
    public final void pushAction(ControlWrapper action) {
        ...
        /*
         * If it's a local transaction then save the transaction pointer. We'll
        ...
        if (action.isLocal()) {
            ...
            /*
             * Given a Control we can maintain a mapping to the actual action.
             *
             * Do we want this to work for remote actions? Yes, because we want
             * all actions to know about active threads, even those that are
             * remote. (But we don't do it yet!)
             *
             * Call action to increment number of threads. This is all we need
             * to do for remote actions. If local, we need to make this action
             * the current action.
             */
    ...
    public ControlWrapper currentPIContext() throws SystemException {
        if (_piCurrent != null) {
            try {
                ...
                /*
                 * If we have something then we must be a server thread. In
                 * which case we save the thread id so that the server
