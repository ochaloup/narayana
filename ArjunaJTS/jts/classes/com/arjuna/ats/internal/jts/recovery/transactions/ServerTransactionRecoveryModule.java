 ...
 * for the recovery of server transactions
 *
 */
        ...
        while (transactionUidEnum.hasMoreElements()) {
            ...
            try {
                ...
                if (_recoveryStore.currentState(currentUid, _transactionType) != StateStatus.OS_UNKNOWN) {
                    ...
                    if (jtsLogger.logger.isDebugEnabled()) {
                        jtsLogger.logger.debug("ServerTransactionRecoveryModule - Transaction " + currentUid
                                + " still in state unknown (?).");
                    }
                }
            } catch (ObjectStoreException e4) {
                if (jtsLogger.logger.isDebugEnabled()) {
                    jtsLogger.logger.debug("ServerTransactionRecoveryModule - Transaction " + currentUid
                            + " is not in object store - assumed completed");
                }
            }
        }
