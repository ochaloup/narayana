...
/**
 * The transaction cache ensures that there is no interference between different
 * threads that may be working with the same transaction. The cache
 * automatically removes or refreshes transactions that require no further
 * recovery or re-recovery respectively. Various volatile information for the
 * transaction is also kept in the cache (e.g. number of attempts to recover
 * since first activated in this RecoveryManager run)
 * <P>
 * 
...
public class TransactionCache {
     ...
     * is not needed) NoTransaction means the original process has gone
     */
    ...
    /**
     * Add a new resource to a recovered transaction. This is primarily to allow
     * a new resource that has been provided through a replay_completion to be
     * added to the transaction and thereby replacing the original resource that
     * was passed in on register_resource.
     */
    public static void addResourceRecord(Uid actionUid, String theType, Uid rcUid, Resource r) {
        ...
        if (cacheItem != null) {
            synchronized (cacheItem) {
                RecoveringTransaction theTransaction = cacheItem.freshTransaction();
                if (theTransaction != null) {
                    // As long as the transaction activated okay then try and
                    // add
                    // the record.
                    if (theTransaction.getRecoveryStatus() != RecoveryStatus.ACTIVATE_FAILED) {
                        theTransaction.addResourceRecord(rcUid, r);
                    }
                    // with a new resource record, start counting attempts from
                    // zero
        ...
        if (cacheItem != null) {
            synchronized (cacheItem) {
                ...
                if (theTransaction != null) {
                    ...
                    if (theTransaction.getRecoveryStatus() != RecoveryStatus.ACTIVATE_FAILED) {
                        /*
                         * if the transaction is known to be committed, make
                         * only a limited number of attempts before assuming the
                         * subordinate resources have received a commit order.
                         * In case they have not the transaction will be
                         * preserved as "assumedcomplete". If the subordinate
                         * sends a replay_completion, the transaction will be
                         * reactivated, and a commit sent (to the new resource
                         * reference) This only applies to transactions that are
                         * known to be committed - a server transaction in
                         * prepared state will retry indefinitely
                         *
                         * attempt count is only for this run of the recovery
                         * manager, so is kept by the cache, not the transaction
                         */
                        ...
                        if (cacheItem.getStatus() == Status.StatusCommitted) {
                            ...
                            if (previousAttempts >= attemptsBeforeConversion) {
                                ...
                                if (converting && (jtsLogger.logger.isDebugEnabled())) {
                                    jtsLogger.logger
                                            .debug(" Transaction " + actionUid + " assumed complete - changing type.");
                                }
                            }
                        }
                        // replayPhase2 will cause a re-persist unless it
                        // completes
                        // in which case it will cause a removal, so we mark it
                        // for
                        ...
                        fullyCompleted = theTransaction.allCompleted(); // only
                                                                        // remove
                                                                        // if
                                                                        // committed?
                ...
                /*
                 * Now remove the transaction from the cache, only removing the
                 * item if it is truly completed.
                 */
                ...
                if (fullyCompleted) {
                    ...
                    cacheItem.clearTransaction(); // just force a reactivate
                                                    // later
                }
            }
        }
