...
public class StrictInterposition extends Interposition {
    ...
    protected synchronized ControlImple createHierarchy(PropagationContext ctx, Uid currentUid) throws SystemException {
        ...
        if (!action.valid()) {
            try {
                ((ServerStrictTopLevelAction) action).rollback(); // does
                                                                    // dispose
                                                                    // as well!
                action = null;
            } catch (Exception e) {
        ...
        if (depth > 0) // current is a nested transaction
        {
            ...
            for (int i = depth - 2; i >= 0; i--) {
                ...
                nestedAction = new ServerStrictNestedAction(control, false); // not
                                                                                // current,
                                                                                // so
                                                                                // don't
                                                                                // register
                ...
                if (!nestedAction.valid()) {
                    ...
                    try {
                        ((ServerStrictNestedAction) nestedAction).rollback_subtransaction(); // does
                                                                                                // dispose
                                                                                                // as
                                                                                                // well!
                        nestedAction = null;
            ...
            nestedAction = new ServerStrictNestedAction(control, true); // current,
                                                                        // so
                                                                        // register
    ...
    protected synchronized ControlImple checkHierarchy(ServerTopLevelAction hier, PropagationContext context) {
        ...
        if (depth == 0) {
            ...
            for (int i = (int) depth - 2; i >= 0; i--) // don't check depth-1 as
                                                        // it is current action!
            {
            ...
            if (differenceIndex != -1) {
                ...
                for (int j = differenceIndex; j >= 0; j--) {
                    ...
                    if (!nestedAction.valid()) {
                        ...
                        try {
                            ((ServerStrictNestedAction) nestedAction).rollback(); // does
                                                                                    // dispose
                                                                                    // as
                                                                                    // well!
                            nestedAction = null;
