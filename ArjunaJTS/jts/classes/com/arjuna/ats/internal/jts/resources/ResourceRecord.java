 ...
 *
 * Resources registered using 'register_resource' ONLY take part in top-level
 * events
 *
 ...
 * Arjuna), then all nested participants will have to repond successfully to
 * prepare before they can be told to commit. The way the OTS mandates the
 * protocol, we could tell some to commit before being told by another resource
 * that it cannot commit! We then have to go through the resources and tell them
 * to abort! May cause heuristic decisions!
 *
...
public class ResourceRecord extends com.arjuna.ats.arjuna.coordinator.AbstractRecord {
    ...
    public ResourceRecord(boolean propagate, Resource theResource, Coordinator myParent, Uid recCoordUid,
    ...
    public final Resource resourceHandle() {
         ...
         * string_to_object. In such cases we cache the stringied reference and
         * retry the narrow when we need to use the _resourceHandle as at this
         * point the Resource may have recovered.
         */
        ...
        if ((_resourceHandle == null) && (_stringifiedResourceHandle != null)) {
            try {
                ...
                if (jtsLogger.logger.isTraceEnabled()) {
                    jtsLogger.logger
                            .trace("ResourceRecord: About to string_to_object on " + _stringifiedResourceHandle);
                }
     ...
     * The ResourceRecord creator is assumed to ensure that plain Resources are
     * only registered with the appropriate top level action.
     *
    ...
    public int nestedAbort() {
         ...
         *
         * The parent may have already been released if abort is being called
    ...
    /**
     * If this resource handles nesting this was done in prepare or it should be
     * ignored. In either case returning FINISH_OK suffices.
     */
    ...
    /**
     * OTS does not use nested prepare at all! However, to make nested
     * commit/abort clean we do commit_subtransaction here. Note that we only
     * get a prepare from Arjuna if the action is to be committed so this is
     * safe in Arjuna terms.
     */
    ...
    public int nestedPrepare() {
        ...
        try {
            ...
            /*
             * Not subtran aware resource, so return PREPARE_OK. Resource will
             * get invocations at top-level only.
             */
    ...
    /*
     * public static AbstractRecord create () { return new ResourceRecord(); }
     */
    ...
    /**
     * restore_state and save_state for ResourceRecords doesn't generally apply
     * due to object pointers. However, we need to save something so we can
     * recover failed transactions. So, rather than insist that all Resources
     * derive from a class which we can guarantee will give us some unique id,
     * we simply rely on string_to_object and object_to_string to be meaningful.
     */
    ...
    public boolean shouldReplace(AbstractRecord rec) {
        ...
        if (rec != null) {
            if (rec.typeIs() == typeIs()) {
                 ...
                 * RecoveryCoordinator as this one. If so replace. Don't replace
                 * if the uids are NIL_UID
                 */
