...
public class StatusChecker {
    /*
     * the interface of this class is intended to allow for possible future
     * extension to do statuschecking by other (non-corba) means
     *
    ...
    // lookup the relevant factory according to the uid of the
    // FactoryContactItem
    ...
    /**
     * get the status in the original process, given the uid of the contact item
     * (which is the uid of the process)
     */
    ...
    /**
     * get the current status in the original process, given the uid of the
     * contact item (which is the uid of the process). Note that this method is
     * used by the GenericRecoveryCoordinator code only.
     */
     ...
     * This method *must* only be called from replay_completion, since it relies
     * upon this fact to differentiate between a committed or rolled back
    ...
    public Status checkOriginalStatus(Uid transactionUid, Uid itemUid, boolean checkTheObjectStore) throws Inactive {
        if (jtsLogger.logger.isDebugEnabled()) {
            jtsLogger.logger.debug("StatusChecker.checkOriginalStatus(" + transactionUid + ", " + itemUid + ", "
                    + checkTheObjectStore + ")");
        }
    ...
    /**
     * try to get the status from a factory and convert to our way. factory must
     * not be null itemUid is the store id as in _itemFromUid
     */
    ...
    private Status getStatus(Uid transactionUid, FactoryContactItem item, boolean checkTheObjectStore) throws Inactive {
        ...
        if (factory != null) {
            ...
            try {
                ...
                if (jtsLogger.logger.isDebugEnabled()) {
                    jtsLogger.logger.debug("StatusChecker.getStatus(" + transactionUid + ") - current status = "
                            + Utility.stringStatus(otsStatus));
                }

                /*
                 * If the factory doesn't know about the transaction, then check
                 * the object store for the intentions list. If not present,
                 * then the transaction must have rolled back. If present, then
                 * we don't know what's going on, since the factory should still
                 * have a reference to the transaction!
                 */
                ...
                if (otsStatus == Status.StatusNoTransaction) {
                    ...
                    if (jtsLogger.logger.isDebugEnabled()) {
                        jtsLogger.logger.debug("StatusChecker.getStatus(" + transactionUid + ") - stored status = "
                                + Utility.stringStatus(otsStatus));
                    }
                    ...
                    switch (otsStatus.value()) {
                        case Status._StatusNoTransaction :
                            /*
                             * A definitive NoTransaction means rolled back
                             * because of presumed abort protocol.
                             */
                        ...
                        default : {
                             ...
                             * factory has just finished with the transaction,
                             * but the state hasn't been removed by the file
                             * system yet - we don't sync the removal to improve
                             * performance.
                             */
                ...
                if (jtsLogger.logger.isDebugEnabled()) {
                    jtsLogger.logger.debug("StatusChecker.getStatus(" + transactionUid + ") - Status = "
                            + Utility.stringStatus(otsStatus));
                }
                ...
                // orbix seems to count unreachable as transient. Over infinite
                // time, all
                // addresses are valid
            ...
            /*
             * In which case we can use the current, in process local factory,
             * to look at the object store and get the status from that. At
             * present all factories can look at the entire object store on a
             * machine, so this will work. If a factory is limited to only a
             * portion of the object store then we may need to create an
             * explicit factory that has "global" knowledge.
             */
    ...
    /**
     * find the IOR for the ArjunaFactory whose FactoryContactItem was saved
     * with this uid. It is possible this Uid was created after the last scan,
     * so if it isn't in the hashtable, look for real directly.
     */
