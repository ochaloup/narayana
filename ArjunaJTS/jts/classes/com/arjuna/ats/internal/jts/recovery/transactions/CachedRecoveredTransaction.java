...
/**
 * Any transaction that is identified by the recovery system to require recovery
 * is manipulated through CachedRecoveredTransactions.
 * CachedRecoveredTransactions delegate through to a static transaction cache
 * which ensures that there is no interference between different threads that
 * may be working with the same transaction
 * <P>
 * 
...
public class CachedRecoveredTransaction {
    public CachedRecoveredTransaction(Uid actionUid, String theType) {
        ...
        if (jtsLogger.logger.isDebugEnabled()) {
            jtsLogger.logger.debug(
                    "CachedRecoveredTransaction created [" + _theTransactionUid + ", " + _theTransactionType + "]");
        }
    }

    public Uid getTransactionUid() {
        if (jtsLogger.logger.isDebugEnabled()) {
            jtsLogger.logger.debug("CachedRecoveredTransaction.getTransactionUid() =" + _theTransactionUid);
        }
    ...
    public synchronized Status get_status() throws SystemException {
        ...
        if (jtsLogger.logger.isDebugEnabled()) {
            jtsLogger.logger.debug("CachedRecoveredTransaction.get_status [" + _theTransactionUid + ", "
                    + _theTransactionType + "] = " + Utility.stringStatus(theStatus));
        }
        return theStatus;
    }
    ...
    boolean /* sync ? */ originalBusy() {
        ...
        if (jtsLogger.logger.isDebugEnabled()) {
            jtsLogger.logger.debug(
                    "CachedRecoveredTransaction.originalBusy - told status is " + Utility.stringStatus(originalStatus));
        }
    ...
    /**
     * what is the status of the transaction in the original process ? (if
     * alive)
     */
    ...
    public void addResourceRecord(Uid rcUid, Resource r) {
        if (jtsLogger.logger.isDebugEnabled()) {
            jtsLogger.logger.debug("CachedRecoveredTransaction.addResourceRecord [" + _theTransactionUid + ", "
                    + _theTransactionType + "]" + "(" + rcUid + ")");
        }
    ...
    public void replayPhase2() {
        ...
        if (jtsLogger.logger.isDebugEnabled()) {
            jtsLogger.logger.debug("CachedRecoveredTransaction.replayPhase2 [" + _theTransactionUid + ", "
                    + _theTransactionType + "]");
        }
