...
public class OSIInterposition extends Interposition {
     ...
     * i.e., if we see the same otid but with a different formatID it's really
    ...
    protected synchronized ControlImple createHierarchy(PropagationContext ctx, Uid currentUid) throws SystemException {
        ...
        /*
         * We only ever register the current transaction with its parent, and as
         * each transaction commits, it registers its parent with the "real"
         * parent.
         */
        ...
        ServerControl control = ServerFactory.create_transaction(currentUid, null, null, tmpCoord, tmpTerm,
                ctx.timeout);
        ...
        if (!action.valid()) {
            try {
                ((ServerOSITopLevelAction) action).rollback(); // does dispose
                                                                // as well!
                action = null;
            } catch (Exception e) {
            }
        ...
        if (depth > 0) // current is a nested transaction
        {
            ...
            for (int i = depth - 2; i >= 0; i--) {
                ...
                control = ServerFactory.create_subtransaction(OTIDMap.find(ctx.parents[i].otid), tmpCoord, tmpTerm,
                        control);

                nestedAction = new ServerOSINestedAction(control, false); // not
                                                                            // current,
                                                                            // so
                                                                            // don't
                                                                            // register
                ...
                if (!nestedAction.valid()) {
                    ...
                    try {
                        ((ServerOSINestedAction) nestedAction).rollback_subtransaction(); // does
                                                                                            // dispose
                                                                                            // as
                                                                                            // well!
                        nestedAction = null;
            ...
            nestedAction = new ServerOSINestedAction(control, true); // current,
                                                                        // so
                                                                        // register
     ...
     * ones. However, in a multi-threaded environment, a thread can make a call
     * from any point in the client's hierarchy, and multiple client threads can
     * invoke the same server object. So, in general we cannot do this
     * optimisation. We must maintain the entire tree until portions of it have
     * explicitly been termined.
     *
     ...
     * transaction! In this implementation, the first to do so will garbage
     * collect the root of the hierarchy, and probably cause the subsequent ones
     * to fail! There is also the problem with many cross-process calls for the
     * *same* transaction. So, we register *one* resource for each level of the
     * hierarchy *when it is required*, i.e., as the previous transaction
     * terminates, and remove terminated transaction resources when they occur.
     * This means that at top-level we only have a single resource to commit.
     * There are the same number of cross-process invocations. However, we also
     * maintain the entire hierarchy at the server, so if it makes subsequent
     * invocations, the right hierarchy gets sent out!
     */
    ...
    protected synchronized ControlImple checkHierarchy(ServerTopLevelAction hier, PropagationContext context) {
         ...
         *
         * To get here we have already checked the id of the parent transaction,
         * i.e., depth -1.
         *
        ...
        if (depth == 0) {
             ...
             * and the n-1th element is the top-level transaction, which we must
             * deal with first!
             */

            for (int i = depth - 2; i >= 0; i--) // don't check depth-1 as it is
                                                    // current action!
            {
                nestedAction = currentAction.getChild(OTIDMap.find(context.parents[i].otid));

                if (nestedAction == null) // point of difference, so stop
                                            // trawling hierarchy
                {
                    ...
                    /*
                     * currentAction *always* points to the last known good
                     * transaction in our hierarchy.
                     */
            ...
            if (differenceIndex != -1) {
                ...
                for (int j = differenceIndex; j >= 0; j--) {
                    ...
                    control = ServerFactory.create_subtransaction(OTIDMap.find(context.parents[j].otid), tmpCoord,
                            tmpTerm, control);
                    ...
                    if (!nestedAction.valid()) {
                        ...
                        try {
                            ((ServerOSINestedAction) nestedAction).rollback(); // does
                                                                                // dispose
                                                                                // as
                                                                                // well!
                            nestedAction = null;
                ...
                /*
                 * Hierarchies may be identical. Remember to check!
                 */
            }
            ...
            /*
             * currentAction points to the parent of the 'current' transaction,
             * i.e., the last element in the TransIdentity structure. So, ask it
             * if the sent hierarchy's child is one of its children.
             */
            ...
            if (nestedAction == null) {
                /*
                 * Different notion of current in sent hierarchy. So, add it to
                 * the hierarchy here.
                 */
                ...
                /*
                 * Same current, so get its control and return it. Remember to
                 * make sure it has registered itself with the "real"
                 * transaction.
                 */
