 ...
 * method of TransactionCache. (handles the creation of different kinds of
 * transaction).
 *
 */
class TransactionCacheItem {
    ...
    private boolean loadTransaction(boolean firstLoad) {
        if (_type.equals(ArjunaTransactionImple.typeName())) {
            ...
            if (firstLoad && _transaction.getRecoveryStatus() == RecoveryStatus.ACTIVATE_FAILED) {
                // this is a previously unknown transaction, and its not in the
                // store
            ...
            try {
                _transaction = (RecoveringTransaction) Class
                        .forName("com.arjuna.ats.internal.jta.recovery.jts.jca.coordinator.RecoveredServerTransaction")
                        .getConstructor(new Class[]{Uid.class, String.class}).newInstance(new Object[]{_uid, _type});
            } catch (Exception e) {
                return false;
            }
    ...
    /**
     * forget the activated copy of the transaction. Makes it subject to garbage
     * collecting
     */
    ...
    RecoveringTransaction freshTransaction() {
        if (_transaction == null || _transaction.getRecoveryStatus() == RecoveryStatus.REPLAYED) {
            /*
             * Not sure why we do a reload at all here. But if we do it based on
             * whether the transaction has really completed, things stop
             * working. Needs further investigation since there may be a
