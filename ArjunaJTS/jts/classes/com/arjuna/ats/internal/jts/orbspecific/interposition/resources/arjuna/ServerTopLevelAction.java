 ...
 * BasicAction or Transaction. This is because of the way in which the OTS
 * creates and manipulates transactions.
 *
 * As with Transaction, we only create actions here, and do not associated these
 * contexts with any thread. We do the association later.
 *
 * If we were to do the creation via a suitably modified current interface then
 * the thread association would be done for us automatically, and we would not
 * have to call resume at all.
 *
 * This is a top-level action proxy.
 */

/*
 * NOTE: all calls to pushAction in this and related classes are to do with AIT
 * abstract records. Some of them rely on being able to call BasicAction.Current
 * to determine what the current transaction is and if we do not do
 * thread-to-transaction association here then this all breaks. There is no
 * other reason for this. So, what we need to do is remove this dependency by
 * fixing the AIT records. This dependency has a knock-on effect on certain
 * optimisations we can make (e.g., local interpositon - see ServerControl.)
 */
...
public class ServerTopLevelAction extends ServerResource implements org.omg.CosTransactions.ResourceOperations {
    ...
    public Resource getReference() {
        if ((_resourceRef == null) && _valid) {
            if (_theControl != null) {
                ...
                _resourceRef = org.omg.CosTransactions.ResourceHelper
    ...
    public org.omg.CosTransactions.Vote prepare() throws HeuristicMixed, HeuristicHazard, SystemException {
        ...
        if (jtsLogger.logger.isTraceEnabled()) {
            jtsLogger.logger
                    .trace("ServerTopLevelAction::prepare for " + _theUid + " : " + TwoPhaseOutcome.stringForm(result));
        }
    ...
    public void rollback() throws HeuristicCommit, HeuristicMixed, HeuristicHazard, SystemException {
        ...
        if (actionStatus == ActionStatus.PREPARED) {
            /*
             * This will also call any after_completions on registered
             * synchronizations.
             */
            ...
            if ((actionStatus == ActionStatus.RUNNING) || (actionStatus == ActionStatus.ABORT_ONLY)) {
                try {
                    /*
                     * Have to do this because of the way PI works with
        ...
        if (jtsLogger.logger.isTraceEnabled()) {
            jtsLogger.logger.trace(
                    "ServerTopLevelAction::rollback for " + _theUid + " : " + ActionStatus.stringForm(actionStatus));
        }
    ...
    public void commit() throws NotPrepared, HeuristicRollback, HeuristicMixed, HeuristicHazard, SystemException {
        ...
        if (jtsLogger.logger.isTraceEnabled()) {
            jtsLogger.logger.trace(
                    "ServerTopLevelAction::commit for " + _theUid + " : " + ActionStatus.stringForm(actionStatus));
        }
    ...
    public void commit_one_phase() throws HeuristicHazard, SystemException {
        ...
        if (theTransaction == null) {
            jtsLogger.i18NLogger
                    .warn_orbspecific_interposition_resources_arjuna_notx("ServerTopLevelAction.commit_one_phase");
        ...
        try {
            /*
             * This will commit and do any before/after_completion calls on
            ...
            /*
             * Is a heuristic, then don't remove the transaction information.
             */
        ...
        if (!_destroyed) {
            try {
                ...
                jtsLogger.i18NLogger.warn_orbspecific_interposition_resources_arjuna_generror(
                        "ServerTopLevelAction.destroyResource", e);
            }
    ...
    protected boolean registerResource(Coordinator theCoordinator) {
        ...
        if (theCoordinator != null) {
            try {
                 ...
                 * the interposed transaction to save and restore.
                 */
                ...
                jtsLogger.i18NLogger.warn_orbspecific_interposition_resources_arjuna_generror(
                        "ServerTopLevelAction.registerResource", ex2);
            } catch (Exception e) {
                jtsLogger.i18NLogger.warn_orbspecific_interposition_resources_arjuna_generror(
                        "ServerTopLevelAction.registerResource", e);
            }
        } else {
            jtsLogger.i18NLogger
                    .warn_orbspecific_interposition_resources_arjuna_nocoord("ServerTopLevelAction.registerResource");
        }
