...
public class Utility {
    ...
    /*
     * Any need for the inverse operation? Could easily do it for JBoss
     * transactions only.
     */

    /**
     * If this control refers to an JBoss transaction then return its native Uid
     * representation. Otherwise return Uid.nullUid().
     */
    ...
    public static final org.omg.CosTransactions.otid_t uidToOtid(String theUid) {
        ...
        /*
         * gtrid must be first then immediately followed by bqual. bqual must be
         * between 1 and 64 bytes if for XA.
         */
    ...
    /*
     * If we cannot deal with the otid then we could simply generate a new Uid.
     * We would need to keep the original otid in case the transaction either
     * retransmits it or the application asks for it (e.g., via the
     * PropagationContext).
     */
    ...
    public static final Uid otidToUid(org.omg.CosTransactions.otid_t otid) {
        if (otid.bqual_length > 0) {
            ...
            byte[] theUid = new byte[uidLength]; // don't need null terminating
                                                    // character

            System.arraycopy(otid.tid, 0, theUid, 0, uidLength);

            Uid u = new Uid(new String(theUid, StandardCharsets.UTF_8), true); // errors
                                                                                // in
                                                                                // string
                                                                                // give
                                                                                // NIL_UID
            ...
            /*
             * Currently we ignore bqual. THIS WILL BE AN ISSUE FOR
             * INTEROPERABILITY!!
             */
