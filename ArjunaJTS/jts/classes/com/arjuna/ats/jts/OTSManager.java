...
/**
 * This class is essentially here for convenience purposes, and until all ORBs
 * provide a means to set their initial references for Current and the
 * TransactionFactory.
 *
...
public class OTSManager {
     ...
     * take care of this, but in certain circumstances (e.g., a context is
     * propagated implicitly but we do not use interposition and we have to
     * manufacture a local control object) it is not possible for the OTS to
     * know when controls can be removed. This is a problem with the
     * specification and CORBA in general.
     */

    public static void destroyControl(ControlImple control)
        ...
        /*
         * Just in case control is a top-level transaction, and has been
         * registered with the reaper, we need to get it removed.
         *
         */
        ...
        if (coord != null) {
            try {
                if (coord.is_top_level_transaction()) {
                    /*
                     * Transaction is local, but was registered as a Control. If
                     * this is a performance hit then add explicit add/removes
                     * for local instances.
                     */
                    ...
                    // wrap the control so it gets compared against reaper list
                    // entries using the correct test
        ...
        /*
         * Watch out for conflicts with multiple threads deleting the same
         * control!
         */

        /*
         * If local, then delete it here, rather than calling the destroy
         * method.
         *
         * Possible problem if a local factory is being accessed remotely?
         */
    ...
    public static void destroyControl(Control control)
        ...
        if (lCont != null) {
            ...
            if (coord != null) {
                try {
                    if (coord.is_top_level_transaction()) {
                        // wrap the control so it gets compared against reaper
                        // list entries using the correct test
                        PseudoControlWrapper wrapper = new PseudoControlWrapper(control);
                        TransactionReaper.transactionReaper().remove(wrapper);
                    }
                } catch (Exception e) {
            ...
            if (action != null) {
                 ...
                 *
                 * We could throw a BadControl exception, but what would that do
                 * for the programmer?
                 */
