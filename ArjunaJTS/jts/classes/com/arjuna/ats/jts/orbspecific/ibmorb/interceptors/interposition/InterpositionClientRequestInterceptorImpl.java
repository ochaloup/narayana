    ...
    public void send_request(ClientRequestInfo request_info) throws SystemException {
        ...
        try {
            if (!otsAlwaysPropagate) {
                ...
                /**
                 * If we are set to always propagate then ensure we're not
                 * already in use
                 **/
            ...
            try {
                /*
                 * We get back an Any, which contains a key which we must now
                 * use to get the actual transaction context. This saves use
                 * having to pack and unpack the context every time it changes,
                 * even if we don't then make a remote invocation.
                 */
                ...
                /*
                 * If we are using co-location optimisations, then filters may
                 * not have been used to set up the thread-to-context
                 * association. So, if the PI slot is null, check whether the
                 * current thread has a context on it already. If so, use it.
                 */

                if (localData.type().kind().value() != TCKind._tk_null) {
                    if ((threadId = localData.extract_string()) == null)
                        throw new UNKNOWN(
                                jtsLogger.i18NLogger.get_orbspecific_javaidl_interceptors_interposition_invalidparam());
                } else
                ...
                if (threadId != null) {
                    ...
                    if (theControl != null) {
                        try {
                            ...
                            /*
                             * We may have to make calls during commit (e.g.,
                             * after_completion) which are valid, but which will
                             * get Unavailable.
                             */
                ...
                /**
                 * If we are set to always propagate then ensure we clear the
                 * inuse flag
                 **/
                if (otsAlwaysPropagate) {
                    _inUse.set(null);
