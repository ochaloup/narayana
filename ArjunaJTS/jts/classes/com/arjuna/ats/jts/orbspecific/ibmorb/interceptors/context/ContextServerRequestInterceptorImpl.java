...
/*
 * Only add and remove transactional context if the destination object is
 * inherited from TransactionalObject.
 *
 * When we have finished implementing fully checked transactions we will use the
 * filter to prevent remote access to the factory and the terminator.
 */

/*
 * If we're not within a transaction when we call a TransactionalObject the
 * spec. implies that we should raise a TransactionExpected exception. This is
 * too restrictive, and removed the ability to build modular applications.
 * Therefore, by default we will *not* do this, but this can be overridden with
 * an appropriate environment variable. (Could do this through a separate base
 * class.)
 */
...
class ContextServerRequestInterceptorImpl extends LocalObject implements ServerRequestInterceptor {
     ...
     *
     * (i) we are a TransactionalObject. (ii) we define otsAlwaysPropagate to
     * TRUE.
     *
    ...
    public void receive_request_service_contexts(ServerRequestInfo request_info) throws SystemException {
        if (jtsLogger.logger.isTraceEnabled()) {
            jtsLogger.logger.trace("ContextServerRequestInterceptorImpl::receive_request_service_contexts ( "
            ...
            try {
                ...
                if (serviceContext != null) {
                    Any receivedData = _codec.decode_value(serviceContext.context_data,
                            ORBManager.getORB().orb().get_primitive_tc(TCKind.tk_string));
                     ...
                     * not have a transaction context already) and then do a
                     * resume.
                     */
    ...
    public void receive_request(ServerRequestInfo request_info) throws SystemException {
        if (jtsLogger.logger.isTraceEnabled()) {
            jtsLogger.logger
                    .trace("ContextServerRequestInterceptorImpl.receive_request ( " + request_info.operation() + " )");
        }
    }

    /**
     * Finished with request, so disassociate this thread from the transaction.
     */

    public void send_reply(ServerRequestInfo request_info) throws SystemException {
        if (jtsLogger.logger.isTraceEnabled()) {
            jtsLogger.logger
                    .trace("ContextServerRequestInterceptorImpl::send_reply ( " + request_info.operation() + " )");
        }

        try {
            suspendContext(request_info);
        } catch (SystemException ex) {
            jtsLogger.i18NLogger.warn_orbspecific_javaidl_interceptors_context_srie(
                    "ContextServerRequestInterceptorImpl::send_reply", ex);

            throw ex;
        } catch (Exception e) {
            jtsLogger.i18NLogger.warn_orbspecific_javaidl_interceptors_context_srie(
                    "ContextServerRequestInterceptorImpl::send_reply", e);
    ...
    public void send_exception(ServerRequestInfo request_info) throws SystemException {
        if (jtsLogger.logger.isTraceEnabled()) {
            jtsLogger.logger
                    .trace("ContextServerRequestInterceptorImpl::send_exception ( " + request_info.operation() + " )");
        }
        ...
        try {
            ...
            jtsLogger.i18NLogger.warn_orbspecific_javaidl_interceptors_context_srie(
                    "ContextServerRequestInterceptorImpl::send_exception", e1);

            throw e1;
        } catch (Exception e2) {
            jtsLogger.i18NLogger.warn_orbspecific_javaidl_interceptors_context_srie(
                    "ContextServerRequestInterceptorImpl::send_exception", e2);
    ...
    public void send_other(ServerRequestInfo request_info) throws SystemException {
        if (jtsLogger.logger.isTraceEnabled()) {
            jtsLogger.logger
                    .trace("ContextServerRequestInterceptorImpl.send_other ( " + request_info.operation() + " )");
        }
        ...
        try {
            ...
            jtsLogger.i18NLogger.warn_orbspecific_javaidl_interceptors_context_srie(
                    "ContextServerRequestInterceptorImpl::send_other", ex);

            throw ex;
        } catch (Exception e) {
            jtsLogger.i18NLogger.warn_orbspecific_javaidl_interceptors_context_srie(
                    "ContextServerRequestInterceptorImpl::send_other", e);
    ...
    /*
     * If there is a thread id associated with PICurrent then it will have been
     * placed there by a server-side thread which executed in the application
     * object and needed to associate an imported transaction with itself. In
     * which case we need to do the equivalent of a suspend to remove the thread
     * from Current and from the current transaction.
     */

    private void suspendContext(ServerRequestInfo request_info) throws SystemException, InvalidSlot {
        if (jtsLogger.logger.isTraceEnabled()) {
            jtsLogger.logger
                    .trace("ContextServerRequestInterceptorImpl.suspendContext ( " + request_info.operation() + " )");
        }
                ...
                if ((threadId = data.extract_string()) != null) {
                    ...
                    if (ctx != null) {
                        try {
                            ...
                            jtsLogger.i18NLogger.warn_orbspecific_javaidl_interceptors_context_srie(
                                    "ContextServerRequestInterceptorImpl.suspendContext", e);
