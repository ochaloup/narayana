 ...
 *
 * This task uses a definitions file (idl-compiler-definitions.xml) which
 * defines how to call each of the supported IDL compilers. For each compiler
 * you specify a number of parameters and whether they are required for this IDL
 * compiler. You then call this ant task and pass it a comma separated list of
 * parameters which it then maps to the IDL compiler specific parameters.
 *
...
public class IDLCompiler extends Task {
    ...
    protected void parseIDLCompilerDefinitionsFile() throws BuildException {
        ...
        try {
            ...
            /**
             * If the configuration file isn't in the classpath the input stream
             * will be null
             */
            if (configIn == null) {
                            ...
                            for (int inCount = 0; inCount < inParameters.getLength(); inCount++) {
                                ...
                                if (inParamNode.getNodeName().equals("param")) {
                                    ...
                                    boolean required = Boolean
                                            .valueOf(paramAttributes.getNamedItem("required").getNodeValue())
                                            .booleanValue();
                            ...
                            for (int inCount = 0; inCount < outParameters.getLength(); inCount++) {
                                ...
                                if (outParamNode.getNodeName().equals("param")) {
                                    ...
                                    if (childrenContainsSubParam(outParamNode)) {
                                        ...
                                        for (int subParamCount = 0; subParamCount < children
                                                .getLength(); subParamCount++) {
            ...
            throw new BuildException(
                    "An unexpected exception occurred while processing the idl compiler definitions file (" + e + ")");
        }
    }
    ...
    public void execute() throws BuildException {
        ...
        if (idlCompiler == null) {
            throw new BuildException(
                    "The IDL compiler for '" + _orb + "' is not defined within the configuration file");
        }
    ...
    public class IDLCompilerDetails {
        ...
        public void addInParameter(String param, boolean required, String delimiter, boolean isClasspath,
                String replace) {
        ...
        protected InParameterDetails setAndFlagParameter(String param, String value, Hashtable map) {
            if (value != null) {
                ...
                if (inParamDef == null) {
                    throw new BuildException("The parameter '" + param
                            + "' used in the parameter attribute is not defined for this ORB");
                }
         ...
         */
        public String[] parse(String filename, String destDir, String packageName, String parameters, String mappings)
            ...
            if ((filename != null) && (filename.length() > 0)) {
                ...
                /**
                 * If the filename wasn't valid or didn't contain data then
            ...
            if (parameters != null) {
                ...
                while (parameters.indexOf(',') != -1) {
                    ...
                    if (inParamDef == null) {
                        throw new BuildException("The parameter '" + parameter
                                + "' used in the parameter attribute is not defined for this ORB");
                    }
                ...
                if ((inParamDef.hasDelimiter()) && (!inParamDef.isClasspath())) {
                    ...
                    if (inParamDef.getReplaceString() != null) {
                        ...
                        while (value.indexOf(find) != -1) {
                            value = value.substring(0, value.indexOf(find)) + replaceWith
                                    + value.substring(value.indexOf(find) + find.length());
                        }
                    }
            ...
            for (Enumeration e = _inParameters.elements(); e.hasMoreElements();) {
                ...
                if (ipd.isRequired() && !usedParameters.contains(ipd)) {
                    throw new BuildException("The parameter '" + ipd.getParameter()
                            + "' is defined as being required but has not been specified");
                }
            }
            ...
            for (int count = 0; count < _outParameters.size(); count++) {
                if (_outParameters.get(count) instanceof ArrayList) {
                    ...
                    try {
                        ...
                        // If one of the parameters in the multi-param block is
                        // not defined
        ...
        private void generateOutputParameters(Object forEachKey, HashSet usedParameters, ArrayList outputParameters)
                throws Exception {
            ...
            if (forEach == null) {
                if (forEachKey instanceof ArrayList) {
                    ...
                    for (int count = 0; count < al.size(); count++) {
                        outputParameters
                                .add(replaceVariables((String) al.get(count), _parameterValueMap, usedParameters));
                    }
                } else {
                ...
                if (obj instanceof ArrayList) {
                    ...
                    if (paramList != null) {
                        ...
                        for (int paramCount = 0; paramCount < paramList.size(); paramCount++) {
                            ...
                            if (!(forEachKey instanceof ArrayList)) {
                                outputParameters
                                        .add(replaceVariables((String) forEachKey, newParamValueMap, usedParameters));
                            } else {
                                ArrayList forEachArrayList = (ArrayList) forEachKey;

                                for (int count = 0; count < forEachArrayList.size(); count++) {
                                    outputParameters.add(replaceVariables((String) forEachArrayList.get(count),
                                            newParamValueMap, usedParameters));
                                }
                            }
                        }
                    }
        ...
        public String replaceVariables(String data, Hashtable variableValueMap, HashSet usedParameters)
                throws Exception {
        ...
        private class InParameterDetails {
            ...
            public InParameterDetails(String param, boolean required, String delimiter, boolean isClasspath,
    ...
    public class InputStreamFileWriter extends Thread {
        ...
        public InputStreamFileWriter(InputStream inStr, String filename)
