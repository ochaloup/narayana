...
/**
 * The current implementation will invoke all registered handlers whenever an
 * object is connected and disconnected. These handlers can then determine
 * whether they want to do anything about it by checking the type of the object
 * (using narrow).
 *
...
/*
 * If we were to allow a registration of a handler on a per type basis then we
 * would have to do narrow ourselves (possibly for every type in a classes
 * hierarchy). A user would also then have to register one instance for each
 * type the object may have, or we would have a more complicated registration
 * process, possibly requiring a "hierarchy" string to be passed in.
 *
 * Until we know how (and if) this class is used, it's better to start off
 * simple and work our way up to extremely complicated!
 */
...
public class EventManager {
    ...
    /**
     * Add the specified handler. If the handler has already been registered
    ...
    /**
     * Remove the specified handler. If the handler has not been registered then
