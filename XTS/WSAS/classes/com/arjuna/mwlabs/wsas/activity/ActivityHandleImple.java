...
/**
 * ActivityHandle is used as a representation of a single activity when it is
 * suspended from a running thread and may be later resumed. The implementation
 * of the token can be as lightweight as required by the underlying
 * implementation in order that it can uniquely represent all activity
 * instances.
 *
 * Since this is a client-facing class, it is unlikely that the application user
 * will typically want to see the entire activity context in order to simply
 * suspend it from the thread.
 */
...
public class ActivityHandleImple implements ActivityHandle {
    ...
    /**
     * Although users won't typically care what the underlying implementation of
     * a context is, they will need to do comparisons. So, although this method
     * is provided by Java.Object we have it here to ensure that we don't forget
     * to implement it!
     *
