...
/**
 * An HLS is registered with activities to be informed of their lifecycle and to
 * augment the basic notion of what an activity is.
 *
 * Currently each HLS is registered globally so that all activities on all
 * threads know about them. However, we may want to have finer granularity such
 * that an HLS may only be registered with a specific thread.
 *
...
public interface HLS {
     ...
     *                Thrown if an error occurs. Any error will cause the
     *                activity to be failed.
     */
    ...
    /*
     * How do we deal with the case where an HLS has seen the activity to be
     * completing successfully then another sets it to fail? Unless we introduce
     * a multi-phase completion protocol this will always be a problem.
     */
    ...
    /**
     * The activity has been resumed on the current thread and is active on that
     * thread.
     */
