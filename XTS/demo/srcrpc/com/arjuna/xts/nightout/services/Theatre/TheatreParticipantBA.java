...
/**
 * An adapter class that exposes the TheatreManager transaction lifecycle API as
 * a WS-T Business Activity participant. Also logs events to a TheatreView
 * object.
 *
...
public class TheatreParticipantBA implements BusinessAgreementWithParticipantCompletionParticipant, Serializable {
     ...
     * @param which_area
     *            of the theatre the seats are in.
     */
    ...
    public void compensate() throws FaultedException, WrongStateException, SystemException {
        ...
        try {
            if (!getTheatreManager().compensateSeats(txID)) {
                ...
                // expecting a close
                System.out.println(
                        "TheatreParticipantBA.compensate : not expecting a compensate for BA participant " + txID);

                throw new WrongStateException("Unexpected compensate for BA participant " + txID);
            }
        } catch (FaultedException fe) {
            getTheatreView().addMessage(
