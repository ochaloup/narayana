 ...
 *
 * </p>
 * The manager maintains the following invariants regarding seating capacity:
 * <ul>
 * <li>nBooked[area] == sum(unpreparedList.seatCount[area]) +
 * sum(preparedList.seatCount[area])
 *
 ...
 * </ul>
 * changes to nPrepared, nFree, nCommitted, nCompensatable, nTotal, preparedList
 * and compensatableList are always shadowed in persistent storage before
...
public class TheatreManager implements Serializable {
     ...
     * @return true on success, false otherwise
     */
    public boolean compensateSeats(Object txID) throws FaultedException {
        boolean success = false;
     ...
     *            The transaction identifier
     * @param compensatable
     *            true if it may be necessary to compensate this commit laer
     * @return true on success, false otherwise
     */
    ...
    public synchronized boolean closeSeats(Object txID) {
        ...
        if (compensatableTransactions.containsKey(txID)) {
            ...
            success = false; // error: transaction not registered for
                                // compensation
        }
     ...
     *
     * @param txID
     *            The uniq id for the transaction
     * @return true if the business logic is holding state related to the given
     *         txID, false otherwise.
     */
     ...
     *
     * @param area
     *            The seating area to change
     * @param nSeats
     *            The new capacity for the area
     */
     ...
     *
     * @param autoCommit
     *            true for automatic commit, false for manual commit.
     */
    ...
    /**
     * (re-)initialise the instance data structures deleting any previously
    ...
    /**
     * (re-)initialise the instance data structures, potentially committing any
     * saved state to disk
     * 
     * @param deleteSavedState
     *            true if any cached transaction state should be deleted
     *            otherwise false
     */
     ...
     *
     * n.b. can only be called once from the singleton constructor before save
     * can be called so there is no need for any synchronization here
     */
    ...
    /**
     * write the current manager state to a shadow disk file then commit it as
     * the latest state by relinking it to the current file
     *
     * n.b. must always called synchronized since the caller must always
     * atomically check the current state, modify it and write it.
     */
