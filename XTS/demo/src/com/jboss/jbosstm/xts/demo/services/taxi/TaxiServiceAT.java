 ...
 */
@WebService(serviceName = "TaxiServiceATService", portName = "TaxiServiceAT", name = "ITaxiServiceAT", targetNamespace = "http://www.jboss.com/jbosstm/xts/demo/Taxi", wsdlLocation = "/WEB-INF/wsdl/TaxiServiceAT.wsdl")
@HandlerChain(file = "/context-handlers.xml", name = "Context Handlers")
@SOAPBinding(style = SOAPBinding.Style.RPC)
public class TaxiServiceAT implements ITaxiServiceAT {
    /**
     * Book a taxi Enrols a Participant if necessary, then passes the call
     * through to the business logic.
     */
    @WebMethod
    public void bookTaxi() {
        ...
        try {
            ...
            if (!taxiManager.knowsAbout(transactionId)) {
                ...
                TransactionManagerFactory.transactionManager().enlistForDurableTwoPhase(taxiParticipant,
                        "org.jboss.jbossts.xts-demo:taxiAT:" + new Uid().toString());
            }
        } catch (Exception e) {
