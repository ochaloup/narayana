 ...
 * modifying bookings <em>in memory only</em>. Taxis are an unlimited resource
 * and it does not really matter if a taxi does not turn up (there's always
 * another one round the corner) nor does it matter of the clients don't appear
 * (someone else will take the ride). so this manager does not maintain any
 * persistent state and the bookings it makes are not resilient to crashes.
 *
...
public class TaxiManager implements Serializable {
    ...
    /**
     * check whether we have already seen a web service request in a given
     * transaction
     * 
     * @param txID
     *            The transaction identifier
