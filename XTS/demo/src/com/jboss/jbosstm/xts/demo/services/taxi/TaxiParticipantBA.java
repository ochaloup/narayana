...
/**
 * An adapter class that exposes the TaxiManager transaction lifecycle API as a
 * WS-T Coordinator CompletionBusiness Activity participant. Also logs events to
 * a TaxiView object.
 *
 * The Taxi Service does not actually manage any persistent local state since it
 * does not really matter if a taxi or the clients fail to turn up (there will
 * always be another taxi or another client round the corner). It uses a
 * participant which employs the coordinator completion protocol but only
...
public class TaxiParticipantBA
    ...
    /**
     * The activity has ended successfully. The participant previously completed
     * its local changes.
     *
     ...
     *             never in this implementation.
     * @throws SystemException
     *             always, because this implementation does not support
     *             compensation.
     */
    ...
    public void compensate() throws FaultedException, WrongStateException, SystemException {
        ...
        getTaxiView().addPrepareMessage(
                "id:" + txID + ". Attempting to compensate participant: " + this.getClass().toString());

        getTaxiView().updateFields();

        // there is no need to compensate a completed taxi booking as the taxi
        // will just pick up another punter
    ...
    public void unknown() throws SystemException {
        // used for callbacks during crash recovery. This impl is not
        // recoverable
    }
     ...
     * 
     * @param txID
     *            the participant's transaction id
     * @param participant
     */
    public static synchronized void recordParticipant(String txID, TaxiParticipantBA participant,
