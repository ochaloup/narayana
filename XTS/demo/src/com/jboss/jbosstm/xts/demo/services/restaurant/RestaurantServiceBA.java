 ...
 */
@WebService(serviceName = "RestaurantServiceBAService", portName = "RestaurantServiceBA", name = "IRestaurantServiceBA", targetNamespace = "http://www.jboss.com/jbosstm/xts/demo/Restaurant", wsdlLocation = "/WEB-INF/wsdl/RestaurantServiceBA.wsdl")
...
public class RestaurantServiceBA implements IRestaurantServiceBA {
    /**
     * Book a number of seats in the restaurant Enrols a Participant if
     * necessary and passes the call through to the business logic.
     *
     * @param how_many
    ...
    public boolean bookSeats(@WebParam(name = "how_many", partName = "how_many") int how_many) {
        ...
        restaurantView.addMessage(
                "id:" + transactionId + ". Received a booking request for one table of " + how_many + " people");
        ...
        try {
            ...
            participantManager = activityManager.enlistForBusinessAgreementWithParticipantCompletion(
                    restaurantParticipant, "org.jboss.jbossts.xts-demo:restaurantBA:" + new Uid().toString());
        ...
        // commit local changes. so we prepare and commit those changes now. if
        // any other participant fails
        ...
        if (restaurantManager.prepare(transactionId)) {
            ...
            try {
                ...
                // this will call back to the participant once a compensation
