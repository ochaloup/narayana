 ...
 * <p/>
 * Stores and manages seating reservations. The restaurant manager provides a
 * book method allowing the web service endpoint to book or unbook seats. It
 * also provides getters which allow the GUI to monitor the state of the service
...
public class RestaurantManager extends ServiceStateManager<RestaurantState> {
    ...
    public synchronized void bookSeats(Object txID, int nSeats) {
        ...
        if (currentState.freeSeats < nSeats || currentState.bookedSeats + nSeats > currentState.totalSeats) {
            throw new WebServiceException("requested number of seats (" + nSeats + ") not available");
        }

        // create a state derived from the current state which reflects the new
        // booking count
    ...
    public boolean confirmPrepare() {
        if (autoCommitMode) {
            ...
            // need to wait for the user to decide whether to go ahead or not
    ...
    /**
     * Create and initialise a new RestaurantManager instance. If the super
     * constructor does not restore a previously persisted current state then
     * create and persist an initial state using appropriate default values.
     */
