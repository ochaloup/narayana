...
/**
 * An adapter class that exposes the TheatreManager as a WS-T Coordinator
 * Completion Business Activity participant. Also logs events to a TheatreView
 * object.
 *
 * The Theatre Service allows up to three bookings in any given transaction, one
 * for each seating area. This means that teh service cannot decide when to
 * complete. After the first or, possibly, second booking request the service
 * cannot determine whether the client still wants to send another request.
 * Hence it uses a participant which implements the coordinator completion
 * protocol. WHen the client asks the coordinator to close the transaction the
 * coordinator will tell the participant to complete.
 *
 * @author Jonathan Halliday (jonathan.halliday@arjuna.com)
 * @version $Revision: 1.3 $
 */
public class TheatreParticipantBA
        implements
     ...
     *
     * @param txID
     *            uniq id String for the transaction instance.
     * @param bookings
     *            holds counts of the seats booked in each of the three seating
     *            areas
     */
    ...
    public void complete() throws WrongStateException, SystemException {
        ...
        if (!getTheatreManager().prepare(txID)) {
            // tell the participant manager we cannot complete. this will force
            ...
            // we just need to return here. the XTS implementation will call
            // confirmComplete
            // identifying whether or not to roll forward or roll back these
            // prepared changes
     ...
     * required, so it is now requested to do so.
     */
    ...
    public void compensate() throws FaultedException, WrongStateException, SystemException {
        ...
        getTheatreView().addPrepareMessage(
                "id:" + txID + ". Attempting to compensate participant: " + this.getClass().toString());

        getTheatreView().updateFields();

        // we perform the compensation by preparing and then committing local
        // changes which
        ...
        if (!getTheatreManager().prepare(compensationTxID)) {
            getTheatreView()
                    .addMessage("id:" + txID + ". Failed to compensate participant: " + this.getClass().toString());
     ...
     * registered with and an unrecoverable error occurs then this operation
     * will be invoked.
     */
     ...
     * 
     * @param txID
     *            the participant's transaction identifier
     * @param participant
     *            the participant to be recorded
     * @param manager
     *            the participant manager to be recorded
     */
    public static synchronized void recordParticipant(String txID, TheatreParticipantBA participant,
    ...
    /************************************************************************/
    /**
     * Id for the transaction which this participant instance relates to. Set by
     * the service (via contrtuctor) at enrolment time, this value is used in
     * informational log messages.
     */
    protected String txID;

    /**
     * array containing bookings for each of the seating areas. each area is
     * booked in its own service request but we need this info in order to be
     * able to detect repeated bookings.
     */
