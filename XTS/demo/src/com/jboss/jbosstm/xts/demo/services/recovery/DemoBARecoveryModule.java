...
/**
 * Application-specific WS-AT participant recovery manager for demo application,
 * This class is responsible for recreating application-specific durable
 * participants from records logged at prepare time.
 */
public class DemoBARecoveryModule implements XTSBARecoveryModule {
    ...
    /**
     * called during deployment of an xts-demo web service to ensure the
     * recovery module for the demo is installed whenever any of the services is
     * active
     */
    ...
    /**
     * called during undeployment of an xts-demo web service to ensure the
     * recovery module for the demo is deinstalled once none of the services is
     * active
     */
     ...
     * n.b. this is only appropriate in case the original was a
     * ParticipantCompletion participant saved using serialization.
     *
     * @param id
     ...
     * @return the deserialized ParticipantCompletion participant if the id
     *         identifies a restaurant participant otherwise null
     * @throws Exception
     *             if an error occurs deserializing the ParticipantCompletion
     *             participant
     */
    public BusinessAgreementWithParticipantCompletionParticipant deserializeParticipantCompletionParticipant(String id,
        ...
        if (id.startsWith("org.jboss.jbossts.xts-demo:restaurantBA")) {
            ...
            // ensure that the participant has completed any changes to local
            // service state.
            ...
            // this should not get called -- xts-demo WS-BA theatre and taxi
            // participants employ the CoordinatorCompletion protocol
            throw new Exception(
                    "xts-demo : invalid request to deserialize as WS-BA ParticipantCompletion participant " + id);
        }
        return null;
    }
    ...
    /**
     * called during recovery processing to allow an application to identify a
     * participant id belonging to one of its participants and use the saved
     * recovery state to recreate the participant. n.b. this is only appropriate
     * in case the original was a ParticipantCompletion participant saved after
     * being converted to a byte array using the PersistibleBAParticipant
     * interface.
     *
     * @param id
     *            the id used when the participant was created
     ...
     * @throws Exception
     *             if the id belongs to this application
     */
    public BusinessAgreementWithParticipantCompletionParticipant recreateParticipantCompletionParticipant(String id,
        ...
        if (id.startsWith("org.jboss.jbossts.xts-demo:restauarantBA")
                ...
                || id.startsWith("org.jboss.jbossts.xts-demo:taxiBA")) {
            // this should not get called -- xts-demo WS-BA participants are
            // saved and restored
            // using serialization
     ...
     * participant by deserializing it from the supplied object input stream.
     * n.b. this is only appropriate in case the original was a
     * CoordinatorCompletion participant saved using serialization.
     *
     * @param id
     *            the id used when the participant was created
     * @param stream
     ...
     *            module's application
     * @return the deserialized ParticipantCompletion participant if the id
     *         identifies a theatre or taxi participant otherwise null
     * @throws Exception
     *             if an error occurs deserializing the CoordinatorCompletion
     *             participant
     */
    public BusinessAgreementWithCoordinatorCompletionParticipant deserializeCoordinatorCompletionParticipant(String id,
            ObjectInputStream stream) throws Exception {
        if (id.startsWith("org.jboss.jbossts.xts-demo:restauarantBA")) {
            // this should not get called -- xts-demo WS-BA restaurant
            // participants employ the ParticipantCompletion protocol
            throw new Exception(
                    "xts-demo : invalid request to deserialize as WS-BA CoordinatorCompletion participant " + id);
        } else if (id.startsWith("org.jboss.jbossts.xts-demo:theatreBA")) {
     ...
     * participant id belonging to one of its participants and use the saved
     * recovery state to recreate the participant. n.b. this is only appropriate
     * in case the original was a CoordinatorCompletion participant saved after
     * being converted to a byte array using the PersistibleBAParticipant
     * interface.
     *
     * @param id
    ...
    public void endScan() {
        if (isFirst) {
            // let the restaurant and theatre state manager know that the BA log
            // records have all been scanned
