...
/**
 * An adapter class that exposes the TaxiManager business API as a transactional
 * Web Service. Also logs events to a TaxiView object.
 *
 * @author Jonathan Halliday (jonathan.halliday@arjuna.com)
 * @version $Revision: 1.5 $
 */
@WebService(serviceName = "TaxiServiceBAService", portName = "TaxiServiceBA", name = "ITaxiServiceBA", targetNamespace = "http://www.jboss.com/jbosstm/xts/demo/Taxi", wsdlLocation = "/WEB-INF/wsdl/TaxiServiceBA.wsdl")
@HandlerChain(file = "/context-handlers.xml", name = "Context Handlers")
@SOAPBinding(style = SOAPBinding.Style.RPC)
public class TaxiServiceBA implements ITaxiServiceBA {
    /**
     * Book a taxi Enrols a Participant if necessary and passes the call through
    ...
    public boolean bookTaxi() {
        ...
        try {
            participantManager = activityManager.enlistForBusinessAgreementWithCoordinatorCompletion(taxiParticipant,
                    "org.jboss.jbossts.xts-demo:taxiBA:" + new Uid().toString());
        } catch (Exception e) {
