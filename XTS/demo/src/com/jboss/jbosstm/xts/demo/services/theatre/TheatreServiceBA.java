 ...
 */
@WebService(serviceName = "TheatreServiceBAService", portName = "TheatreServiceBA", name = "ITheatreServiceBA", targetNamespace = "http://www.jboss.com/jbosstm/xts/demo/Theatre", wsdlLocation = "/WEB-INF/wsdl/TheatreServiceBA.wsdl")
...
public class TheatreServiceBA implements ITheatreServiceBA {
    /**
     * Book a number of seats in the Theatre Enrols a Participant if necessary,
     * then passes the call through to the business logic.
     *
     * @param how_many
     *            The number of seats to book.
     * @param which_area
     *            The area of the theatre to book seats in.
     * @return true on success, false otherwise.
     */
    @WebMethod
    @WebResult(name = "bookSeatsBAResponse", partName = "bookSeatsBAResponse")
    public boolean bookSeats(@WebParam(name = "how_many", partName = "how_many") int how_many,
            @WebParam(name = "which_area", partName = "which_area") int which_area)

    {
        ...
        theatreView.addMessage("id:" + transactionId + ". Received a theatre booking request for " + how_many
                + " seats in area " + which_area);
        ...
        if (theatreParticipant == null) {
            ...
            try {
                participantManager = activityManager.enlistForBusinessAgreementWithCoordinatorCompletion(
                        theatreParticipant, "org.jboss.jbossts.xts-demo:theatreBA:" + new Uid().toString());
            ...
            // hmm, this means we have already completed changes in this
            // transaction and are awaiting a close
            // or compensate request. this service does not support repeated
            // requests in the same activity so
            // we ensure the activity cannot continue by calling cannotComplete
            // and also roll back
            // any local changes
        ...
        // this service employs the coordinator completion protocol which means
        // we don't actually prepare and
        // commit these changes until the coordinator sends a complete request
        // through.
