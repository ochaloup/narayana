...
public class RecoverySubordinateATCoordinator extends SubordinateATCoordinator {
    ...
    /**
     * run parent activate and also make this coordinator visible if there might
     * be a durable participant waiting for it to commit.
     * 
     * @return
     */
    public boolean activate() {
        ...
        // if we cannot activate we want the participant which was registered on
        // behalf of this
        // coordinator to produce a heuristic result for the transaction. it
        // will do this if it
        // finds no entry for the coordinate in the subordinate coordinators
        // list. in this case
        // the subordinate transaction record needs to left as is awaiting
        // manual intervention.
        ...
        if (result) {
            ...
            if (status == ActionStatus.PREPARED || status == ActionStatus.COMMITTING) {
                // we need to install this coordinator in a global table so that
                // the participant which
                // was driving it will know that it has been recovered but not
                // yet committed
    ...
    public void replayPhase2() {
        ...
        if (RecoveryLogger.logger.isDebugEnabled()) {
            RecoveryLogger.logger.debugv(
                    "RecoverySubordinateATCoordinator.replayPhase2 recovering {0} ActionStatus is {1}",
                    new Object[]{get_uid(), ActionStatus.stringForm(status)});
        }

        if (_activated) {
            // we don't run phase 2 again if status is PREPARED or COMMITTING
            // because we need to wait
            // for the parent coordinator to tell us what to do. this is true if
            // the coordinator has
            // been recreated by the first recovery operation after a crash or
            // by a reload from the
            // log after it was saved in response to a comms tiemout from one of
            // the participants.
            ...
            if (RecoveryLogger.logger.isDebugEnabled()) {
                RecoveryLogger.logger.debugv("RecoverySubordinateATCoordinator.replayPhase2( {0} )  finished",
                        new Object[]{get_uid()});
            }
        } else {
