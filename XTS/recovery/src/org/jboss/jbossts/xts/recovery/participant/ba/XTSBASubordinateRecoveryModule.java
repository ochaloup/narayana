...
public class XTSBASubordinateRecoveryModule implements XTSBARecoveryModule {
    public BusinessAgreementWithParticipantCompletionParticipant deserializeParticipantCompletionParticipant(String id,
        ...
        if (id.startsWith(SubordinateBACoordinator.PARTICIPANT_PREFIX)) {
            ...
            // use serialization
            throw new Exception(
                    "XTSBASubordinateRecoveryModule : invalid request to deserialize() subordinate WS-BA coordinator participant completion participant "
                            + id);
        }
        return null;
    }
    ...
    public BusinessAgreementWithParticipantCompletionParticipant recreateParticipantCompletionParticipant(String id,
        ...
        if (id.startsWith(SubordinateBACoordinator.PARTICIPANT_PREFIX)) {
            // throw an exception because we don't expect participant completion
    ...
    public BusinessAgreementWithCoordinatorCompletionParticipant deserializeCoordinatorCompletionParticipant(String id,
        ...
        if (id.startsWith(SubordinateBACoordinator.PARTICIPANT_PREFIX)) {
            ...
            // use serialization
            throw new Exception(
                    "XTSBASubordinateRecoveryModule : invalid request to deserialize() subordinate WS-BA coordinator coordinator completion participant "
                            + id);
        }
        return null;
    }
    ...
    public BusinessAgreementWithCoordinatorCompletionParticipant recreateCoordinatorCompletionParticipant(String id,
        ...
        if (id.startsWith(SubordinateBACoordinator.PARTICIPANT_PREFIX)) {
            if (!id.endsWith("_CCP")) {
                // throw an exception because we don't expect participant
                // completion participants at present
                throw new Exception(
                        "XTSBASubordinateRecoveryModule : invalid name for subordinate WS-BA coordinator coordinator completion participant participant "
                                + id);
            }
            ...
            BusinessAgreementWithCoordinatorCompletionParticipant participant = (BusinessAgreementWithCoordinatorCompletionParticipant) participantClass
                    .newInstance();
    ...
    /**
     * we don't need to use this because the BA recovery manager tracks whether
     * a recovery scan has happened
     */
    public void endScan() {
        // do nothing
    }
