...
public class RecoveryBACoordinator extends BACoordinator {
    ...
    /**
     * provide the recovered participants with a handle on this coordinator so
     * they can propagate events through to it.
     */
    ...
    public void replayPhase2() {
        ...
        if (RecoveryLogger.logger.isDebugEnabled()) {
            RecoveryLogger.logger.debugv("RecoveryBACoordinator.replayPhase2 recovering {0} ActionStatus is {1}",
        ...
        if (_activated) {
            ...
            // which happens
            // when we crash between a successful complete beginning a close, or
            // COMMITTING, which
            // happens when we get a comms timeout from one of the participants
            // after sending it a CLOSE
            // message. in the former case all participant records will be
            // listed in the prepared list.
            ...
            if (RecoveryLogger.logger.isDebugEnabled()) {
                RecoveryLogger.logger.debugv("RecoveryBACoordinator.replayPhase2( {0} )  finished",
                        new Object[]{get_uid()});
            }
        } else {
