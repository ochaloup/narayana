...
public class UserTransactionImple extends UserTransaction {
    ...
    private final void enlistCompletionParticipants()
        ...
        try {
            completionCoordinator = tm.registerParticipant(completionParticipant,
    ...
    protected final Context startTransaction(int timeout, TxContextImple current)
            throws com.arjuna.wsc.InvalidCreateParametersException, SystemException {
        try {
    ...
    /**
     * Create an endpoint for the local participant service labelled with the
     * current context id which can be passed to the registration service and
     * handed on to the registered coordinator to call back to this transaction
     * 
     * @param id
     *            the current transaction context identifier
     * @return
     */
    private W3CEndpointReference getCompletionParticipant(final String id, final boolean isSecure) {
        ...
        final String address = serviceRegistry
                .getServiceURI(AtomicTransactionConstants.COMPLETION_INITIATOR_SERVICE_NAME, isSecure);
