...
public class CoordinationContextHelper {
    ...
    public static CoordinationContextType deserialise(final Element headerElement) throws JAXBException {
        ...
        try {
            ...
            // the header element is a valid CoordinationContextType node so we
            // can unpack it directly
            // using JAXB. n.b. we will see a mustUnderstand=1 in the
            // otherAttributes which we probably don't
            // want but it will do no harm.
            CoordinationContextType coordinationContextType = unmarshaller
                    .unmarshal(headerElement, CoordinationContextType.class).getValue();
    ...
    public static void serialise(final CoordinationContextType coordinationContextType, Element headerElement)
            throws JAXBException {
        // we would really like to just serialise the coordinationContextType
        // direct. But the JAXB context will
        // only generate a Node and we need to add a SOAPHeaderElement. So, we
        // cheat by serialising the
        // coordinationContextType into a header created by the caller, moving
        // all its children into the
        ...
        try {
            ...
            // sigh. native creates a copy list while CXF gives us the actual
            // child list so there is no simple way
            // to index into it from 0 to l which will work with both stacks.
            // instead we copy the entries to an array
            // and use it to locate the element we want to delete and the append
            // to the parent.
            for (int i = 0; i < l; i++) {
