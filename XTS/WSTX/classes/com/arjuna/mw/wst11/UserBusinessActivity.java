...
/**
 * This is the interface that allows BAs to be started and terminated. The
 * messaging layer converts the messages into calls on this.
 *
 ...
 * @version $Id: UserBusinessActivity.java,v 1.4.6.1 2005/11/22 10:36:06 kconner
 *          Exp $
 * @since XTS 1.0.
 */
...
public abstract class UserBusinessActivity {
     ...
     *
     * this is a UserBusinessActivity whose begin method will create an
     * interposed BA transaction
     * 
     * @return The user business activity.
     */
    ...
    /**
     * Start a new business activity with atomic outcome. If one is already
     * associated with this thread then the WrongStateException will be thrown.
     * Upon success, this operation associates the newly created transaction
     * with the current thread.
     */
    public abstract void begin() throws WrongStateException, SystemException;

    /**
     * Start a new BA with atomic outcome and the specified timeout as its
     * lifetime. If one is already associated with this thread then the
    ...
    /**
     * The BA is normally terminated by the close method. This signals to all
     * registered participants that the BA has ended and no compensation is
     * required.
     */
     ...
     * protocol then they will be expecting the application to inform them when
     * all work intended for them has been sent (and responded to). The complete
