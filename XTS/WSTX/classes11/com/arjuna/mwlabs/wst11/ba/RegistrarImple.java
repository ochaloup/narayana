...
public class RegistrarImple implements com.arjuna.wsc11.Registrar {
    ...
    public W3CEndpointReference register(final W3CEndpointReference participantProtocolService,
        ...
        if (tx instanceof SubordinateBACoordinator)
            return registerWithSubordinate((SubordinateBACoordinator) tx, participantProtocolService,
                    protocolIdentifier, isSecure);
        ...
        if (BusinessActivityConstants.WSBA_SUB_PROTOCOL_PARTICIPANT_COMPLETION.equals(protocolIdentifier)) {
            ...
            try {
                final ParticipantCompletionCoordinatorEngine engine = new ParticipantCompletionCoordinatorEngine(id,
            ...
            final String id = new Uid().stringForm();
            try {
                final CoordinatorCompletionCoordinatorEngine engine = new CoordinatorCompletionCoordinatorEngine(id,
                        participantProtocolService);
        ...
        } else if (com.arjuna.webservices.wsarjtx.ArjunaTXConstants.WSARJTX_PROTOCOL_TERMINATION
            ...
            terminator = (BusinessActivityTerminatorImple) TerminationCoordinatorProcessor.getProcessor()
        ...
        } else if (com.arjuna.webservices.wsarjtx.ArjunaTXConstants.WSARJTX_PROTOCOL_TERMINATION_RPC
                .equals(protocolIdentifier)) {
            /*
             * there is no end point ot update here so there is no need to look
             * up the terminator
             *
             * this is a hack because the terminator has already been created at
             * context create. this allows the enlist to be delayed until
             * close/cancel and just return the end point with the requisite
             * instance identifier. unfortunately this also means that you have
             * to register in order to complete but not remove the terminator.
             * 
             * TODO sort this out
             */
            /*
             * BusinessActivityTerminatorImple terminator; terminator =
             * (BusinessActivityTerminatorImple)
             * TerminationCoordinatorProcessor.getProcessor().getParticipant(
             * instanceIdentifier);
             */
            ...
            wstxLogger.i18NLogger.warn_mwlabs_wst_ba_Registrar11Imple_1(
    ...
    private final W3CEndpointReference registerWithSubordinate(final SubordinateBACoordinator theTx,
        ...
        if (BusinessActivityConstants.WSBA_SUB_PROTOCOL_PARTICIPANT_COMPLETION.equals(protocolIdentifier)) {
            ...
            try {
                // we use a manager which goes direct to the tx rather than via
                // the activity service
                BAParticipantManager manager = new SubordinateBAParticipantManagerImple(theTx, id);
                final ParticipantCompletionCoordinatorEngine engine = new ParticipantCompletionCoordinatorEngine(id,
                        participantProtocolService);
                BusinessAgreementWithParticipantCompletionImple participant = new BusinessAgreementWithParticipantCompletionImple(
                        manager, new BusinessAgreementWithParticipantCompletionStub(engine), id);
                engine.setCoordinator(participant.participantManager());
            ...
            try {
                ...
                final CoordinatorCompletionCoordinatorEngine engine = new CoordinatorCompletionCoordinatorEngine(id,
                        participantProtocolService);
                BusinessAgreementWithCoordinatorCompletionImple participant = new BusinessAgreementWithCoordinatorCompletionImple(
                        manager, new BusinessAgreementWithCoordinatorCompletionStub(engine), id);
                engine.setCoordinator(participant.participantManager());
    ...
    private W3CEndpointReference getParticipantManager(final QName serviceName, final QName endpointName,
            final String address, final String id) {
