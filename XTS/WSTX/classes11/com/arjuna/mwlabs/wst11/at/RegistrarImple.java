...
public class RegistrarImple implements Registrar {
    ...
    public W3CEndpointReference register(final W3CEndpointReference participantProtocolService,
        ...
        if (tx instanceof SubordinateATCoordinator)
            return registerWithSubordinate((SubordinateATCoordinator) tx, participantProtocolService,
        ...
        if (AtomicTransactionConstants.WSAT_SUB_PROTOCOL_DURABLE_2PC.equals(protocolIdentifier)) {
            ...
            try {
                final CompletionCoordinatorParticipant participant = new CompletionCoordinatorImple(_coordManager, hier,
                        true, participantProtocolService);
                CompletionCoordinatorProcessor.getProcessor().activateParticipant(participant,
            ...
            try {
                final CompletionCoordinatorParticipant participant = new CompletionCoordinatorRPCImple(_coordManager,
                        hier, true, participantProtocolService);
                CompletionCoordinatorRPCProcessor.getProcessor().activateParticipant(participant,
                        instanceIdentifier.getInstanceIdentifier());
            ...
            wstxLogger.i18NLogger.warn_mwlabs_wst_at_Registrar11Imple_1(AtomicTransactionConstants.WSAT_PROTOCOL,
                    protocolIdentifier);
    ...
    private W3CEndpointReference getCompletionCoordinator(final InstanceIdentifier instanceIdentifier,
        ...
        String address = ServiceRegistry.getRegistry()
                .getServiceURI(AtomicTransactionConstants.COMPLETION_COORDINATOR_SERVICE_NAME, isSecure);
    ...
    private W3CEndpointReference getCompletionCoordinatorRPC(final InstanceIdentifier instanceIdentifier,
            final boolean isSecure) {
        W3CEndpointReferenceBuilder builder = new W3CEndpointReferenceBuilder();
        String address = ServiceRegistry.getRegistry()
                .getServiceURI(AtomicTransactionConstants.COMPLETION_COORDINATOR_RPC_SERVICE_NAME, isSecure);
    ...
    private W3CEndpointReference getCoordinator(final String participantId, final boolean isSecure) {
        W3CEndpointReferenceBuilder builder = new W3CEndpointReferenceBuilder();
        String address = ServiceRegistry.getRegistry()
                .getServiceURI(AtomicTransactionConstants.COORDINATOR_SERVICE_NAME, isSecure);
