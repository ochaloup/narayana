 ...
 * ArjunaCore TwoPhaseCoordinator via the BACoordinator. This is the subordinate
 * coordinator implementation which we use when doing interposition.
 *
 * This implementation registers itself as a coordinator completion participant
 * in its parent activity. It has to use the coordinator completion protocol
 * because it relies upon dispatch of a complete request from the parent to
 * drive logging of i) the subordinate transaction state then ii) the state of
 * its registered proxy participant. These operations must both happen in that
 * order and only when the parent transaction is preparing to close. Note that
 * this does not imply that the transaction is closed, merely that it goes
 * through phase 1 of the transaction termination protocol (PREPARE) leaving a
 * transaction log record on the disk (albeit in state COMMITTING). It must
 * still be possible to leave phase 2 open for either close (COMMIT) or
 * compensate (ROLLBACK).
 *
 * Why? Well, if the participant logs its state first either at parent complete
 * or, if participant completion is in use, before parent complete, then a crash
 * risks losing details of the subordinate transaction -- including potentialy
 * completed participants which may require closing or compensating. Note that
 * this danger bypasses any question of whether or when subsequently the
 * subordinate transaction state might be logged -- the participant cannot log
 * its state and guarantee to close or compensate correctly without having first
 * ensured that it can guarantee to close or compensate subordinate
 * participants.
 *
 * Contrariwise, if the subordinate transaction logs its state first and a crash
 * occurs before the proxy participant state can be saved then it is still
 * possible during recovery to correlate the presence of the logged subordinate
 * transaction with the absence of its proxy participant and automatically
 * compensate the subordinate transaction's completed participants. This will
 * match the action of the parent since absence of the proxy participant
 * requires it also to compensate. It is not possible to commit any earlier than
 * parent complete because it is still legitimate for participants to register
 * before that event.
 *
 * So, the implication of this is that when the proxy notifies complete to this
 * coordinator it must perform the usual complete processing and also drive
 * phase 1 of the commit process. Later, when a close request is dispatched, it
 * can drive phase 2 of the commit process.
 *
 * If there is a failure during phase 1 of the commit process then the
 * subordinate transaction will not be logged. In this case the proxy
 * participant must notify a fail to the parent transaction and avoid logging
 * its own state. This is safe because a crash will automatically cause a
 * subsequent resend of the complete request to return fail because the proxy
 * participant is unknown.
 *
 * If phase 1 completes successfully then the proxy participant must log its
 * state and notify completed to the parent trasaction. If a crash occurs
 * between logging the subordinate transaction state and logging the proxy state
 * then the absence of the proxy participant can be reconciled during recovery
 * and the subordinate transaction can be automatically compensated. If a crash
 * occurs between logging the proxy participant and notifying completed to the
 * parent transaction then the participant will be recreated during recovery and
 * a subsequent resend of complete can be answered with completed. Note that
 * this means complete requests for unknown participants must only be answered
 * after the first pass of the BA participant recovery module has completed.
 *
...
public class SubordinateBACoordinator extends BACoordinator {
    ...
    public void complete() throws WrongStateException, SystemException {
        ...
        // if we have prepared ok (or are read only) then status will be
        // COMMITTING
    ...
    /**
     * this is driven by a coordinator-completion participant registered on
     * behalf of the coordinator and is required to propagate the close to all
    ...
    public int close() throws SystemException {
        ...
        if (status == ActionStatus.COMMITTING) {
            ...
            // we have not yet completed so we can rely upon the parent
    ...
    /**
     * this is driven by a coordinator-completion participant registered on
     * behalf of the coordinator and is required to propagate the cancel to all
     * registered participants.
     */
    public int cancel() {
        int status = status();
        ...
        // TODO -- check if there is a window here where status could change to
        // COMMITTING
        if (status == ActionStatus.COMMITTING) {
            phase2Abort(true);
            result = status();
    ...
    /**
     * return a uid for the corodinator completion participant registered on
     * behalf of this coordinator
     */
