...
/**
 * The CoordinatorManager is the way in which services can enlist participants
 * with the current coordinator.
 *
 ...
 * TODO
 * 
 * Currently there is no way for users to get direct access to the current
 * coordinator instance (c.f. getting the Transaction from the JTA equivalent
 * interfaces). Do we need to add this? If so, then we should add an interface
 * for the coordinator implementations to implement, so that implementation
 * specific details don't creep into the API.
 */
...
public interface CoordinatorManager extends UserCoordinator {
    ...
    public void enlistParticipant(Participant act) throws NoActivityException, WrongStateException,
            DuplicateParticipantException, InvalidParticipantException, SystemException;

    /**
     * Remove the specified participant from the coordinator associated with the
     * current thread.
     *
     * @param act
    ...
    public void enlistSynchronization(Synchronization act) throws NoActivityException, WrongStateException,
            DuplicateSynchronizationException, InvalidSynchronizationException, SystemException;

    /**
     * Remove the specified synchronization from the coordinator associated with
     * the current thread.
     *
     * @param act
     *            The synchronization to remove.
     *
     * @exception NoActivityException
     *                Thrown if there is no activity associated with the current
     *                thread.
    ...
    public void delistSynchronization(Synchronization act)
            throws NoActivityException, InvalidSynchronizationException, WrongStateException, SystemException;
