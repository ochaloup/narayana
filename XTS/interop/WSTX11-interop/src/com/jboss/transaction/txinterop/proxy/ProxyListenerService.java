...
public class ProxyListenerService extends HttpServlet {
    ...
    public void doPost(final HttpServletRequest httpServletRequest, final HttpServletResponse httpServletResponse)
        ...
        final String alternateConversationIdentifier = ProxyConversation
                .getAlternateConversationId(conversationIdentifier);
        ...
        // Search header for wsa:To and wsa:Address elements, changing their URL
        // parts as we go.
        ...
        try {
            ...
            final AddressingProxySAXHandler addressingHandler = new AddressingProxySAXHandler(writerHandler,
                    alternateConversationIdentifier);
            ...
            final StringBuffer messageBuffer = (jbossClient
                    ? newMessageBuffer
                    : new StringBuffer(new String(requestContents)));
                ...
                while (headerNameEnum.hasMoreElements()) {
                    ...
                    if (name.equalsIgnoreCase(TRANSFER_ENCODING_HEADER)) {
                        ...
                        if (!value.equalsIgnoreCase(TRANSFER_ENCODING_VALUE_CHUNKED)) {
                            // this may actually cause a problem depending upon
                            // the encoding but try it anyway
                        ...
                        if (is != null) {
                            ...
                            if (responseBytes.length > 0) {
                                try {
                                    ...
                                    final WriterSAXHandler responseWriterHandler = new WriterSAXHandler(
                                            newResponseWriter);
                                    final AddressingProxySAXHandler responseAddressingHandler = new AddressingProxySAXHandler(
                                            responseWriterHandler, alternateConversationIdentifier);

                                    // refetch the state handler so it gets a
                                    // chance to go away
                                    ...
                                    // always use the standard handler for
                                    // replies?
                                    ...
                                    final String responseString = (jbossClient
                                            ? newResponseString
                                            : new String(responseBytes));
                                    if ((contentType != null) && !(SOAP_11_CONTENT_TYPE.equals(contentType)
                                            || SOAP_12_CONTENT_TYPE.equals(contentType))) {
                                        ProxyConversation.appendConversation(conversationIdentifier,
                                                escapeContents(responseString));
                                    } else {
     ...
     *             for errors.
     */
    private byte[] getContents(final InputStream is) throws IOException {
    ...
    /**
     * Check for the XML declaration and remove. This method is only used if we
     * are intending to log the SOAP message so that it is easy to combine the
     * XML without creating invalid documents.
     * 
     * @param contents
     *            The current stream contents.
     * @return The stream contents as a string.
     */
    private static String checkForXMLDecl(final StringBuffer contents) {
        int count = 0;
        try {
            while (Character.isWhitespace(contents.charAt(count)))
                count++;
