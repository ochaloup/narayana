...
/**
 * bean storing WS-C implementation configuration values derived from the xts
 * properties file, system property settings and, in case we are running inside
 * JBossAS the xts bean.xml file
 */
@PropertyPrefix(prefix = "org.jboss.jbossts.xts")
public class WSCEnvironmentBean {
    /**
     * the initial time to wait before resending a coordination protocol
     * messaage. this is increased gradually, doubling every two resends up to
     * the maximum value
     */
    ...
    /**
     * the maximum time to wait before resending a coordination protocol
     * messaage
     */
    ...
    /**
     * the wait period after sending a protocol message and not getting the
     * expected reply before it is decided that the server at the other end has
     * crashed. this wait is only used for some exchanges. a timeout may be
     * handled by transitioning to a different transaction state. alternatively
     * the service may merely begin resending the message starting at the
     * initial transport period.
     */
    @FullPropertyName(name = "org.jboss.jbossts.xts.transport.transportTimeout")
    private volatile int transportTimeout = 30 * 1000;

    /**
     * the bind address on which the web service is listening. when running in
     * JBoss AS this is derived from the JBoss Web service bean by injection.
     */
    ...
    /**
     * the URL to be used by XTS clients to address the Activation Coordinator
     * service when starting a transaction. if this is undefined then a URL is
     * constructed using the scheme, host address, port and URL path specified
     * in the properties file or, where they are missing, using their default
     * values.
     */
    ...
    /**
     * the scheme which will be used to construct a coordinator URL if it is not
     * specified
     */
    ...
    /**
     * the address which will be used to construct a coordinator URL if it is
     * not specified
     */
    ...
    /**
     * the port which will be used to construct a coordinator URL if it is not
     * specified
     */
    ...
    /**
     * the path which will be used to construct a coordinator URL if it is not
     * specified
     */
    ...
    /**
     * the bind address on which the web service is listening. when running in
     * JBoss AS this is derived from the JBoss Web service bean by injection. if
     * not set it defaults to localhost.
     */
    @Deprecated
    ...
    /**
     * the port which will be used to service non-secure requests. if not set it
     * defaults to 8080.
     */
    @Deprecated
    ...
    /**
     * the port which will be used to service secure requests. if not set it
     * defaults to 8080. n.b. the 1.0 implementation deploys its own servlet and
     * it only listens on the non-secure port.
     */
    @Deprecated
    ...
    /**
     * the URL path component of the URL at which 1.1 WS-C services have been
     * mapped. if this is null then the path is defaulted to "ws-c11"
     */
    @FullPropertyName(name = "org.jboss.jbossts.xts.wsc11.serviceURLPath")
    private volatile String serviceURLPath = null;

    /**
     * Returns initial time to wait before resending a coordination protocol
     * message. This is increased gradually, doubling every two resends up to
     * the maximum value
     *
     * @return initial time to wait before resending a coordination protocol
     *         message.
     */
    ...
    /**
     * Sets initial time to wait before resending a coordination protocol
     * message.
     *
     * @param initialTransportPeriod
     *            initial time to wait before resending a coordination protocol
     *            message.
     */
    ...
    /**
     * Returns maximum time to wait before resending a coordination protocol
     * message.
     *
     * @return maximum time to wait before resending a coordination protocol
     *         message.
     */
    ...
    /**
     * Sets maximum time to wait before resending a coordination protocol
     * message.
     *
     * @param maximumTransportPeriod
     *            maximum time to wait before resending a coordination protocol
     *            message.
     */
    ...
    /**
     * Returns the wait period after sending a protocol message and not getting
     * the expected reply before it is decided that the server at the other end
     * has crashed. This wait is only used for some exchanges. A timeout may be
     * handled by transitioning to a different transaction state. Alternatively
     * the service may merely begin resending the message starting at the
     * initial transport period.
     *
     * @return wait period before deciding that the server at the other end has
     *         crashed.
     */
    ...
    /**
     * Sets the wait period after sending a protocol message and not getting the
     * expected reply before it is decided that the server at the other end has
     * crashed.
     *
     * @param transportTimeout
     *            wait period before deciding that the server at the other end
     *            has crashed.
     */
    ...
    /**
     * this setter is used by the microcontainer to inject the inet address
     * supplied during app server startup.
     */
    ...
    /**
     * Returns the bind address on which the web service is listening. When
     * running in JBoss AS this is derived from the JBoss Web service bean by
     * injection.
     *
     * WS-C version: 1.1.
     *
    ...
    /**
     * Returns the URL to be used by XTS clients to address the Activation
     * Coordinator service when starting a transaction. If this is undefined
     * then a URL is constructed using the scheme, host address, port and URL
     * path specified in the properties file or, where they are missing, using
     * their default values.
     *
     * WS-C version: 1.1.
     *
     * @return the URL to be used by XTS clients to address the Activation
     *         Coordinator service when starting a transaction.
     */
    public String getCoordinatorURL11() {
        if (coordinatorURL11 == null) {
            // compute the required value
            return getCoordinatorScheme11() + "://" + getCoordinatorAddress11() + ":" + getCoordinatorPort11() + "/"
    ...
    /**
     * Sets the URL to be used by XTS clients to address the Activation
     * Coordinator service when starting a transaction.
     *
     * WS-C version: 1.1.
     *
     * @param coordinatorURL11
     *            the URL to be used by XTS clients to address the Activation
     *            Coordinator service when starting a transaction.
     */
    ...
    /**
     * Returns the scheme which will be used to construct a coordinator URL if
     * it is not specified.
     *
     * WS-C version: 1.1.
     *
     * @return the scheme which will be used to construct a coordinator URL if
     *         it is not specified.
     */
    ...
    /**
     * Sets the scheme which will be used to construct a coordinator URL if it
     * is not specified.
     *
     * WS-C version: 1.1.
     *
     * @param coordinatorScheme11
     *            the scheme which will be used to construct a coordinator URL
    ...
    /**
     * Returns the address which will be used to construct a coordinator URL if
     * it is not specified.
     *
     * WS-C version: 1.1.
     *
     * @return the address which will be used to construct a coordinator URL if
     *         it is not specified.
     */
    ...
    /**
     * Sets the address which will be used to construct a coordinator URL if it
     * is not specified.
     *
     * WS-C version: 1.1.
     *
     * @param coordinatorAddress11
     *            the address which will be used to construct a coordinator URL
     *            if it is not specified.
     */
    ...
    /**
     * Returns the port which will be used to construct a coordinator URL if it
     * is not specified.
     *
     * WS-C version: 1.1.
     *
     * @return the port which will be used to construct a coordinator URL if it
     *         is not specified.
     */
    ...
    /**
     * Sets the port which will be used to construct a coordinator URL if it is
     * not specified.
     *
     * WS-C version: 1.1.
     *
     * @param coordinatorPort11
     *            the port which will be used to construct a coordinator URL if
     *            it is not specified.
     */
    ...
    /**
     * Returns the path which will be used to construct a coordinator URL if it
     * is not specified.
     *
     * WS-C version: 1.1.
     *
     * @return the path which will be used to construct a coordinator URL if it
     *         is not specified
     */
    ...
    /**
     * Sets the path which will be used to construct a coordinator URL if it is
     * not specified.
     *
     * WS-C version: 1.1.
     *
     * @param coordinatorPath11
     *            the path which will be used to construct a coordinator URL if
     *            it is not specified.
     */
     ...
     * Returns the bind address on which the web service is listening. When
     * running in JBoss AS this is derived from the JBoss Web service bean by
     * injection. If not set it defaults to localhost.
     *
     * WS-C version: 1.0.
     *
     * @return the bind address on which the web service is listening.
    ...
    /**
     * Returns the port which will be used to service non-secure requests. If
     * not set it defaults to 8080.
     *
     * WS-C version: 1.0.
     *
     * @return the port which will be used to service non-secure requests.
    ...
    /**
     * Returns the port which will be used to service secure requests. If not
     * set it defaults to 8080. n.b. the 1.0 implementation deploys its own
     * servlet and it only listens on the non-secure port.
     *
     * WS-C version: 1.0.
     *
     * @return the port which will be used to service secure requests.
    ...
    /**
     * Sets the port which will be used to service secure requests. n.b. the 1.0
     * implementation deploys its own servlet and it only listens on the
     * non-secure port.
     *
     * WS-C version: 1.0.
     *
     * @param bindPortSecure10
     *            the port which will be used to service secure requests.
     */
    @Deprecated
    ...
    public String getCoordinatorURL10() {
        if (coordinatorURL10 == null) {
            // compute the required value
            return getCoordinatorScheme10() + "://" + getCoordinatorAddress10() + ":" + getCoordinatorPort10() + "/"
    ...
    /**
     * Returns the URL path component of the URL at which 1.1 WS-C services have
     * been mapped. If this is null then the path is defaulted to "ws-c11".
     *
     * @return the URL path component of the URL at which 1.1 WS-C services have
     *         been mapped.
     */
    ...
    /**
     * Sets the URL path component of the URL at which 1.1 WS-C services have
     * been mapped.
     *
     * @param serviceURLPath
     *            the URL path component of the URL at which 1.1 WS-C services
     *            have been mapped.
     */
