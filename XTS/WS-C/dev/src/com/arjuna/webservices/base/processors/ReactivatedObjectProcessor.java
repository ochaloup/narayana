...
/**
 * A specialization of ActivatedObjectProcessor which allows for ghost entries
 * to be left in the table after deletion. A ghost entry cannot be retrieved by
 * a normal getObject(id) which will return null, indicating that no object with
 * the supplied id exists. However, the ghost's presence can be detected using
 * getGhost(id).
 * </p>
 *
 * Ghost entries are used to identify objects which have failed to be terminated
 * due to an unavailable participant or coordinator and so are still present in
 * an unprocessed log record. When recovery processing recreates a participant
 * the recovered instance replaces the ghost entry, ensuring that sbsequent
 * messages update the participant whose recovery is being driven by the
 * coordinator.
 */
...
public class ReactivatedObjectProcessor extends ActivatedObjectProcessor {
    ...
    public synchronized boolean getGhost(String identifier) {
        if (reactivationProcessingStarted) {
            ...
            // until we have been notified of at least one complete recovery
            // scan pass we have
            // to assume that any identifier may have an entry in the log so we
            // return true
            return true;
        }
    }
    ...
    /**
     * a global flag which is false at boot and is set to true once a recovery
     * log scan for XTS data has completed
     */
