...
public class BATestCase extends InteropTestCase {
    ...
    /**
     * testBA1_1 - 1.1 Cancel The IA cancels the activity before the PA
     * completes its work on behalf of the activity.
     * 
     * @throws Exception
     *             on failure.
     */
    public void testBA1_1() throws Exception {
        ...
        try {
            ...
            final CoordinationContextType context = BAInteropUtil
                    .createAtomicOutcomeCoordinationContext(getSourceCoordinatorURI());
            getParticipantStub().cancel(serviceURI, context);

            final BusinessActivityTerminator terminator = BAInteropUtil.registerTerminator(context,
                    context.getIdentifier().getValue());
            terminator.cancel();
    ...
    /**
     * testBA1_2 - 1.2 Exit The PA exits without completing any work on behalf
     * of the activity.
     * 
     * @throws Exception
     *             on failure.
    ...
    /**
     * testBA1_3 - 1.3 Fail The PA fails before completing its work on behalf of
     * the activity.
     * 
     * @throws Exception
     *             on failure.
    ...
    /**
     * testBA1_4 - 1.4 CannotComplete The PA sends CannotComplete before
     * completing its work on behalf of the activity.
     * 
     * @throws Exception
     *             on failure.
     */
    ...
    /**
     * testBA1_5 - 1.5 ParticipantCompleteClose The PA successfully completes
     * its work on behalf of the activity and the activity is closed.
     * 
     * @throws Exception
     *             on failure.
     */
    public void testBA1_5() throws Exception {
        final String conversationId = getConversationId();
        final BAInteropParticipantCompletedState state = new BAInteropParticipantCompletedState(
                BusinessActivityConstants.WSBA_ACTION_CLOSED);
        ProxyConversation.setConversationState(conversationId, state);
        try {
            final String serviceURI = ProxyURIRewriting.rewriteURI(conversationId, getParticipantURI());
            final CoordinationContextType context = BAInteropUtil
    ...
    /**
     * testBA1_6 - 1.6 CoordinatorCompleteClose The PA successfully completes
     * its work on behalf of the activity and the activity is closed.
     * 
     * @throws Exception
     *             on failure.
    ...
    /**
     * testBA1_7 - 1.7 UnsolicitedComplete Tests a protocol error, participant
     * send Completed for coordinator completion. Throws an invalid state fault.
     * 
     * @throws Exception
     *             on failure.
     *
     *             this test relies on rewriting the protocol at registration
     *             time so that the returned endpoint is for the coordinator
     *             completion coordinator rather than the participant completion
     *             coordinator. the participant engine tries to use the endpoint
     *             to send an unsolicited complete to the coordinator completion
     *             coordinator. unfortunately, since the endpoint nwo contains
     *             service and port metadata the send fails. so this test has
     *             had to be decommissioned.
     */

    /*
     * public void testBA1_7() throws Exception { final String conversationId =
     * getConversationId() ; final BAInteropUnsolicitedCompleteState state = new
     * BAInteropUnsolicitedCompleteState() ;
     * ProxyConversation.setConversationState(conversationId, state) ; try {
     * final String serviceURI = ProxyURIRewriting.rewriteURI(conversationId,
     * getParticipantURI()) ; final CoordinationContextType context =
     * BAInteropUtil.createAtomicOutcomeCoordinationContext(
     * getSourceCoordinatorURI()) ;
     * getParticipantStub().unsolicitedComplete(serviceURI, context) ;
     * 
     * state.waitForCompletion(getTestTimeout()) ;
     * 
     * final BusinessActivityTerminator terminator =
     * BAInteropUtil.registerTerminator(context,
     * context.getIdentifier().getValue()) ; terminator.cancel() ; } finally {
     * ProxyConversation.clearConversationState(conversationId) ; }
     * assertTrue("Conversation did not complete successfully",
     * state.isSuccessful()) ; }
     */
    //
    /**
     * testBA1_8 - 1.8 Compensate The PA successfully completes its work on
     * behalf of the activity and the activity is compensated.
     * 
     * @throws Exception
     *             on failure.
     */
    public void testBA1_8() throws Exception {
        final String conversationId = getConversationId();
        final BAInteropParticipantCompletedState state = new BAInteropParticipantCompletedState(
                BusinessActivityConstants.WSBA_ACTION_COMPENSATED);
        ProxyConversation.setConversationState(conversationId, state);
        try {
            final String serviceURI = ProxyURIRewriting.rewriteURI(conversationId, getParticipantURI());
            final CoordinationContextType context = BAInteropUtil
    ...
    /**
     * testBA1_9 - 1.9 CompensationFail The PA successfully completes its work
     * on behalf of the activity but compensation fails..
     * 
     * @throws Exception
     *             on failure.
     */
    public void testBA1_9() throws Exception {
        final String conversationId = getConversationId();
        final BAInteropParticipantCompletedState state = new BAInteropParticipantCompletedState(
                BusinessActivityConstants.WSBA_ACTION_FAILED);
        ProxyConversation.setConversationState(conversationId, state);
        try {
            final String serviceURI = ProxyURIRewriting.rewriteURI(conversationId, getParticipantURI());
            final CoordinationContextType context = BAInteropUtil
    ...
    /**
     * testBA1_10 - 1.10 ParticipantCancelCompletedRace Simulate a race between
     * cancel and completed.
     * 
     * @throws Exception
     *             on failure.
     */
    public void testBA1_10() throws Exception {
        final String conversationId = getConversationId();
        final BAInteropDroppedParticipantCompletedState state = new BAInteropDroppedParticipantCompletedState(
                BusinessActivityConstants.WSBA_ACTION_COMPENSATED);
        ProxyConversation.setConversationState(conversationId, state);
        try {
            final String serviceURI = ProxyURIRewriting.rewriteURI(conversationId, getParticipantURI());
    ...
    /**
     * testBA1_11 - 1.11 MessageLossAndRecovery The IA cancels the activity
     * before the PA completes its work on behalf of the activity.
     * 
     * @throws Exception
     *             on failure.
     */
    ...
    // * The IA cancels the activity before the PA completes its work on behalf
    // of the activity.
    ...
    // {
    // final String conversationId = getConversationId() ;
    // final ATInteropWaitState state = new
    // ATInteropWaitState(BusinessActivityConstants.WSAT_ACTION_COMMITTED) ;
    // ProxyConversation.setConversationState(conversationId, state) ;
    // try
    // {
    // final String serviceURI = ProxyURIRewriting.rewriteURI(conversationId,
    // getParticipantURI()) ;
    // final CoordinationContextType context =
    // BAInteropUtil.createMixedOutcomeCoordinationContext(getSourceCoordinatorURI())
    // ;
    // getParticipantStub().commit(serviceURI, context) ;
    //
    // final CompletionCoordinatorParticipant participant =
    // BAInteropUtil.registerCompletion(context,
    // context.getIdentifier().getValue()) ;
    // participant.commit() ;
    //
    // state.waitForCompletion(getTestTimeout()) ;
