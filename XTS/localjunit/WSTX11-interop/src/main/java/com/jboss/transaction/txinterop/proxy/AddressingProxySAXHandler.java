...
public class AddressingProxySAXHandler extends BaseHandler {
    ...
    public void startElement(final String uri, final String localName, final String qName, final Attributes attributes)
        ...
        } else if (ArjunaConstants.WSARJ_NAMESPACE.equals(uri)
                && ArjunaConstants.WSARJ_ELEMENT_INSTANCE_IDENTIFIER.equals(localName)
                && (attributes.getValue(WSA_NAMESPACE_URI,
                        AddressingConstants.WSA_ATTRIBUTE_IS_REFERENCE_PARAMETER) != null)) {
    ...
    public void endElement(final String uri, final String localName, final String qName) throws SAXException {
        if (characterContent.length() > 0) {
            if (inRewriteElement) {
                final String newValue = ProxyURIRewriting.rewriteURI(conversationIdentifier,
                        characterContent.toString().trim());
