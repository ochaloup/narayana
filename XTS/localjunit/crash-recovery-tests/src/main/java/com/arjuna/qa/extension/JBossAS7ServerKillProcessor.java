...
public class JBossAS7ServerKillProcessor implements ServerKillProcessor {
    ...
    public void kill(Container container) throws Exception {
        ...
        if (killed) {
            ...
            if (env == null)
                ...
                p = Runtime.getRuntime().exec(shutdownSequence, new String[]{"JAVA_OPTS=" + env});
    ...
    private boolean checkJBossAlive() throws Exception {
        ...
        if (rc != 0 && rc != 1) {
            ...
            p.destroy();
            throw new RuntimeException("Kill Sequence " + killSequence + " failed. Returned " + rc + " CLI_IPV6_OPTS="
                    + System.getenv().get("CLI_IPV6_OPTS"));
        }
