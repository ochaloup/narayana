...
/**
 * A class which provides the ability to interpret XTS service test commands.
 * Each of the XTS service test classes imnplementing a specific test extends
 * this class so that it can drive the test from a command script. Each of the
 * XTS service test endpoints employs an instance of this class to process
 * comands which are dispatched to it either from one of the test class
 * instances or recursively via another intermediate service.
 *
 * The interpreter executes a list of Strings which represents one or more
 * commands each with its own sequence of associated arguments. It returns a
 * list of strings indicating the outcome of processing the commands. It throws
 * an exception if command processing fails.
 *
 * A simple command comprises a list of Strings headed by one of a fixed set of
 * command keywords followed by zero or more argument strings. Command arguments
 * are either simple string literals or bound variable references. A variable
 * reference is an alphanumeric variable name bracketed by '{' and '}'
 * characters. The interpreter resolves this to a string value by looking up the
 * variable in its binding list and uses this vale as the argument for th
 * command. It is possible to seed the binding list with default values before
 * processing a sequence of commands. Alternatively, bindings may be added
 * during execution of block commands.
 *
 * A block commands comprises a list of Strings headed by the keyword "block"
 * and terminated by the keyword "endblock". Intervening elements of the list
 * comprise simple commands separated either by "next" keywords or by bind
 * commands terminated with a "next" keyword. A bind commands is a list of
 * strings headed by keyword "bind" and succeeded by a sequence of bind
 * operations. A bind operation operates on the results returned from the last
 * executed command and comes in one of three formats:
 * <ul>
 * <li>set var idx
 * <li>check idx value/{var}"
 * <li>output idx
 * </ul>
 * A set operation binds a variable to the nth result returned in the results
 * list of the previously executed command. A check operation checks that the
 * nth result returned in the results list of the previously executed command
 * has a specific value, throwing an exception if not. An output operation
 * appends the nth result returned in the results list of the previously
 * executed command to the results list for the block command.
 *
 ...
 *
 * serviceURL is a URL identifying an instance of the test service to which
 * commands are to be recursively dispatched within the relevant subtransaction
 * or subactivity
 */
public class XTSServiceTestInterpreter {
    ...
    /**
     * lookup (or create and index) then return a service implementation keyed
     * by a specific servlet path
     * 
     * @param servletPath
     *            the key used to identify a service obtained from an HTTP
     *            request's servlet path
    ...
    /**
     * simple command interpreter which executes the commands in the command
     * list, inserting the corresponding results in the results list. n.b. this
     * method should only ever be invoked from within an AT or BA transaction.
     *
     * @param commandList
     *            a list of command strings
     * @param resultsList
     *            a list into which results strings are to be inserted
     */
    public void processCommands(List<String> commandList, List<String> resultsList) throws WebServiceException {
        HashMap<String, String> bindings = new HashMap<String, String>(defaultBindings);
        processCommands(commandList, resultsList, bindings);
    }

    /**
     * add a binding to the default binding set used to supply command
     * parameters
     * 
     * @param var
     * @param val
    ...
    /**
     * simple command interpreter which executes the commands in the command
     * list, inserting the corresponding results in the results list and using
     * the supplied bindings list to provide values for any parameters supplied
     * in the commands and to bind any results obtained by executing the
     * commands
     *
     * @param commandList
     *            a list of command strings
     ...
     *            a list into which results strings are to be inserted
     * @param bindings
     *            a list of bound variables to be substituted into commands or
    ...
    private void processCommands(List<String> commandList, List<String> resultsList, HashMap<String, String> bindings)
        ...
        if (command.equals("block")) {
            // we don't bind the command block immediately since bindings may be
            // produced
            ...
            if (command.equals("enlistDurable")) {
                ...
                // we should find a web service URL and a list of commands to
                // dispatch to that service
                ...
                // this is surplus to requirements since we should really be
                // running against a service which uses
                // the subordinate interposition JaxWS handler to install a
                // subordinate transaction before
                // entering the service method. we ought to test that handler
                // rather than hand crank the
                ...
                // this is surplus ot requirements since we should really be
                // running against a service which uses
                // the subordinate interposition JaxWS handler to install a
                // subordinate activity before
                // entering the service method. we ought to test that handler
                // rather than hand crank the
                // interposition in the service
                TxContext currentTx;
                ...
                // we should find the id of a subordinate transaction, a web
                // service URL
                ...
                // ok, now we install the relevant transaction and then just
                // pass the commands on to
                // the web service

                // we allow unresolved variable references in the rest of the
                // command list as
    ...
    /**
     * execute a block of commands by recursively executing each embedded
     * command list. results are verified and bound to command variables in
     * accordance with embedded bind commands. command variable references in
     * embedded command lists are substituted with the corresponding bound
     ...
     * @throws WebServiceException
     */
    private void processCommandBlock(List<String> commandList, List<String> resultsList,
            HashMap<String, String> bindings) throws WebServiceException {
        // break up the command block into a list of nested command lists.
        // successive command lists will
        // be separated by a "next" or "bind" command. the block will be
        // terminated by an "endblock" command.
        ...
        // ok, we should have a list of command lists all starting with either
        // next or bind
        // now we execute each next command list in turn after substituting
        // bound variables
        // of the form $xxx and we execute each bind command list by testing
        // result values
        // against literals or binding result values to variables and possibly
        // outputting
    ...
    /**
     * for each command in the command list which contains variable references
     * substitute a command containing the value for the variable found in
     * bindings. variables are mentioned by wrappng their name in braces.
     *
     * @param commands
     *            the list of commands to be processed
     * @param bindings
     *            a map from variable names to the associated values
     * @throws WebServiceException
     *             if a variable reference is invalidly formatted or refers to
     *             an unbound variable
     */
    private void bindCommands(List<String> commands, HashMap<String, String> bindings) throws WebServiceException {
     ...
     *            a map from variable names to the associated values
     * @param mustResolve
     *            is true if references to unbound variables should result in a
     *            thrown exception and false if they should be tolerated by
     *            being left unsubstituted
     * @throws WebServiceException
     *             if a variable reference is invalidly formatted or refers to
     *             an unbound variable when mustResolve is true
     */
    private void bindCommands(List<String> commands, HashMap<String, String> bindings, boolean mustResolve)
        ...
        int idx;
        for (idx = 0; idx < size; idx++) {
            // pop the command and append either the original or a substituted
            // copy
     ...
     * 
     * @param command
     *            the command to be substituted
     * @param bindings
     *            the map from currently bound variables to their string values
     * @param mustResolve
     *            is true if references to unbound variables should result in a
     *            thrown exception and false if they should be tolerated by
     *            being left unsubstituted
     * @return the substituted command or the original command if it contains no
     *         variable references
     * @throws WebServiceException
     *             if a variable reference is invalidly formatted or refers to
     *             an unbound variable
    ...
    private String bindCommand(String command, HashMap<String, String> bindings, boolean mustResolve)
        ...
        if (command.contains("{")) {
            ...
            while (pos < len) {
                ...
                if (c == '{') {
                    // must have room for at least one character and a closing
                    // brace
                    if (pos > len - 2) {
                        throw new WebServiceException(
                                "bindCommand : invalid variable reference " + command + " @ " + pos);
                    }
    ...
    /**
     * process each command in the binding commands list. binding commands
     * include "set var idx" which binds a variable to a value obtained from
     * inResults, "check idx value" which checks that a value obtaind from
     * inResults has a given value and "output idx", which inserts a value
     * obtained from inResults into outResults.
     * 
     * @param commands
     * @param inResults
     * @param outResults
     ...
     * @throws WebServiceException
     */
    private void bindResults(List<String> commands, List<String> inResults, List<String> outResults,
            HashMap<String, String> bindings) throws WebServiceException {
        int size = commands.size();
        int idx;
        for (idx = 0; idx < size; idx++) {
    ...
    /**
     * utiilty method provided to simplify recursive dispatch of commands to
     * another web service. this is intended to be used to create and drive
     * participants in subordinate transactions
     * 
     * @param url
     * @param commands
     * @return
    ...
    /**
     * obtain a new participant name starting with a prefix recognised by the
     * recovery code and terminated with the supplied suffix and a unique
     * trailing number
     * 
     * @param suffix
     *            a component to be added to the name before the counter
     *            identifying the type of participant
     * @return
     */
    ...
    /**
     * obtain a new transaction name starting with a transaction prefix and
     * terminated with the supplied suffix and a unique trailing number
     * 
     * @param suffix
     *            a component to be added to the name before the counter
    ...
    /**
     * map used to identify specific service implementations with server url
     * paths
     */
    ...
    /**
     * a table used to retain a handle on enlisted participants so that they can
     * be driven by the client to perform actions not contained in the original
     * command script.
     */
    ...
    /**
     * a client used to propagate requests recursively from within
     * subtransactions or subactivities
     */
