...
/**
 * Handler to serialise and deserialise a coordination context to/from a SOAP
 * header.
 */
public class CoordinationContextHandler implements SOAPHandler<SOAPMessageContext> {
     ...
     *         instance. <code>QName</code> is the qualified name of the
    ...
    /**
     * Handle an outgoing message by inserting any current arjuna context
     * attached to the context into the message headers and handle an incoming
     * message by retrieving the context from the headers and attaching it to
     * the context,
     *
     * @param context
     *            the message context.
    ...
    /**
     * check for an arjuna context attached to the message context and, if
     * found, install its identifier as the value of a soap message header
     * element
     * 
     * @param context
     * @return
    ...
    protected boolean handleMessageOutbound(SOAPMessageContext context) throws ProtocolException {
        try {
            ...
            if (coordinationContext != null) {
                ...
                // insert a header into the current message containing the
                 ...
                 *
                 * The problem here is that the marshaller creates plain old
                 * elements and inserts them top down as it goes along. but the
                 * soap header add child method checks its argument and replaces
                 * plain elements with soap header elements before inserting
                 * them. it copies the inserted element substructure into the
                 * rpelacement but since it does not exist at copy time the
                 * chiuldren get lost Marshaller marshaller =
                 * jaxbCtx.createMarshaller();
                 * marshaller.marshal(coordinationContext, soapHeader);
                 */
                /*
                 * ok, here's the workaround -- marshall the object as a child
                 * of a dummy header, detach it and then insert it as a header
                 * element.
                 */
                ...
                // ok, now we need to locate the inserted node and set the
                // mustunderstand attribute
                Iterator<SOAPHeaderElement> iterator = soapHeader.examineAllHeaderElements();
                while (iterator.hasNext()) {
                    headerElement = iterator.next();
                    if (CoordinationConstants.WSCOOR_ELEMENT_COORDINATION_CONTEXT_QNAME
                            .equals(headerElement.getElementQName())) {
    ...
    /**
     * check for an arjuna instance identifier element embedded in the soap
     * message headesr and, if found, use it to label an arjuna context attached
    ...
    private boolean handlemessageInbound(SOAPMessageContext context) throws ProtocolException {
        try {
            ...
            while (iterator.hasNext()) {
                ...
                if (CoordinationConstants.WSCOOR_ELEMENT_COORDINATION_CONTEXT_QNAME
                        .equals(headerElement.getElementQName())) {
                    // found it - clear the must understand flag, retrieve the
                    // value and store an arjuna
                    // context in the message context
                    headerElement.setMustUnderstand(false);
                    final JAXBContext jaxbCtx = getJaxbContext();
                    final JAXBElement<CoordinationContextType> elt = jaxbCtx.createUnmarshaller()
                            .unmarshal(headerElement, CoordinationContextType.class);
     ...
     */
    private static Set<QName> headers = Collections
            .singleton(CoordinationConstants.WSCOOR_ELEMENT_COORDINATION_CONTEXT_QNAME);
