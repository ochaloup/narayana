...
public class CoordinatorCompletionParticipantEngine implements CoordinatorCompletionParticipantInboundEvents {
    ...
    /**
     * the time which will elapse before the next message resend. this is
     * incrementally increased until it reaches RESEND_PERIOD_MAX
     */
    ...
    /**
     * the maximum period we will allow between resends. n.b. the coordinator
     * uses the value returned by getTransportTimeout as the limit for how long
     * it waits for a response. however, we can still employ a max resend period
     * in excess of this value. if a message comes in after the coordinator has
     * given up it will catch it on the next retry.
     */
    ...
    /**
     * true if this participant's recovery details have been logged to disk
     * otherwise false
     */
    private boolean persisted;

    /**
     * true if the participant should send getstatus rather than resend a
     * completed message
     */
     ...
     *            The initial state.
     * @param recovered
     *            true if the engine has been recovered from th elog otherwise
     *            false
     */
    public CoordinatorCompletionParticipantEngine(final String id, final W3CEndpointReference coordinator,
        ...
        // we always check the status of a recovered participant and we always
        // start off sending completed
     ...
     *            The arjuna context.
     *
     *            Active -> Canceling Canceling -> Canceling Completing ->
     *            Canceling Completed -> Completed (resend Completed) Closing ->
     *            Closing Compensating -> Compensating Failing-Active ->
     *            Failing-Active (resend Fail) Failing-Canceling ->
     *            Failing-Canceling (resend Fail) Failing-Completing ->
     *            Failing-Completing (resend Fail) Failing-Compensating ->
     ...
     *            (invalid state) Completing -> Completing (invalid state)
     *            Completed -> Closing Closing -> Closing Compensating ->
     *            Compensating (invalid state) Failing-Active -> Failing-Active
     *            (invalid state) Failing-Canceling -> Failing-Canceling
     *            (invalid state) Failing-Completing -> Failing-Completing
     *            (invalid state) Failing-Compensating -> Failing-Compensating
     *            (invalid state) NotCompleting -> NotCompleting (invalid state)
     *            Exiting -> Exiting (invalid state) Ended -> Ended (send
     *            Closed)
     */
     ...
     *            Compensating -> Compensating Failing-Active -> Failing-Active
     *            (invalid state) Failing-Canceling -> Failing-Canceling
     *            (invalid state) Failing-Completing -> Failing-Completing
     *            (invalid state) Failing-Compensating -> Failing-Compensating
     *            (resend Fail) NotCompleting -> NotCompleting (invalid state)
     *            Exiting -> Exiting (invalid state) Ended -> Ended (send
     *            Compensated)
     */
     ...
     *            The arjuna context.
     *
     *            Active -> Completing Canceling -> Canceling Completing ->
     *            Completing Completed -> Completed (resend Completed) Closing
     *            -> Closing Compensating -> Compensating Failing-Active ->
     *            Failing-Active (resend Fail) Failing-Canceling ->
     *            Failing-Canceling (resend Fail) Failing-Completing ->
     *            Failing-Completing (resend Fail) Failing-Compensating ->
     ...
     *            Active -> Active (invalid state) Canceling -> Canceling
     *            (invalid state) Completing -> Completing (invalid state)
     *            Completed -> Completed (invalid state) Closing -> Closing
     ...
     *            Completed -> Completed (invalid state) Closing -> Closing
     *            (invalid state) Compensating -> Compensating (invalid state)
     *            Failing-Active -> Ended Failing-Canceling -> Ended
     *            Failing-Completing -> Ended Failing-Compensating -> Ended
     *            NotCompleting -> NotCompleting (invalid state) Exiting ->
    ...
    public void failed(final NotificationType failed, final MAP map, final ArjunaContext arjunaContext) {
        ...
        if (deleteRequired) {
            if (!XTSBARecoveryManager.getRecoveryManager().deleteParticipantRecoveryRecord(id)) {
                // hmm, could not delete entry -- nothing more we can do than
                // log a message
                WSTLogger.i18NLogger.warn_wst11_messaging_engines_CoordinatorCompletionParticipantEngine_failed_1(id);
            }
        }
        // now we have removed the log record we can safely get rid of the
        // participant
        if ((current == State.STATE_FAILING_ACTIVE) || (current == State.STATE_FAILING_CANCELING)
                || (current == State.STATE_FAILING_COMPLETING) || (current == State.STATE_FAILING_COMPENSATING)) {
            ended();
     ...
     * 
     * @param notCompleted
     *            The not completed notification.
     * @param map
     *            The addressing context.
     * @param arjunaContext
     *            The arjuna context.
    ...
    public void status(final StatusType status, final MAP map, final ArjunaContext arjunaContext) {
        ...
        // revert to sending completed messages and reset the resend period to
        // the initial period
        checkStatus = false;
    ...
    public void soapFault(final SoapFault soapFault, final MAP map, final ArjunaContext arjunaContext) {
        ...
        try {
            boolean isInvalidState = soapFault.getSubcode()
                    .equals(CoordinationConstants.WSCOOR_ERROR_CODE_INVALID_STATE_QNAME);
            if (checkingStatus && isInvalidState) {
                // coordinator must have died before reaching close so just
                // cancel
                WSTLogger.i18NLogger
                        .warn_wst11_messaging_engines_CoordinatorCompletionParticipantEngine_soapFault_2(id);
                participant.compensate();
            } else {
                // hmm, something went wrong -- notify the participant of the
                // error
                WSTLogger.i18NLogger
                        .warn_wst11_messaging_engines_CoordinatorCompletionParticipantEngine_soapFault_3(id);
                participant.error();
            }
        ...
        if (deleteRequired) {
            if (!XTSBARecoveryManager.getRecoveryManager().deleteParticipantRecoveryRecord(id)) {
                ...
                // log a message
                WSTLogger.i18NLogger
                        .warn_wst11_messaging_engines_CoordinatorCompletionParticipantEngine_soapFault_1(id);
            }
        }
    }
     ...
     *
     * Active -> Active (invalid state) Canceling -> Canceling (invalid state)
     * Completing -> Completed Completed -> Completed Closing -> Closing
     * (invalid state) Compensating -> Compensating (invalid state)
     * Failing-Active -> Failing-Active (invalid state) Failing-Canceling ->
     * Failing-Canceling (invalid state) Failing-Completing ->
     * Failing-Completing (invalid state) Failing-Compensating ->
     * Failing-Compensating (invalid state) Exiting -> Exiting (invalid state)
     * NotCompleting -> NotCompleting (invalid state) Ended -> Ended (invalid
     * state)
     */
    public State completed() {
        // TODO -- check. not sure this can or should ever be called for a
        ...
        if (current == State.STATE_COMPLETING) {
            // ok we need to write the participant details to disk because it
            // has just completed
            BAParticipantRecoveryRecord recoveryRecord = new BAParticipantRecoveryRecord(id, participant, false,
                    coordinator);
            ...
            if (!XTSBARecoveryManager.getRecoveryManager().writeParticipantRecoveryRecord(recoveryRecord)) {
                ...
                WSTLogger.i18NLogger
                        .warn_wst11_messaging_engines_ParticipantCompletionParticipantEngine_completed_1(id);
                // we need to fail this transaction
                failRequired = true;
            }
        }
        // recheck state before we decide whether we need to fail -- we might
        // have been sent a cancel while
        // writing the log
        ...
        synchronized (this) {
            ...
            if (current == State.STATE_COMPLETING) {
                if (!failRequired) {
                    ...
                    // record the fact that we have persisted this object so
                    // later operations will delete
                    // the log record
                    persisted = true;
                    // if necessary notify the client now. n.b. this has to be
                    // done synchronized because
                    // if we release the lock then a resent COMPLETE may result
                    // in a COMPLETED being
                    // sent back and we cannot allow that until after the
                    // confirm
                    if (confirm) {
                        ((ConfirmCompletedParticipant) participant).confirmCompleted(true);
                    }
                } else {
                    // we must force a fail but we don't have a log record to
                    // delete
                    changeState(State.STATE_FAILING_COMPLETING);
                }
            } else {
                // we need to delete the log record here as the cancel would not
                // have known it was persisted
                deleteRequired = true;
            }
        }

        // check to see if we need to send a fail or delete the log record
        // before going ahead to complete
        ...
        if (failRequired) {
            ...
            if (!XTSBARecoveryManager.getRecoveryManager().deleteParticipantRecoveryRecord(id)) {
                ...
                WSTLogger.i18NLogger
                        .warn_wst11_messaging_engines_ParticipantCompletionParticipantEngine_completed_2(id);
            }
            // we can safely do this now
            if (confirm) {
                ((ConfirmCompletedParticipant) participant).confirmCompleted(false);
     ...
     *
     * Active -> Exiting Canceling -> Canceling (invalid state) Completing ->
     * Exiting Completed -> Completed (invalid state) Closing -> Closing
     * (invalid state) Compensating -> Compensating (invalid state)
     * Failing-Active -> Failing-Active (invalid state) Failing-Canceling ->
     * Failing-Canceling (invalid state) Failing-Completing ->
     * Failing-Completing (invalid state) Failing-Compensating ->
     * Failing-Compensating (invalid state) Exiting -> Exiting NotCompleting ->
     * NotCompleting (invalid state) Ended -> Ended (invalid state)
     */
    public State exit() {
        final State current;
        synchronized (this) {
     ...
     *
     * Active -> Failing-Active Canceling -> Failing-Canceling Completing ->
     * Failing-Completing Completed -> Completed (invalid state) Closing ->
     * Closing (invalid state) Compensating -> Failing-Compensating
     * Failing-Active -> Failing-Active Failing-Canceling -> Failing-Canceling
     * Failing-Completing -> Failing-Completing Failing-Compensating ->
     ...
     *
     * Active -> NotCompleting Canceling -> Canceling (invalid state) Completing
     * -> NotCompleting Completed -> Completed (invalid state) Closing ->
     * Closing (invalid state) Compensating -> Compensating (invalid state)
     * Failing-Active -> Failing-Active (invalid state) Failing-Canceling ->
     * Failing-Canceling (invalid state) Failing-Completing ->
     * Failing-Completing (invalid state) Failing-Compensating ->
     ...
     *
     * @param timedOut
     *            true if this is in response to a comms timeout
     */
    private void sendCompleted(boolean timedOut) {
        final MAP map = createContext();
        try {
            // if we are trying to reestablish the participant state then send
            // getStatus otherwise send completed
        ...
        // if we timed out the increase the resend period otherwise make sure it
        // is reset to the
    ...
    private synchronized void updateResendPeriod(boolean timedOut) {
        // if we timed out then we multiply the resend period by ~= sqrt(2) up
        // to the maximum
        // if not we make sure it is reset to the initial period

        if (timedOut) {
            if (resendPeriod < maxResendPeriod) {
                long newPeriod = resendPeriod * 14 / 10; // approximately
                                                            // doubles every two
                                                            // resends
                ...
                // ok, we hit our maximum period last time -- this time switch
                // to sending getStatus
                checkStatus = true;
            }
        } else {
            ...
            // if we were previously checking status we need to revert to
            // sending Completed
            if (checkStatus) {
                checkStatus = false;
            }
    ...
    private void sendStatus(final State state) {
        final MAP map = createContext();
        try {
            CoordinatorCompletionCoordinatorClient.getClient().sendStatus(coordinator, map, instanceIdentifier,
                    state.getValue());
        } catch (final Throwable th) {
            if (WSTLogger.logger.isTraceEnabled()) {
    ...
    private void executeCancel(boolean duringComplete) {
        ...
        // the state diagrams in the spec say that if a cancel comes in while
        // completing we have to cancel
        // but the participant may be part way through executing a complete.
        // strictly, that's something
        ...
        try {
             ...
             * problem is that coming from state COMPLETING we don't know
             * whether the completing thread has logged its state or logged it
             * and then deleted it because it saw the transition to CANCELING.
             * so we roll back to ACTIVE but fail if we have come from
             * COMPLETING
             */
            synchronized (this) {
    ...
    private void executeClose() {
        ...
        if (persisted) {
            // if we cannot delete the participant record we effectively drop
            // the close message
            // here in the hope that we have better luck next time..
            if (!XTSBARecoveryManager.getRecoveryManager().deleteParticipantRecoveryRecord(id)) {
                // hmm, could not delete entry -- leave it so we can maybe retry
                // later
                WSTLogger.i18NLogger
                        .warn_wst11_messaging_engines_ParticipantCompletionParticipantEngine_executeClose_2(id);
                // restore previous state so we can retry the close otherwise we
                // get stuck in state closing forever
    ...
    private void executeCompensate() {
        ...
        synchronized (this) {
            current = state;
            // need to do this while synchronized so no fail calls can get in on
            // between

            if (current == State.STATE_COMPENSATING) {
                if (persisted) {
                    if (!XTSBARecoveryManager.getRecoveryManager().deleteParticipantRecoveryRecord(id)) {
                        // we have to fail since we don't want to run the
                        // compensate method again
                        WSTLogger.i18NLogger
                                .warn_wst11_messaging_engines_ParticipantCompletionParticipantEngine_executeCompensate_3(
                                        id);
                        failRequired = true;
                        changeState(State.STATE_FAILING_COMPENSATING);
                    }
                }
                // if we did not fail then we can decommission the participant
                // now avoiding any further races
                // we will send the compensate after we exit the synchronized
                // block
                if (!failRequired) {
                    ended();
                }
    ...
    private void executeComplete() {
        ...
        if (current == State.STATE_COMPLETING) {
            ...
            if (!XTSBARecoveryManager.getRecoveryManager().writeParticipantRecoveryRecord(recoveryRecord)) {
                // hmm, could not write entry log warning
                WSTLogger.i18NLogger
                        .warn_wst11_messaging_engines_CoordinatorCompletionParticipantEngine_executeComplete_2(id);
                // we need to fail this transaction
                failRequired = true;
            }
        }
