...
public class CoordinatorCompletionCoordinatorEngine implements CoordinatorCompletionCoordinatorInboundEvents {
     ...
     *            The arjuna context.
     *
     *            Active -> Active (invalid state) Canceling-Active -> Ended
     *            Canceling-Completing -> Ended Completing -> Completing
     ...
     *            The arjuna context.
     *
     *            Active -> Active (invalid state) Canceling-Active ->
     *            Canceling-Active (invalid state) Canceling-Completing ->
     *            Canceling-Completing (invalid state) Completing -> Completing
     *            (invalid state) Completed -> Completed (invalid state) Closing
     *            -> Ended Compensating -> Compensating (invalid state)
     ...
     *            Canceling-Completing (invalid state) Completing -> Completing
     *            (invalid state) Completed -> Completed (invalid state) Closing
     *            -> Closing (invalid state) Compensating -> Ended
     *            Failing-Active -> Failing-Active (invalid state)
     *            Failing-Canceling -> Failing-Canceling (invalid state)
     *            Failing-Completing -> Failing-Completing (invalid state)
     *            Failing-Compensating -> Failing-Compensating (invalid state)
     ...
     *            The arjuna context.
     *
     *            Active -> Exiting Canceling -> Exiting Canceling-Active ->
     *            Exiting Canceling-Completing -> Exiting Completing -> Exiting
     *            Completed -> Completed (invalid state) Closing -> Closing
     *            (invalid state) Compensating -> Compensating (invalid state)
     *            Faulting -> Faulting (invalid state) Faulting-Active ->
     *            Faulting (invalid state) Faulting-Compensating -> Faulting
     *            (invalid state) Exiting -> Exiting Ended -> Ended (resend
     *            Exited)
     */
     ...
     *            The arjuna context.
     *
     *            Active -> Failing-Active Canceling-Active -> Failing-Canceling
     *            Canceling-Completing -> Failing-Canceling Completing ->
     ...
     *            Failing-Completing Failing-Compensating ->
     *            Failing-Compensating NotCompleting -> NotCompleting (invalid
     ...
     *            The arjuna context.
     *
     *            Active -> NotComleting Canceling-Active -> NotCompleting
     *            Canceling-Completing -> NotCompleting Completing ->
     ...
     *            Failing-Completing -> Failing-Completing (invalid state)
     *            Failing-Compensating -> Failing-Compensating (invalid state)
     *            NotCompleting -> NotCompleting Exiting -> Exiting (invalid
     *            state) Ended -> Ended (resend NotCompleted)
     */
    public void cannotComplete(final NotificationType cannotComplete, final MAP map,
        ...
        synchronized (this) {
            ...
            if ((current == State.STATE_ACTIVE) || (current == State.STATE_CANCELING_ACTIVE)
                ...
                // ending now avoids a race condition similar to the one for a
                // fail message
                // we set a failure state which ensures that any waiting
                // coordinator detects
        ...
        if ((current == State.STATE_ACTIVE) || (current == State.STATE_CANCELING_ACTIVE)
                || (current == State.STATE_CANCELING_COMPLETING)) {
            // we need to make sure the coordinator marks the participant as not
            // completing
     ...
     * @return The state.
     *
     *         Active -> Canceling-Active Canceling-Active -> Canceling-Active
     *         Canceling-Completing -> Canceling-Completing Completing ->
     *         Canceling-Completing Completed -> Completed (invalid state)
     ...
     *         NotCompleting (invalid state) Exiting -> Exiting (invalid state)
     ...
     * @return The state.
     *
     *         Active -> Completing Canceling-Active -> Canceling-Active
     *         (invalid state) Canceling-Completing -> Canceling-Completing
     *         (invalid state) Completing -> Completing Completed -> Completed
     *         (invalid state) Closing -> Closing (invalid state) Compensating
     *         -> Compensating (invalid state) Failing-Active -> Failing-Active
     *         (invalid state) Failing-Canceling -> Failing-Canceling (invalid
     *         state) Failing-Completing -> Failing-Completing (invalid state)
     *         Failing-Compensating -> Failing-Compensating (invalid state)
     *         NotCompleting -> NotCompleting (invalid state) Exiting -> Exiting
     *         (invalid state) Ended -> Ended (invalid state)
     */
    ...
    private void sendStatus(final State state) {
        final MAP map = createContext();
        try {
            CoordinatorCompletionParticipantClient.getClient().sendStatus(participant, map, instanceIdentifier,
                    state.getValue());
        } catch (final Throwable th) {
            if (WSTLogger.logger.isTraceEnabled()) {
        ...
        try {
            final SoapFault11 soapFault = new SoapFault11(SoapFaultType.FAULT_SENDER,
                    CoordinationConstants.WSCOOR_ERROR_CODE_INVALID_STATE_QNAME, WSTLogger.i18NLogger
                            .get_wst11_messaging_engines_CoordinatorCompletionCoordinatorEngine_sendInvalidStateFault_2());
            CoordinatorCompletionParticipantClient.getClient().sendSoapFault(soapFault, participant, map,
                    getFaultAction());
        } catch (final Throwable th) {
            if (WSTLogger.logger.isTraceEnabled()) {
