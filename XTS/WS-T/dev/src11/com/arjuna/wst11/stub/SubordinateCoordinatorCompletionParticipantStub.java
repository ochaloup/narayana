...
/**
 * A coordinator completion participant registered on behalf of an interposed
 * WS-BA coordinator in order to ensure that durable participants in the
 * interposed transaction are completed, closed or cancelled when requested from
...
public class SubordinateCoordinatorCompletionParticipantStub
    ...
    /**
     * set the participant manager for this stub. this cannot be supplied to the
     * constructor because it refers to the engine which drives this participant
     * and the engine can only be created once the participant has been created.
     * it is needed in order to be able to send a fail upward to the parent
     * coordinator during complete processing. it is not needed during close or
     * compensate processing which is ok because we cannot save and restore it
     * anyway.
     *
     * @param manager
    ...
    public void complete() throws WrongStateException, SystemException {
        if (!recovered) {
            ...
            // if status is COMMITTING then we return allowing the participant
            // to be logged
            // if status is ABORTED then the participant must fail avoiding any
            // logging

            if (coordinator.status() == ActionStatus.ABORTED) {
                manager.fail(BusinessActivityConstants.WSBA_ELEMENT_FAIL_QNAME);
            } else {
                // null out the manager so we don't attempt to save it to the
                // log
                manager = null;
            }
        } else {
            ...
            if (coordinator == null) {
                ...
                if (status == ActionStatus.PREPARED || status == ActionStatus.COMMITTING) {
                    ...
                    try {
                        ...
                        // throw an exception causing the commit to be retried
                        // later
                        throw new SystemException();
                    }
                ...
                // TODO -- check if this is really necessary given the catch
                // above?
    ...
    public void compensate() throws FaultedException, WrongStateException, SystemException {
        if (!recovered) {
            int result = coordinator.cancel();
            // test result and throw a SystemException if the compensate was
            // delayed
        } else {
            XTSBARecoveryManager recoveryManager = null;
            boolean isRecoveryScanStarted = false;
    ...
    /*
     * this participant implements the PersistableBAParticipant interface so it
