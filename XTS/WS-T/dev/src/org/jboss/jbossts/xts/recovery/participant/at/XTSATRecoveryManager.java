...
/**
 * Abstract class defining API for managing both paritcipant and coordinator
 * recovery On hteparticipant this is responsible for saving WS-AT participant
 * recovery records during prepare, deleting them at commit and recreating and
 * reactivating them during recovery. On the coordinator side it currently
 * merely records whether coordinator recovery has copleted its first scan
 */
public abstract class XTSATRecoveryManager {
     ...
     *            the instance to use as the recovery manager
     * @return the singleton previously in use or null if it was not previously
     *         set
     */
    ...
    /*****************************************************************************************/
    /*
     * API used by application client code to register XTS recovery modules at
     * application
     */
     ...
     * to recreate a WS-AT durable participant from the participant's recovery
     ...
     * as a helper to recretae WS-AT durable participants
     * 
     * @param module
    ...
    /*****************************************************************************************/
    /*
     * API used by participant service to create and delete participant recovery
     * records in
     */
    ...
    /*****************************************************************************************/
    /*
     * API used by recovery scanning thread to add entries to the AT recovery
     * manager's
     */
     ...
     *            store
     * @param participantRecoveryRecord
     *            the in-memory represenattion of the recovery record saved to
     *            disk
     */
    ...
    /**
     * test whether the first AT participant recovery scan has completed. this
    ...
    /**
     * test whether the first AT coordinator recovery scan has completed. this
     * indicates whether there may or may not still be unknown AT transcation
    ...
    /**
     * test whether the first AT subordinate coordinator recovery scan has
     * completed. this indicates whether there may or may not still be unknown
     * AT subtransaction records on disk. If the first scan has not yet
     * completed then a commit for an unknown subtransaction must raise an
     * exception delaying commit of the parent transaction.
     */
    public abstract boolean isSubordinateCoordinatorRecoveryStarted();

    /**
     * record the fact that the first AT coordinator recovery scan has
     * completed.
     */
    ...
    /**
     * record the fact that the first AT subordinate coordinator recovery scan
     * has completed.
     */
