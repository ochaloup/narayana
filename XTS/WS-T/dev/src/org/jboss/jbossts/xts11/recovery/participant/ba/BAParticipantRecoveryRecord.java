...
/**
 * recovery record specific to WS-AT 1.1 protocol participants. this implements
 * the behaviours necessary to save and restore a 1.1 participant to or from the
 * TX object store
 */
public class BAParticipantRecoveryRecord
        extends
            org.jboss.jbossts.xts.recovery.participant.ba.BAParticipantRecoveryRecord {

    /**
     * constructor used during prepare processing to create an AT 1.1
     * participant record for saving to the object store
     * 
     * @param id
     *            the id of the application-specific participant
     * @param participant
     *            the application-specific participant
     * @param
     */
    public BAParticipantRecoveryRecord(String id, BusinessAgreementWithParticipantCompletionParticipant participant,
            boolean isParticipantCompletion, W3CEndpointReference endpoint) {
        super(id, participant, isParticipantCompletion);
        this.endpoint = endpoint;
    }

    /**
     * constructor used during recovery processing to create a record whose
     * contents will be recovered from the object store
     */
    ...
    public void activate() {
        if (isParticipantCompletion) {
            ParticipantCompletionParticipantEngine engine = new ParticipantCompletionParticipantEngine(id, endpoint,
                    participant, State.STATE_COMPLETED, true);
            ...
            CoordinatorCompletionParticipantEngine engine = new CoordinatorCompletionParticipantEngine(id, endpoint,
                    coordinatorCompletionParticipant, State.STATE_COMPLETED, true);
