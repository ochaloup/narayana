...
/**
 * When a Business Activity participant registers with a BA transaction it gets
 * a reference to the transaction coordinator that lets it callback into the
 * transaction and drive the state-transition.
 *
...
public interface BAParticipantManager {
    ...
    /**
     * The participant has exited the business activity. The participant uses
     * this to inform the coordinator that is has left the activity. It will not
     * be informed when (and how) the business activity terminates.
     */

    public void exit() throws WrongStateException, UnknownTransactionException, SystemException;

    /**
     * The participant has completed it works, but wishes to continue in the
     * business activity, so that it will eventually be told when (and how) the
     * activity terminates. The participant may later be asked to compensate for
