...
public class CoordinatorCompletionCoordinatorProcessorImpl extends CoordinatorCompletionCoordinatorProcessor {
     ...
     *            The identifier.
     */
    public void activateCoordinator(final CoordinatorCompletionCoordinatorInboundEvents coordinator,
     ...
     * 
     * @param fail
     *            The fail exceptionnotification.
     * @param map
     *            The addressing context.
     * @param arjunaContext
     *            The arjuna context.
    ...
    public void cannotComplete(final NotificationType cannotComplete, final MAP map,
        ...
        if (coordinator != null) {
            ...
            if (WSTLogger.logger.isTraceEnabled()) {
                WSTLogger.logger.tracev("cannotComplete called on unknown coordinator: {0}",
                        new Object[]{instanceIdentifier});
            }
            ...
            try {
                final SoapFault11 soapFault = new SoapFault11(SoapFaultType.FAULT_SENDER,
                        CoordinationConstants.WSCOOR_ERROR_CODE_INVALID_STATE_QNAME, WSTLogger.i18NLogger
                                .get_wst11_messaging_CoordinatorCompletionCoordinatorProcessorImpl_getStatus_4());
                CoordinatorCompletionParticipantClient.getClient().sendSoapFault(soapFault, null, faultMAP,
                        getFaultAction());
            } catch (final Throwable th) {
                WSTLogger.i18NLogger.info_wst11_messaging_CoordinatorCompletionCoordinatorProcessorImpl_getStatus_3(
    ...
    private void sendExited(final MAP map, final ArjunaContext arjunaContext) {
        ...
        try {
            // supply a null endpoint indicating that the port should be
            // configured from the addressing properties!
            CoordinatorCompletionParticipantClient.getClient().sendExited(null, responseAddressingContext,
    ...
    private void sendFailed(final MAP map, final ArjunaContext arjunaContext) {
        ...
        try {
            ...
            // deliver message
            CoordinatorCompletionParticipantClient.getClient().sendFailed(null, responseAddressingContext,
                    arjunaContext.getInstanceIdentifier());
        } catch (final Throwable th) {
            if (WSTLogger.logger.isTraceEnabled()) {
    ...
    private void sendNotCompleted(final MAP map, final ArjunaContext arjunaContext) {
        ...
        try {
            // supply null endpoint so that addressing properties are used to
            // deliver message
            CoordinatorCompletionParticipantClient.getClient().sendNotCompleted(null, responseAddressingContext,
                    arjunaContext.getInstanceIdentifier());;
        } catch (final Throwable th) {
            if (WSTLogger.logger.isTraceEnabled()) {
