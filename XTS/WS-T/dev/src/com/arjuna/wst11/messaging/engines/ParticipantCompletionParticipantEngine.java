...
public class ParticipantCompletionParticipantEngine implements ParticipantCompletionParticipantInboundEvents {
    ...
    public void failed(final NotificationType failed, final MAP map, final ArjunaContext arjunaContext) {
        ...
        // now the log record has been deleted we can safely end this
        // participant
        if ((current == State.STATE_FAILING_ACTIVE) || (current == State.STATE_FAILING_CANCELING)
                || (current == State.STATE_FAILING_COMPENSATING)) {
            ended();
    ...
    public void soapFault(final SoapFault soapFault, final MAP map, final ArjunaContext arjunaContext) {
        ...
        try {
            ...
            if (checkingStatus && isInvalidState) {
                ...
                WSTLogger.i18NLogger
                        .warn_wst11_messaging_engines_ParticipantCompletionParticipantEngine_soapFault_2(id);
                participant.compensate();
            } else {
                // hmm, something went wrong -- notify the participant of the
                // error
                WSTLogger.i18NLogger
                        .warn_wst11_messaging_engines_ParticipantCompletionParticipantEngine_soapFault_3(id);
                participant.error();
            }
        ...
        if (deleteRequired) {
            if (!XTSBARecoveryManager.getRecoveryManager().deleteParticipantRecoveryRecord(id)) {
                ...
                // log a message
                WSTLogger.i18NLogger
                        .warn_wst11_messaging_engines_ParticipantCompletionParticipantEngine_soapFault_1(id);
            }
        }
    }
    ...
    public State completed() {
        ...
        synchronized (this) {
            ...
            // we have to do this synchronized so that we don't try writing the
            // participant details twice

            if (current == State.STATE_ACTIVE) {
                // ok we need to write the participant details to disk because
                // it has just completed
                BAParticipantRecoveryRecord recoveryRecord = new BAParticipantRecoveryRecord(id, participant, true,
                        coordinator);
    ...
    private void sendStatus(final State state) {
        ...
        final MAP map = createContext();
        try {
            ParticipantCompletionCoordinatorClient.getClient().sendStatus(coordinator, map, instanceIdentifier,
                    state.getValue());
        } catch (final Throwable th) {
            if (WSTLogger.logger.isTraceEnabled()) {
    ...
    private void executeCancel() {
        ...
        try {
            ...
            /*
             * we only get here in from state ACTIVE so if we are stll in state
             * CANCELING then roll back the state allowing a retry of the cancel
             */
            synchronized (this) {
