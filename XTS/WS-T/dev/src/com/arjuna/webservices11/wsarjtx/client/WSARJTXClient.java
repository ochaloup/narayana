...
public class WSARJTXClient {
    ...
    // service?
    /**
     * thread local which maintains a per thread termination coordinator service
     * instance
     */
    ...
    /**
     * thread local which maintains a per thread termination participant service
     * instance
     */
    ...
    private static synchronized TerminationCoordinatorService getTerminationCoordinatorService() {
        if (terminationCoordinatorService.get() == null) {
            // terminationCoordinatorService.set(new
            // TerminationCoordinatorService(null, new
            // QName("http://schemas.arjuna.com/ws/2005/10/wsarjtx",
    ...
    private static synchronized TerminationParticipantService getTerminationParticipantService() {
        if (terminationParticipantService.get() == null) {
            // terminationParticipantService.set(new
            // TerminationParticipantService(null, new
            // QName("http://schemas.arjuna.com/ws/2005/10/wsarjtx",
    ...
    // we use this in situations where we don't have a proper endpoint but we do
    // have caller addressing properties
    ...
    public static TerminationCoordinatorRPCPortType getTerminationCoordinatorRPCPort(
        ...
        TerminationCoordinatorRPCPortType port = service.getPort(endpointReference,
                TerminationCoordinatorRPCPortType.class, new AddressingFeature(true, true));
        BindingProvider bindingProvider = (BindingProvider) port;
        /*
         * we no longer have to add the JaxWS WSAddressingClientHandler because
         * we can specify the WSAddressing feature List<Handler>
