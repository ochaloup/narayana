...
public class LockManager extends StateManager {
    ...
    public void finalize() throws Throwable {
        ...
        /*
         * terminate should have been called. Check and warn/do something about
                        ...
                        synchronized (syncObject) {
                            synchronized (locksHeldLockObject) {
                                if (loadState()) {
                                    ...
                                    if (locksHeld != null) {
                                        /*
                                         * scan through old list of held locks
                                        ...
                                        /*
                                         * Cannot create new locklist - abort
                                         * and try again.
                                         */
                    ...
                    synchronized (syncObject) {
                        synchronized (locksHeldLockObject) {
                            ...
                            if (conflict != ConflictType.CONFLICT) {
                                /*
                                 * When here the conflict was resolved or the
                                 * retry limit expired.
                                 */
                            ...
                            /*
                             * Unload internal state into lock store only if
                             * lock list was modified if this fails claim the
                             * setlock failed. If we are using the lock daemon
                             * we can arbitrarily throw the lock away as the
                             * daemon has it.
                             */
                            ...
                            /*
                             * Postpone call on modified to here so that
                             * semaphore will have been released. This means
                             * when modified invokes save_state that routine may
                             * set another lock without blocking.
                             */
                            ...
                            /*
                             * Make sure we free state while we still have the
                             * lock.
                             */
            ...
            synchronized (locksHeldLockObject) {
                do {
                    if (loadState()) {
                         ...
                         * owner (oneOrAll = ALL_LOCKS) or the uid of the actual
                         * lock itself (oneOrAll = SINGLE_LOCK).
                         */
                    ...
                    if (!result) {
                        ...
                        if (!loaded) {
                            ...
                            /*
                             * No need to freeState since we will have done that
                             * by now.
                             */
                        } else {
                            if (!deleted) {
    ...
    protected final boolean loadState() {
        ...
        if (super.objectModel == ObjectModel.SINGLE) {
            ...
            try {
                ...
                if (S != null) {
                    Uid u = null; /*
                                     * avoid system calls in Uid creation
                                     */
                    Lock current = null;
                    int count = 0;
    ...
    protected final boolean unloadState() {
        ...
        if (super.objectModel == ObjectModel.SINGLE) {
            ...
            if (txojLogger.logger.isTraceEnabled()) {
                txojLogger.logger.trace("LockManager::unloadState() unloading " + lockCount + " lock(s)");
            }
