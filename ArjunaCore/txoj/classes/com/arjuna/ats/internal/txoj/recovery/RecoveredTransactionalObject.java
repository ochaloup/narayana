...
public class RecoveredTransactionalObject extends StateManager {
    ...
    protected final void replayPhase2() {
        if (findHoldingTransaction()) {
             ...
             * eventually.
             */
            if ((tranStatus == ActionStatus.PREPARED) || (tranStatus == ActionStatus.COMMITTING)
                    || (tranStatus == ActionStatus.COMMITTED) || (tranStatus == ActionStatus.H_COMMIT)
                    || (tranStatus == ActionStatus.H_MIXED) || (tranStatus == ActionStatus.H_HAZARD)) {
                commit();
            } else if ((tranStatus == ActionStatus.ABORTED) || (tranStatus == ActionStatus.H_ROLLBACK)
                    || (tranStatus == ActionStatus.ABORTING) || (tranStatus == ActionStatus.ABORT_ONLY)) {
                rollback();
            } else {
