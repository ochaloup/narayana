...
public class TxControl {
    ...
    public static class Shutdown extends Thread {
        public void run() {
            ...
            // synchronize on the class since the shutdown method on TxControl is static
            // synchronized
     ...
     * this value will be used. A value of 0 means that the transaction will never
     * time out.
     */
    ...
    /**
     * Set the timeout to be associated with a newly created transaction if there is
     * no other timeout to be used.
     *
     * @param timeout
     */
    ...
    /**
     * Start the transaction system. This allows new transactions to be created and
     * for recovery to execute.
     */
    ...
    /**
     * Stop the transaction system. New transactions will be prevented but recovery
    ...
    /**
     * Stop the transaction system. New transactions will be prevented and recovery
     * will cease.
     *
     * WARNING: make sure you know what you are doing when you call this routine!
     */

    public static final synchronized void disable(boolean disableRecovery) {
        /*
         * We could have an implementation that did not return until all transactions
         * had finished. However, this could take an arbitrary time, especially if
         * participants could fail. Since this information is available anyway to the
         * application, let it handle it.
         */
    ...
    /**
     * @return the <code>ObjectStore</code> implementation which the transaction
    ...
    static final boolean asyncBeforeSynch = arjPropertyManager.getCoordinatorEnvironmentBean()
            .isAsyncBeforeSynchronization();

    static final boolean asyncAfterSynch = arjPropertyManager.getCoordinatorEnvironmentBean()
    ...
    static final boolean readonlyOptimisation = arjPropertyManager.getCoordinatorEnvironmentBean()
            .isReadonlyOptimisation();

    static final boolean dynamic1PC = arjPropertyManager.getCoordinatorEnvironmentBean().getDynamic1PC();

    /**
     * flag which is true if transaction service is enabled and false if it is
     * disabled
     */
    ...
    /**
     * flag which is true if enable and disable operations, respectively, start and
     * stop the transaction status manager and false if they do not perform a start
     * and stop. this flag is true by default and can only be set to false by
     * setting
     * property @see#com.arjuna.ats.arjuna.common.TRANSACTION_STATUS_MANAGER_ENABLE
     * to value "NO"
     */
    static final boolean _enableTSM = arjPropertyManager.getCoordinatorEnvironmentBean()
            .isTransactionStatusManagerEnable();

    static final boolean beforeCompletionWhenRollbackOnly = arjPropertyManager.getCoordinatorEnvironmentBean()
     ...
     */
    static {
        // TODO -- add this check to respect the environment setting for
        // Environment.START_DISABLED?
        // TODO -- is this feature actually needed (it appears not to be used
        // internally)
        // if (enable) {
        createTransactionStatusManager();
        // }
