...
public class RecordList {
     ...
     * algorithm. Starting at the record indicated by 'startat' examine each entry
     * in the list in turn and perform the following code 1) If the new record
     * should be merged with the old, call nr.merge passing the old record as an
     * argument and then INSERT the new record IN PLACE OF the old and exit 2) If
     * the new record should replace the old then INSERT the new record IN PLACE OF
     * the old and exit 3) If the new record should be added in addition to the old
     * then INSERT the new record BEFORE the old and exit 4) If the two records are
     * the same (determined by the == operator) simply exit 5) Otherwise determine
     * if the new record should be added here regardless due to the ordering
     * constraints on the list and if so add and exit, otherwise step on to the next
     * element and repeat all the steps. Steps 1-4 effectively ensure that
     * information maintained in any two records for the same object is current
     * either by merging in new information, replacing the old with new, adding in
     * new, or leaving the old alone. Step 5 ensures that if no existing record
     * exists insertion takes place at the correct point
     *
