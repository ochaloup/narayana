...
/**
 * The outcomes which can be generated when a transaction attempts to
...
public class TwoPhaseOutcome {
    ...
    public static final int PREPARE_NOTOK = 1; // did not prepare so force roll
                                                // back
    public static final int PREPARE_READONLY = 2; // only used to read the
                                                    // state, so no need for
                                                    // second phase
    public static final int HEURISTIC_ROLLBACK = 3; // after prepare decided to
                                                    // roll back without waiting
                                                    // for coordinator
    public static final int HEURISTIC_COMMIT = 4; // after prepare decided to
                                                    // commit without waiting
                                                    // for coordinator
    public static final int HEURISTIC_MIXED = 5; // after prepare some
                                                    // sub-participants
                                                    // committed and some rolled
                                                    // back without waiting for
                                                    // coordinator
    public static final int HEURISTIC_HAZARD = 6; // after prepare some
                                                    // sub-participants
                                                    // committed, some rolled
                                                    // back and some we don't
                                                    // know
    public static final int FINISH_OK = 7; // the second phase completed ok
    public static final int FINISH_ERROR = 8; // there was a failure during the
                                                // second phase and we should
                                                // retry later (not necessarily
                                                // a heuristic)
    public static final int NOT_PREPARED = 9; // participant told to do second
                                                // phase operation when it
                                                // hadn't seen the first phase
    public static final int ONE_PHASE_ERROR = 10; // WARNING this has different
