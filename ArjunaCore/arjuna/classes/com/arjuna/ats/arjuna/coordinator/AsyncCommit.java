 ...
 * rollback as well as commit. The transaction will have been prepared by the
 * time an instance of this class is used.
 *
...
class AsyncCommit implements Runnable {
    public void run() {
        {
            if (_theAction != null) {
                /*
                 * This is a transient thread so we don't want to register it with the action it
                 * is committing/aborting, only change its notion of the current transaction so
                 * that any abstract records that need that information can still have it.
                 */
    ...
    protected synchronized boolean doPhase2Commit() {
        if (_theAction != null) {
            /*
             * Don't want heuristic information, otherwise would not be asynchronous.
             */
