...
public class TransactionTypeManager {
    ...
    public interface TransactionTypeMap {
        ...
        public String getType(); // the shorthand name (could be the same as
                                    // getRealType iff we want people to write
                                    // really loooong strings).

        public String getRealType(); // the real type (used by object store
                                        // operations)
    }
    ...
    /*
     * All log implementations that we want to support should be in here so they
     * are registered. We could use a dynamic approach, but we don't have a
     * rapidly growing number of object stores anyway to justify that overhead.
     */
