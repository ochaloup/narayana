...
//    public static final String SERVICE_URL = "service:jmx:rmi:///jndi/rmi://localhost:9999/server";
    ...
    /**
     * Construct a holder for Participant and Recovery Store proxies. There is one
     * instance for each connection to a JVM. In practice there will only ever be
     * one instance.
     *
     * @param serviceUrl the url for the MBean server to connect to. Use default to
     *                   connect to the local MBean Server
     * @param listener   optionally register a listener for notifications
     * @throws JMException if there are JMX errors during registration of MBeans and
    ...
    /**
     * Mechanism for JMX clients to remove listeners and to close the JMX connection
     * if the client create one
     *
     * @throws JMException if there are errors removing listeners
     * @throws IOException if there are errors removing listeners or closing the JMX
     *                     connection
     */
     ...
     * @throws IOException if the serviceUrl is invalid or if the connection cannot
     *                     be started
     */
     ...
     * release proxies to the object stores
     *
     * @param serviceUrl the service url of the MBean Server where the proxies are
     *                   located
     * @throws JMException if there are errors removing listeners
     * @throws IOException if there are errors removing listeners or closing the JMX
     *                     connection
     ...
     * Get a recovery store proxy from the local MBeanServer
     *
     * @param listener listener an optional notification listener (use null if one
     *                 is not required)
     * @return a proxy for the target RecoveryStore
     * @throws JMException if there are JMX errors during registration of MBeans and
     *                     notification listeners
     ...
     * @param listener   an optional notification listener (use null if one is not
     *                   required)
     * @return a proxy for the target RecoveryStore
     * @throws JMException if there are JMX errors during registration of MBeans and
     *                     notification listeners
