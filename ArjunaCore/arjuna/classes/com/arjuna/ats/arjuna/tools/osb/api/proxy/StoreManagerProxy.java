...
/**
 * Miscellaneous methods for obtaining remote proxies to the JBossTS Recovery
 * and Participant stores
 */
public class StoreManagerProxy {
    // class and bean names of MBeans representing the JBossTS recovery and
    // participant stores
    ...
    private static Map<String, StoreManagerProxy> proxies = Collections
            .synchronizedMap(new HashMap<String, StoreManagerProxy>());
    private static JMXConnectorServer jmxCServer;

    private MBeanServerConnection mbsc; // MBean server implementing the object
                                        // store MBeans
    ...
    private ObjectName recoveryStoreON; // object name of the recover store
                                        // MBean
    private ObjectName participantStoreON; // object name of the participant
                                            // store MBean
    private NotificationListener recoveryListener = null;
    private NotificationListener participantListener = null;

    /**
     * Construct a holder for Participant and Recovery Store proxies. There is
     * one instance for each connection to a JVM. In practice there will only
     * ever be one instance.
     *
     * @param serviceUrl
     *            the url for the MBean server to connect to. Use default to
     *            connect to the local MBean Server
     * @param listener
     *            optionally register a listener for notifications
     * @throws JMException
     *             if there are JMX errors during registration of MBeans and
    ...
    private StoreManagerProxy(String serviceUrl, NotificationListener listener) throws JMException, IOException {
        ...
        psProxy = new ParticipantStoreProxy(
                JMX.newMBeanProxy(mbsc, participantStoreON, ParticipantStoreBeanMBean.class, true));
    ...
    /**
     * Mechanism for JMX clients to remove listeners and to close the JMX
     * connection if the client create one
     * 
     * @throws JMException
     *             if there are errors removing listeners
     * @throws IOException
     *             if there are errors removing listeners or closing the JMX
     *             connection
     */
     ...
     *
     * @param serviceUrl
     *            the url on which the target MBean Server resides
     * @throws IOException
     *             if the serviceUrl is invalid or if the connection cannot be
     *             started
     */
    public static void startServerConnector(String serviceUrl) throws IOException {
        jmxCServer = JMXConnectorServerFactory.newJMXConnectorServer(new JMXServiceURL(serviceUrl), null,
     ...
     * @param register
     *            whether to register or unregister the MBean
    ...
    private static synchronized StoreManagerProxy getProxy(String serviceUrl, NotificationListener listener)
     ...
     *
     * @param serviceUrl
     *            the service url of the MBean Server where the proxies are
     *            located
     * @throws JMException
     *             if there are errors removing listeners
     * @throws IOException
     ...
     * @throws JMException
     *             if there are JMX errors during registration of MBeans
     * @throws IOException
     *             if there are errors on the connection to the MBean Server
     */
     ...
     * 
     * @param listener
     *            listener an optional notification listener (use null if one is
     *            not required)
     * @return a proxy for the target RecoveryStore
     * @throws JMException
     *             if there are JMX errors during registration of MBeans and
     ...
     *             if there are errors on the connection to the MBean Server
     */
    public static synchronized RecoveryStoreProxy getRecoveryStore(NotificationListener listener)
            throws IOException, JMException {
     ...
     * @param listener
     *            an optional notification listener (use null if one is not
     *            required)
     * @return a proxy for the target RecoveryStore
     * @throws JMException
     *             if there are JMX errors during registration of MBeans and
     ...
     *             if there are errors on the connection to the MBean Server
     */
    public static synchronized RecoveryStoreProxy getRecoveryStore(String serviceUrl, NotificationListener listener)
            throws IOException, JMException {
     ...
     *             if there are errors on the connection to the MBean Server
     */
    public static synchronized ParticipantStoreProxy getParticipantStore(NotificationListener listener)
            throws IOException, JMException {
     ...
     *             if there are errors on the connection to the MBean Server
     */
    public static synchronized ParticipantStoreProxy getParticipantStore(String serviceUrl,
            NotificationListener listener) throws IOException, JMException {
