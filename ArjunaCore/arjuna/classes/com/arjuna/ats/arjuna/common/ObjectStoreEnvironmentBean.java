...
public class ObjectStoreEnvironmentBean implements ObjectStoreEnvironmentBeanMBean {
    ...
    /**
     * Returns the maximum allowed size, in bytes, of the cache store's in-memory
     * cache.
     *
    ...
    /**
     * Sets the maximum number of removed items that may be held in the cache before
     * being purged.
     *
     * This property is used by the following object store implementations:
     * CacheStore.
    ...
    public String getObjectStoreDir() {
        if (Utility.isAndroid() && !androidDirCheck) {
            try {
                /*
                 * Use reflection so we can build this in an environment that does not have the
                 * various Android libraries available.
                 */
    ...
    /**
     * Returns true if ObjectStore operations should be synched to disk. Note that
     * this value may be overridden by store implementation specific configuration.
    ...
    /**
     * Returns the number of directories over which the ObjectStore contents will be
     * distributed. Splitting the contents is important for performance on some file
     * systems, as it reduces chain length (number of items in a directory) and
    ...
    /**
     * Sets if transaction log operations should be synched to disk or not. Caution:
     * Disabling this may be lead to non-ACID transaction behaviour.
     *
     * This property is used by the following object store implementations:
     * ActionStore, HashedActionStore, LogStore.
     ...
     * Default: false
     *
     * @return true if scan results should include zero length files, false if they
     *         should be excluded.
     */
     ...
     * NullActionStore, ShadowingStore, ShadowNoFileLockStore.
     *
     * @param scanZeroLengthFiles true to include zero length files in scan results,
    ...
    /**
     * Sets the maximum number of attempts which may be made to create a direcory
     * tree in the store.
     *
     * This property is used by the following object store implementations:
     * ActionStore, CacheStore, HashedActionStore, HashedStore, LogStore,
     ...
     * LogStore.
     *
     * @param synchronousRemoval true for synchronous operation, false for
     *                           asynchronous.
     */
    ...
    /**
     * Determine whether the ObjStoreBrowser should report basic information about
     * all logs. Because exposing log records has the side effect of activating the
     * default is to only activate ones for which we know the side effects are
     * harmless {@link com.arjuna.ats.arjuna.tools.osb.mbean.ObjStoreBrowser
     * ObjectStoreBrowser}
     *
     ...
     * JMX}
     *
     * @param exposeAllLogRecords Set to true to expose basic information about all
     *                            log records
     */
     ...
     *
     * @param ignoreMBeanHeuristics if false heuristic participants may only be
     *                              deleted after the heuristic has been cleared
     */
    ...
    /**
     *
     * @return whether or not MBean operations that delete a transaction will delete
     *         participants that are still in a heuristic state
     */
     ...
     * API methods
     *
     * @param volatileStoreSupportAllObjUids if true then add support for finding
     *                                       Uids by type in the volatile stores
     */
