...
public class TransactionStatusManager {
     ...
     *
     * this must be private as it should only be called once. otherwise we leak
     * listener threads
     */
    ...
    /*
     * Removes the TransactionStatusManager from the object store and closes
     ...
     * void finalize() { if ( ! _finalizeCalled ) { _finalizeCalled = true ;
     * 
     * _listener.stopListener() ; TransactionStatusManagerItem.removeThis(
    ...
    private void start(String serviceName, String host, int port) {
        try {
            ...
            if (recoveryPropertyManager.getRecoveryEnvironmentBean().getTransactionStatusManagerPort() == 0) {
                tsLogger.i18NLogger.info_recovery_TransactionStatusManager_3(
     ...
     * 
     * @param defValue
     *            the value to use if no valid port number can be found
    ...
    /**
     * Create a new listener socket. If the input paramters are invalid use the
     * config properties to choose the desired address and port to bind the
     * listener to. A port value of -1 is considered invalid.
     *
     * @param hostNameOverride
     *            override the config property for the hostname
     * @param portOverride
     *            override the config property for the port
    ...
    private ServerSocket getTsmServerSocket(String hostNameOverride, int portOverride) throws IOException {
        ...
        if (_port == -1) {
            ...
            throw new com.arjuna.ats.arjuna.exceptions.FatalError(
                    tsLogger.i18NLogger.get_recovery_TransactionStatusManager_13());
        }
    ...
    /**
     * Bound port for listener socket A value of -1 means that the attempt to
