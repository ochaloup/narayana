...
public class TransactionStatusManager {
    ...
    /*
     * Removes the TransactionStatusManager from the object store and closes down
     * the listener thread.
     *
     ...
     * proper shutdown method which closes the listener socket causing the thread to
     * shut down
     *
     * // TODO consider adding a shutdown operation (signature change) public void
     * finalize() { if ( ! _finalizeCalled ) { _finalizeCalled = true ;
     *
     ...
     * config properties to choose the desired address and port to bind the listener
     * to. A port value of -1 is considered invalid.
     *
    ...
    /**
     * Bound port for listener socket A value of -1 means that the attempt to create
     * the socket failed
     */
    private int _port = 0;
