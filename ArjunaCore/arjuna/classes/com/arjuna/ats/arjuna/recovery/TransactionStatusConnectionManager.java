...
public class TransactionStatusConnectionManager {
    ...
    /**
     * Obtain the transaction status for the specified transaction. At this point we
     * don't know the type of the transaction, only it's Uid. So, we're going to
     * have to search through the object store. This assumes that the transaction id
     * is present in the local object store. If it isn't, or there is a possibility
     * it may not be, then you should use the other variant of this method and
     * determine the type through another method.
     */
    ...
    public int getTransactionStatus(String transactionType, Uid tranUid) {
        ...
        // This skips the communication with TransactionStatusManager, which is just
        // backed
        // by ActionStatusService anyhow. That allows TSM to be turned off for local
        // only cases if desired.
        // Note: condition assumes ObjectStore is not shared between machines i.e. that
        // processId is globally uniq.
        if (!process_id.equals(_localUid.getHexPid())) {
            status = getRemoteTransactionStatus(process_id, transactionType, tranUid);
        }

        /*
         * Try to read status from disc locally if invalid status, as the tx may be
         * local or, if it is remote, the TransactionStatusManager may have died or
         * comms may have failed. Use an ActionStatusService instance as that's what the
         * remote recovery manager would have used, and it contains all of the logic to
         * find and map the state type.
         */
    ...
    /**
     * Use the TransactionStatusConnector to remotly query a transaction manager to
     * get the tx status.
     *
    ...
    /**
     * Examine the Object Store for any new TrasactionStatusManagerItem objects, and
