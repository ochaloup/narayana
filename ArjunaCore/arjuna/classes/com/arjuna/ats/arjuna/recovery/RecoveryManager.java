...
public class RecoveryManager {
    /**
     * In this mode the recovery manager runs periodically but may also be driven
     * through messages or via the scan operation if it is embedded.
     */
     ...
     * work if driven through messages or via the scan operation if it is embedded.
     */
     ...
     *                                  created in a different mode to that
     *                                  requested.
     *
     * @return the manager.
     */
    ...
    /**
     * Obtain a reference to the RecoveryManager singleton. If it hasn't been
     * created yet then it will be. The manager can be created in a management mode
    ...
    /**
     * Force a recovery scan now. This is a blocking operation and will only return
     * once the recovery scan has completed.
     *
    ...
    /**
     * Force a recovery scan now. This is a non-blocking operation and will return
     * immediately. Notification of completion of the scan is done through the
     * RecoveryScan object.
     *
     * @param callback callback The callback mechanism used to inform users that the
    ...
    /**
     * Terminate and cleanup the recovery manager. There is no going back from this.
     * This is a synchronous operation so return means that the recovery has
     * completed.
     *
     * @throws IllegalStateException if the recovery manager has been shutdown.
     */
    ...
    /**
     * Terminate and cleanup the recovery manager. There is no going back from this.
     * Can be called synchronous or asynchronously. If you have any intention of
     * restarting the recovery manager later then you MUST use the async=false
     * option.
     *
    ...
    /**
     * If the recovery manager has been shutdown previously then recreate it in the
     * same mode as before. Otherwise ignore.
     */
    ...
    /**
     * wait for the recovery thread to be shutdown. n.b. this will not return unless
     * and until shutdown is called.
     *
     * @throws IllegalStateException if the recovery manager has been shutdown.
     */
     ...
     *
     * @param async false means wait for the recovery manager to finish any scans
     *              before returning.
     * @throws IllegalStateException if the recovery manager has been shutdown.
     */
    ...
    /**
     * Indicates what mode (INDIRECT_MANAGEMENT or DIRECT_MANAGEMENT) the recovery
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            try {
                ...
                if (testMode) {
                    // in some test cases the recovery manager is killed and restarted in quick
                    // succession.
                    // sometimes the O/S does not free up the port fast enough, so we can't reclaim
                    // it on restart.
            ...
            // this is never going to return because it only returns when shutdown is called
            // and
            // there is nothing which is going to call shutdown. we probably aught to
            // provide a
