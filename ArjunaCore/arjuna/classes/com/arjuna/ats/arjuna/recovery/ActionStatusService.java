...
public class ActionStatusService implements Service {
    ...
    /**
     * Retrieve the transaction status for the specified Uid and if available
     * transaction type.
     */
    ...
    /**
     * Does the main work of reading in a uid and transaction type from the
     * recovery manager, retrieving the status of the transaction and sending it
     * back to the Recovery Manager.
     */
            ...
            if (rmStatus.equals("OK")) {
                for (;;) {
                    ...
                    try {
                        ...
                        // recovery manager has torn down connection, so end
                        // loop
                    }

                    /*
                     * check for null - in theory we get this from readLine when
                     * EOF has been reached, although in practice since we are
                     * reading from a socket we will probably get an IOException
                     * in which case we will still see null
                     */
    ...
    /**
     * Check for transaction status in the local hash table, if does not exist,
     * then retrieve the status from the Object Store.
     */
    private int getActionStatus(Uid tranUid, String transactionType) {
        ...
        try {
            ...
            if (basic_action != null) {
                ...
                /*
                 * If there is a persistent representation for this transaction,
    ...
    /**
     * Get transaction status for a transaction when the transactionType is
     * unknown.
     */
    ...
    private int getTranStatus(Uid tranUid) {
        ...
        try {
            ...
            if (basic_action != null) {
                ...
                /**
                 * Run through the object store and try and find the matching
                 * id.
                 */
    ...
    /**
     * Obtains the status for the specified transaction Uid when the transaction
     * type is unknown.
     */
        ...
        try {
            ...
            switch (osState) {
                ...
                case StateStatus.OS_UNKNOWN :
                    action_status = ActionStatus.ABORTED; // no state means
                                                            // aborted because
                                                            // of presumed abort
                                                            // rules
                    break;
