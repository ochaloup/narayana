...
public class ActionStatusService implements Service {
    ...
    /**
     * Does the main work of reading in a uid and transaction type from the recovery
     * manager, retrieving the status of the transaction and sending it back to the
     * Recovery Manager.
     */
        ...
        try {
            ...
            if (rmStatus.equals("OK")) {
                for (;;) {
                    ...
                    /*
                     * check for null - in theory we get this from readLine when EOF has been
                     * reached, although in practice since we are reading from a socket we will
                     * probably get an IOException in which case we will still see null
                     */
    ...
    /**
     * Check for transaction status in the local hash table, if does not exist, then
     * retrieve the status from the Object Store.
     */
    private int getActionStatus(Uid tranUid, String transactionType) {
        ...
        try {
            ...
            if (basic_action != null) {
                ...
                /*
                 * If there is a persistent representation for this transaction, then return
                 * that status.
                 */
                ...
                while (!endOfList) {
                    ...
                    if (theTypeName.compareTo("") == 0) {
                        ...
                        if (_recoveryStore.allObjUids(theTypeName, uids)) {
                            ...
                            return action_status; // Errors contacting recovery store for the list of uids it has for a
                                                  // type so return INVALID state
                        }
                    }
                }
            } else {
                return action_status; // Errors contacting recovery store for the list of types it holds so return
                                      // INVALID state
            }
        } catch (Exception ex) {
