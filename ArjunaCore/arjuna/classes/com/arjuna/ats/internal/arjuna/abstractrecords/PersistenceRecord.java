...
public class PersistenceRecord extends RecoveryRecord {
    ...
    /**
     * topLevelAbort may have to remove the persistent state that was written into
     * the object store during the processing of topLevelPrepare. It then does the
     * standard abort processing.
     */
    ...
    /**
     * topLevelPrepare attempts to save the object. It will either do this in the
     * action intention list or directly in the object store by using the
     * 'deactivate' function of the object depending upon the size of the state. To
     * ensure that objects are correctly hidden while they are in an uncommitted
     ...
     * the one saved in the intention list. If the store cannot cope with being
     * given an empty state we revert to the old protocol.
     */
    ...
    public int topLevelPrepare() {
        ...
        if ((sm != null) && (targetParticipantStore != null)) {
            /*
             * Get ready to create our state to be saved. At this stage we're not sure if
             * the state will go into its own log or be written into the transaction log for
            ...
            if (writeOptimisation && (!targetParticipantStore.fullCommitNeeded()
                ...
                /*
                 * We assume that crash recovery will always run before the object can be
                 * reactivated!
                 */
                ...
                if (PersistenceRecord.classicPrepare) {
                    ...
                    /*
                     * Write an empty shadow state to the store to indicate one exists, and to
                     * prevent bogus activation in the case where crash recovery hasn't run yet.
                     */
     ...
     * object store during the processing of topLevelPrepare intact. Crash recovery
     * will take care of its resolution
     */
    ...
    public boolean restore_state(InputObjectState os, int ot) {
        ...
        try {
            ...
            // objectstore config when records exist in the tx store is therefore a Bad
            // Thing.
    ...
    public boolean save_state(OutputObjectState os, int ot) {
        ...
        if (targetParticipantStore != null) {
            ...
            try {
                ...
                /*
                 * If we haven't written a shadow state, then pack the state into the
                 * transaction log. There MUST be a state at this point.
                 */
    ...
    /**
     * Creates a 'blank' persistence record. This is used during crash recovery when
    ...
    /**
     * Cadaver records force write shadows. This operation supresses to abbreviated
     * commit This should never return false
     */
