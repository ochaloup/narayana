...
public class VolatileStore extends ObjectStore {
     ...
     * @return the current state of the object's state (e.g., shadowed, committed
     *         ...)
     */
     ...
     *
     * We assume Uid is globally unique, so we don't store or key on type name at
     * all - it's only needed during recovery, which we obviously don't support.
     *
     * This implementation is intended to be memory efficient, as we also want to
     * use it on small footprint devices. Hence we map Uid to a byte array rather
     * than e.g. some more complex value class containing the byte[] state plus
     ...
     * small footprint installs. We therefore prefer ConcurrentHashMap, even though
     * it's not available in J2ME. Fork from previous version that used synchronized
     * Map if you want a build for small footprint environments.
     *
