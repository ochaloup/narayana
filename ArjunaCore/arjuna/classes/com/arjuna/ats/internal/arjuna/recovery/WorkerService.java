    ...
    public void doWork(InputStream is, OutputStream os) throws IOException {
        ...
        try {
            ...
            if (request.equals("PING")) {
                ...
                // hmm, we need to synchronize on the periodic recovery object
                // in order to wake it up via notify.
                // but the periodic recovery object has to synchronize on this
                // object and then call notify
                // in order to tell it that the last requested scan has
                // completed. i.e. we have a two way
                ...
                if (request.equals(RecoveryDriver.SCAN)) {
                    synchronized (this) {
                        if (doWait) {
                            // ok, the periodic recovery thread cannot have
                            // finished responding to the last scan request
                            // so it is safe to wait. if we delivered the
                            // request while the last scan was still going
