...
public class EditableAtomicAction extends AtomicAction implements EditableTransaction // going
                                                                                        // to
                                                                                        // have
                                                                                        // to
                                                                                        // be
                                                                                        ...
                                                                                        // differences
{
    ...
    /**
     * Move a previous heuristic participant back to the prepared list so that
     * recovery can try again. If it fails again then it may end up back on the
     * heuristic list.
     */
    ...
    public void moveHeuristicToPrepared(int index) throws IndexOutOfBoundsException {
        if ((index < 0) || (index >= super.heuristicList.size()))
            ...
            /*
             * Before we move we need to tell the resource that is can forget
             * about the heuristic. This assumes that a) the user knows what
             * they are doing, and b) the resource can then be driven through
             * commit once more. If either of those assumptions is incorrect
             * then we could be in a world of pain!
             */
            ...
            if (rec.forgetHeuristic()) {
                 ...
                 * 
                 * We don't maintain a per-resource heuristic so we cannot
                 * change the heuristic each time we remove a resource, i.e., we
                 * have to assume that the original heuristic applies to all of
    ...
    public void deleteHeuristicParticipant(int index) throws IndexOutOfBoundsException {
        if ((index < 0) || (index >= super.heuristicList.size()))
            ...
            // if the log is not entry this call will delete the log
            // automatically.
