...
public class RecoveryManagerImple {
    ...
    public RecoveryManagerImple(boolean threaded) {
        ...
        /*
         * Check whether there is a recovery daemon running - only allow one per object
         * store
         *
         * Note: this does not actually check if a recovery manager is running for the
         * same ObjectStore, only if one is on the same port as our configuration. Thus
         * it's not particularly robust. TODO: add a lock file to the ObjectStore as a
         * belt and braces approach?
         *
         * This check works by trying to bind the server socket, so don't do it if we
         * are running local only (yup, that means there is a greater chance of winding
         * up with more than one recovery manager if we are running without a listener.
         * See comment on robustness and file locking.)
         */
         ...
         *
         * This has to happen after initiating periodic recovery, because periodic
         * recovery registers record types used by the expiry scanner
         */
    ...
    /**
     * Test whether the recovery manager (RM) port and address are available - if
     * not assume that another recovery manager is already active.
     *
     * Ideally this method needs to discover whether or not another RM is already
    ...
    private final boolean isRecoveryManagerEndPointInUse() {
        /*
         * attempt to create the server socket. If an exception is thrown then some
         * other process is using the RM endpoint
         */
