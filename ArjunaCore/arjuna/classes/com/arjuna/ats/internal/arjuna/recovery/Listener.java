...
public class Listener extends Thread {
    /**
     * Creates a listener thread on the specified port for the specified service
     * to run.
     */
    ...
    /**
     * Creates a listener thread on the specified server socket for the
     * specified service to run.
     */
     ...
     *
     * this is pointless because this instance is a thread so never gets garbage
     * collected until it has stopped running. but that means shutdown will have
     * been called making the call to close in this method redundant.
     * 
     * public void finalize() { stopListener();
     * 
     * try { _listener_socket.close(); } catch ( IOException ex ) {
     * tsLogger.arjLoggerI18N.warn(
     * "com.arjuna.ats.internal.arjuna.recovery.Listener_1"); } }
     */

    /**
     * Loops waiting for connection requests from client, creates a new
     * Connection object for each connection.
     */
        ...
        while (!stopRequested()) {
            try {
                ...
                if (addConnection(conn)) {
                    // ok the connection is in the list -- ensure it clears
                    // itself out
                    ...
                    if (tsLogger.logger.isDebugEnabled()) {
                        tsLogger.logger.debug("Connected to " + conn.getInetAddress().getHostAddress() + " on port "
                                + conn.getPort() + " on listener port " + conn.getLocalPort() + " for service "
                ...
                if (tsLogger.logger.isDebugEnabled()) {
                    tsLogger.logger.debug("Recovery listener existing " + _listener_service.getClass().getName());
                }
    ...
    public synchronized boolean addConnection(Socket conn) {
        if (!_stop_listener) {
            ...
            // call to this method. it will have closed all the other
            // connections
            // and will be waiting on this (listener) thread. so close this
            // connection
    ...
    public synchronized void stopListener() {
        ...
        // there is no need for this as the close will interrupt any i/o that is
        // in progress
        // this.interrupt();

        // ok, closing a connection socket will cause the connection thread to
        // remove it from the list as it
