...
/**
 * Implementation of the tx store backed by HornetQ's journal. This is a bean
 * suitable for hooking into the app server lifecycle.
 *
...
public class HornetqJournalStore {
    ...
    public void start() throws Exception {
        ...
        TransactionFailureCallback failureCallback = new TransactionFailureCallback() {
            public void failedTransaction(long l,
                    java.util.List<org.apache.activemq.artemis.core.journal.RecordInfo> recordInfos,
                    java.util.List<org.apache.activemq.artemis.core.journal.RecordInfo> recordInfos1) {
                tsLogger.i18NLogger.warn_journal_load_error();
            }
        };

        JournalLoadInformation journalLoadInformation = journal.load(committedRecords, preparedTransactions,
                failureCallback);
        ...
        for (RecordInfo record : committedRecords) {
            ...
            // don't unpack the rest yet, we may never need it. read_committed
            // does it on demand.
        }
    }
    ...
    public HornetqJournalStore(HornetqJournalEnvironmentBean envBean) throws IOException {
        ...
        journal = new JournalImpl(envBean.getFileSize(), envBean.getMinFiles(), envBean.getPoolSize(),
                envBean.getCompactMinFiles(), envBean.getCompactPercentage(), sequentialFileFactory,
                envBean.getFilePrefix(), envBean.getFileExtension(), envBean.getMaxIO());
    }
    ...
    public InputObjectState read_committed(Uid uid, String typeName) throws ObjectStoreException {
        ...
        // this repeated unpacking is a little inefficient - subclass RecordInfo
        // to hold unpacked form too?
    ...
    /**
     * @return the "name" of the object store. Where in the hierarchy it
     *         appears, e.g., /ObjectStore/MyName/...
     */
