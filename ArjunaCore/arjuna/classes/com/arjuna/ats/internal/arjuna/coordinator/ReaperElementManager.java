...
public class ReaperElementManager {
    /**
     * @return the first (i.e. earliest to time out) element of the colleciton or
     *         null if empty
     */
    ...
    public void add(ReaperElement reaperElement) throws IllegalStateException {
        if (pendingInsertions.putIfAbsent(reaperElement, reaperElement) != null) {
            // note this is best effort - we'll allow double inserts if the element is also
            // in the ordered set.
     ...
     * @param delayMillis   the amount of time to increment the element's timeout
     *                      by.
    ...
    public void remove(ReaperElement reaperElement) {
        ...
        // we missed finding it in the unsorted set - perhaps it has already been copied
        // to the sorted set...
        synchronized (this) {
            removeSorted(reaperElement);
        }
    ...
    // public methods - see class header doc comments for concurrency/performance
    // info.
    ...
    private void flushPending() {
        ...
        // purge the pending inserts before doing anything else. This is potentially
        // expensive.
        ...
        if (entrySet != null) {
            ...
            // iterator is weakly consistent - will traverse elements present at its time of
            // creation,
