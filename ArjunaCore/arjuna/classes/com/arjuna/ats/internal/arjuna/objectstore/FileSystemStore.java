...
public abstract class FileSystemStore extends ObjectStore {
    ...
    /*
     * read an uncommitted instance of State out of the object store. The instance
    ...
    /**
     * Given a type name initialise the <code>state</code> to contains all of the
    ...
    private File openAndLockInternal(String fname, int lmode, boolean create) throws ObjectStoreException {
        ...
        /*
         * Consider re-introducing the FdCache concept from original Arjuna to save time
         * on checking if exists etc.
         */
    ...
    /*
     * Renaming on Unix works if the file to rename to already exists. However, on
     * Windows if the file exists then rename fails! So, we need to delete the file
     * to rename to before we can rename. But, we must ensure that we don't get into
     * any race conditions, so we exclusively lock the file to be deleted first.
     * Failure scenarios:
     *
     * (i) if we crash after deleting, but before we have had a chance to rename the
     * file, then the lock file will exist and prevent anyone from deleting the
     * shadow.
     *
     * (ii) if we crash after renaming and before the lock file is removed then
     * subsequent lock attempts will fail, and the sys admin will have to resolve.
     * But consistency will be maintained!
     *
     ...
     * ShadowNoFileLockStore can still get away with no locking elsewhere to improve
     * performance.
     */
    ...
    protected final boolean renameFromToInternal(File from, File to) {
        if (!isWindows)
            ...
            if (!from.exists()) {
                 ...
                 *
                 * (i) two different users are using the same state and should have been using
                 * the OS_SHARED flag.
                 *
                 * or
                 *
                 ...
                 *
                 * If (ii) we can't force OS_SHARED on all users for the minority case. So,
                 * assume (ii) and issue a warning.
                 */
             ...
             * "com.arjuna.ats.internal.arjuna.objectstore.ShadowingStore_21", new
             * Object[]{to}); }
             *
             * return false; }
             */
    ...
    /**
     * @return the file name for the state of the object identified by the Uid and
     *         TypeName. If the StateType argument is OS_SHADOW then the Uid part of
     *         the name includes # characters. The magic number SLOP below is the
     *         number of extra characters needed to make up the entire path.
     */
    ...
    protected String genPathName(Uid objUid, String tName, int ostype) throws ObjectStoreException {
        ...
        /*
         * storeName always ends in '/' so we can remove any at the start of the type
         * name.
         */
     ...
     * is that one thread will succeed and the other(s) will fail. However, if a
     * failed thread just assumes that the directory is being created by another
     * thread, then we're in trouble, because although this may be the case, the
     * directory structure may not have actually been created - it may still be in
     * the process of being created. So, we have to err on the side of caution and
     * try to create the directory a few times. (This can happen across processes
     * too.)
     */
    ...
    protected final synchronized boolean createHierarchy(String path) throws ObjectStoreException {
        ...
        if ((path != null) && (path.length() > 0)) {
            ...
            /*
             * Is string a complete directory list, or is it an absolute file name?
             */
            ...
            do {
                if (f.exists()) {
                    ...
                    /*
                     * Assume problem is due to concurrent processes, since we're synchronized
                     * within the same VM.
                     */
    ...
    /**
     * If this object store implementation is exclusively working on a set of object
     * states, then this method will check a file cache first. Otherwise, we must go
     * back to the file system each time to check the status of the file.
     *
     * If we add a FileDescriptor cache a la the C++ version then we would be able
     * to get rid of the state cache and simply check to see if we had a fd for it.
     */
    ...
    protected final boolean exists(String path) {
        ...
        /*
         * If here then we need to check the file system. If there, we will put it into
         * the cache (if appropriate).
         */

        File f = new File(path);
        // files created but not yet written are normally considered to not exist yet.
        // JBTM-821
