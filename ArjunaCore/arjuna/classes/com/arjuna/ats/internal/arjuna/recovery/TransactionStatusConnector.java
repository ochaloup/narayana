...
public class TransactionStatusConnector {
    /**
     * Recreate TransactionStatusManagerItem and attempt to establish a
     * connection to the host/port of the TransactionStatusManager.
     */
    ...
    /**
     * Check that a connection can be made to the Transaction Status Manager's
     * process.
     */
    ...
    /**
     * If the TransactonStatusManagers' process is deemed dead, then its
     * TransactonStatusManagerItem is removed from the object store.
     */
    ...
    /**
     * Retrieve the transaction status for the specified transaction, attempts
     * to re-establish connection if necessary.
     */
        ...
        if (!_dead) {
            ...
            if (_tsmFound) {
                try {
                    ...
                    /*
                     * TODO we should optimise this so we only close once all
                     * transactions for a particular host have been sent.
                     */

                    // Retrieve current status from the
                    // TransactionStatusManager.
    ...
    private boolean establishConnection() {
        ...
        if (_tsmi != null) {
            try {
                ...
                _from_server = new BufferedReader(
                        new InputStreamReader(_connector_socket.getInputStream(), StandardCharsets.UTF_8));

                _to_server = new PrintWriter(
                        new OutputStreamWriter(_connector_socket.getOutputStream(), StandardCharsets.UTF_8));
                ...
                if (Utility.hexStringToInt(server_pid) == Utility.hexStringToInt(_pid)) {
                    if (!_testMode) {
                        ...
                        tsLogger.i18NLogger.info_recovery_TransactionStatusConnector_4(_pid, serverHost,
                                Integer.toString(serverPort), _connector_socket.toString());
                    } else {
