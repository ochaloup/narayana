...
public class CadaverRecord extends PersistenceRecord {
     ...
     *
     * @param os
     *            the state of the object that is being removed.
     * @param participantStore
    ...
    /**
     * The nested transaction has aborted. The record will invalidate any new
     * state.
     */
    ...
    public int nestedAbort() {
        ...
        /*
         * No need to forget the action since this object is being deleted so it
         * is unlikely to have modified called on it!
         */
    ...
    /**
     * The nested transaction is preparing. If there is any new state for the
     * object being removed, and that state is valid, then this record will call
    ...
    /**
     * At topLevelCommit we commit the uncommitted version already saved into
     * object participantStore. Cannot use inherited version since that assumes
     * object is alive instead talk directly to the object participantStore
     * itself.
     */
    ...
    /**
     * At topLevelPrepare write uncommitted version into object
     * participantStore. Cannot use inherited version since that assumes object
     * is alive instead talk directly to the object participantStore itself.
     */
    ...
    /**
     * Override AbstractRecord.print to write specific information to the
     * specified stream.
     *
     * @param strm
     *            the stream to use.
    ...
    /**
     * merge takes the information from the incoming PersistenceRecord and uses
     * it to initialise the oldState information. This is required for
     * processing of action aborts since CadaverRecords maintain the final state
     * of an object normally - which is required if the action commits.
     *
     * @param mergewith
    ...
    public void merge(AbstractRecord mergewith) {
        /*
         * Following assumes that value returns a pointer to the old state
         * maintained in the PersistenceRecord (as an ObjectState). Here we
         * create a copy of that state allowing the original to be deleted
         */
     ...
     *
     * @param ar
     *            the record to potentially merge with.
     *
     * @return <code>true</code> if this instance and the parameter have the
     *         same id (order()) and the parameter is either persistent or
     *         recoverable. <code>false</code> otherwise.
     * @see com.arjuna.ats.arjuna.coordinator.RecordType
     */
     ...
     *
     * @param ar
     *            the record to potentially replace this instance.
     *
     * @return <code>true</code> if this instance and the parameter have the
     *         same id (order()) and the parameter is either persistent or
     *         recoverable. <code>false</code> otherwise.
