 ...
 * still in the scope of an atomic action. This ensures that if the action
 * commits the state of the persistent objects gets properly reflected back in
 * the object participantStore. For objects that are only recoverable such work
 * is unnecessary. Cadaver records replace PersistenceRecords in the record list
 * of an atomic action so they must be merged with such records to enable both
...
public class CadaverRecord extends PersistenceRecord {
     ...
     *
     * @param os               the state of the object that is being removed.
     * @param participantStore the object participantStore instance used to
    ...
    public int nestedAbort() {
        ...
        /*
         * No need to forget the action since this object is being deleted so it is
         * unlikely to have modified called on it!
         */
     ...
     * being removed, and that state is valid, then this record will call
     * nestedPrepare on the object being removed.
     *
     * If we have no new state then we cannot commit and must force an abort. Do
     * this by failing the prepare phase.
     */
    ...
    /**
     * At topLevelCommit we commit the uncommitted version already saved into object
     * participantStore. Cannot use inherited version since that assumes object is
     * alive instead talk directly to the object participantStore itself.
     */
     ...
     * Cannot use inherited version since that assumes object is alive instead talk
     * directly to the object participantStore itself.
     */
    ...
    /**
     * Override AbstractRecord.print to write specific information to the specified
     * stream.
     *
    ...
    /**
     * The type of the class - may be used to save information in an hierarchical
     * manner in the object participantStore.
     */
     ...
     *
     * @return <code>true</code> if the object being removed is a persistent object
     *         (RecordType.PERSISTENT). <code>false</code> otherwise.
     * @see com.arjuna.ats.arjuna.coordinator.RecordType
     */
    ...
    /**
     * merge takes the information from the incoming PersistenceRecord and uses it
     * to initialise the oldState information. This is required for processing of
     * action aborts since CadaverRecords maintain the final state of an object
     * normally - which is required if the action commits.
     *
    ...
    public void merge(AbstractRecord mergewith) {
        /*
         * Following assumes that value returns a pointer to the old state maintained in
         * the PersistenceRecord (as an ObjectState). Here we create a copy of that
         * state allowing the original to be deleted
         */
     ...
     *
     * @return <code>true</code> if this instance and the parameter have the same id
     *         (order()) and the parameter is either persistent or recoverable.
    ...
    public boolean shouldMerge(AbstractRecord ar) {
        return (((order().equals(ar.order()))
                && ((ar.typeIs() == RecordType.PERSISTENCE) || (ar.typeIs() == RecordType.RECOVERY))) ? true : false);
    }

    /**
     * Overrides AbstractRecord.shouldReplace
     *
     * @param ar the record to potentially replace this instance.
     *
     * @return <code>true</code> if this instance and the parameter have the same id
     *         (order()) and the parameter is either persistent or recoverable.
     *         <code>false</code> otherwise.
    ...
    /**
     * Create a new instance using default values. Typically used during failure
     * recovery.
     */
