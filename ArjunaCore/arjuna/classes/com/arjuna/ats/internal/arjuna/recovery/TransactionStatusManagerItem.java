...
public class TransactionStatusManagerItem {
    ...
    /**
     * Crash Recovery uses this method to recreate a representation of the
     * Transaction Status Managers host/port pair contact.
     */
    ...
    /**
     * Read host/port pair from the ObjectStore using the process Uid as a unique
     * identifier.
     */
    ...
    /**
     * Write host/port pair to the ObjectStore using the process Uid as a unique
     * identifier.
     */
    private boolean saveThis() {
        ...
        try {
            ...
            if (save_state(objstate)) {
                ret_status = getStore().write_committed(_pidUid, _typeName, objstate);
            }
        } catch (ObjectStoreException ex) {
            tsLogger.i18NLogger.warn_recovery_TransactionStatusManagerItem_2(ex);
        }
    ...
    /**
     * Constructor which obtains the process uid and host for use with the specified
     * port.
     */
    ...
    /**
     * Constructor which obtains the process uid for use with the specified host and
     * port.
     */
    ...
    /**
     * Used by a Recovery Manager to recreate a Transaction status manager contact
     * item.
     */
