...
public class ReaperThread extends Thread {
    ...
    public void run() {
        ...
        for (;;) {
            /*
             * Cannot assume we sleep for the entire period. We may be
             * interrupted. If we are, just run a check anyway and ignore.
             */

            synchronized (reaperObject) {
                // test our condition -- things may have changed while we were
                // checking
                ...
                if (sleepPeriod > 0) {
                    try {
                        if (tsLogger.logger.isTraceEnabled()) {
                            tsLogger.logger.trace(
                                    "Thread " + Thread.currentThread() + " sleeping for " + Long.toString(sleepPeriod));
                        }
                    ...
                    // test our condition -- things may have changed while we
                    // were waiting
