...
public class PeriodicRecovery extends Thread {
    ...
    public static enum Status {
        ...
        INACTIVE,
        /**
         * state value indicating that some thread is scanning. n.b. the
         * scanning thread may not be the singleton PeriodicRecovery thread
         * instance
         */
        SCANNING
    }
    ...
    /**
     * state values indicating operating mode of scanning process for ad hoc
     * threads and controlling behaviour of singleton periodic recovery thread.
     * used to define values of field {@link PeriodicRecovery#_currentMode}
     *
     * n.b. {@link PeriodicRecovery#_currentStatus} may not transition to state
     * SCANNING when {@link PeriodicRecovery#_currentStatus} is in state
     * SUSPENDED or TERMINATED. However, if a scan is in progress when
     * {@link PeriodicRecovery#_currentMode} transitions to state SUSPENDED or
     * TERMINATED {@link PeriodicRecovery#_currentStatus} may (temporarily)
     * remain in state SCANNING before transitioning to state INACTIVE.
     */
    public static enum Mode {
        ...
        ENABLED,
        /**
         * state value indicating that new scans may not proceed and the
         * periodic recovery thread should suspend
         */
        SUSPENDED,
        /**
         * state value indicating that new scans may not proceed and that the
         * singleton PeriodicRecovery thread instance should exit if it is still
         * running
         */
        TERMINATED
    }
    ...
    /**
     * initiate termination of the periodic recovery thread and stop any
     * subsequent scan requests from proceeding.
     *
     * this switches the recovery operation mode to TERMINATED. if a scan is in
     * progress when this method is called and has not yet started phase 2 of
     * its scan it will be forced to return before completing phase 2.
     *
     * @param async
    ...
    public void shutdown(boolean async) {
        ...
        synchronized (_stateLock) {
            ...
            if (!async) {
                // synchronous, so we keep waiting until the currently active
                // scan stops or scanning
        ...
        // if the shutdown is synchronous then make sure the periodic recovery
        // thread really has stopped running
     ...
     *
     * This switches the recovery operation mode to SUSPENDED. Any attempt to
     * start a new scan either by an ad hoc threads or by the periodic recovery
     * thread will suspend its thread until the mode changes. If a scan is in
     * progress when this method is called it will complete its scan without
     * suspending.
     *
     * @param async
     *            false if the calling thread should wait for any in-progress
     ...
     * threads which suspended when they tried to start a scan will be woken up
     * by this transition.
     */
     ...
     * @throws IOException
     *             if the host name is unknown or the endpoint has already been
     *             bound
     */
    ...
    public void run() {
        ...
        do {
            ...
            synchronized (_stateLock) {
                if (getStatus() == Status.SCANNING) {
                    ...
                    doScanningWait();
                    // we don't wait around if a worker scan request has just
                    // come in
                    if (getMode() == Mode.ENABLED && !_workerScanRequested) {
                        // the last guy just finished scanning so we ought to
                        // wait a bit rather than just
                    ...
                    // status == INACTIVE so we can go ahead and scan if
                    // scanning is enabled
                    switch (getMode()) {
                        case ENABLED :
                        ...
                        case SUSPENDED :
                            ...
                            doSuspendedWait();
                            // we come out of here with the lock and either
                            // ENABLED or TERMINATED
                            finished = (getMode() == Mode.TERMINATED);
                            break;
                        case TERMINATED :
                            finished = true;
            ...
            // its ok to start work if requested -- we cannot be stopped now by
            // a mode change to SUSPEND
            // or TERMINATE until we get through phase 1 and maybe phase 2 if we
            // are lucky

            if (workToDo) {
                // ok it is now this thread's turn to run a scan. before
                // starting we check if there is a
                // worker waiting and reset the waiting flag. we will check
                // again after the scan has
                // completed to see if a worker request has come in after
                // starting this scan.
                // if so we avoid notifying the worker ensuring a requst is only
                // confirmed when a
                ...
                synchronized (_stateLock) {
                    ...
                    // check if we need to notify a listener worker that we just
                    // finished a scan
        ...
        // make sure the worker thread is not wedged waiting for a scan to
        // complete
     ...
     *
     * <b>Caveats:</b> if a scan is already in progress this method will wait
     * for it to complete otherwise it will perform its own scan before
     * returning. If scanning is suspended this will require waiting for
     * scanning to resume.
     */
    ...
    public final void doWork() {
        ...
        synchronized (_stateLock) {
            ...
            if (getMode() == Mode.TERMINATED) {
                ...
                // ok scanning must be enabled -- see if we can start a scan or
                // whether we have to wait on another one
        ...
        if (workToDo) {
            // ok it is now this thread's turn to run a scan. before starting we
            // check if there is a
            // worker waiting and reset the waiting flag. we will check again
            // after the scan has
            // completed to see if a worker request has come in after starting
            // this scan.
            // if so we avoid notifying the worker ensuring a request is only
            // confirmed when a
            ...
            // ok to start work -- we cannot be stopped now by a mode change to
            ...
            synchronized (_stateLock) {
                ...
                // notify the worker if it was waiting before we started the
                // scan otherwise just leave it to
    ...
    /**
     * called by the listener worker to wake the periodic recovery thread and
     * get it to start a scan if one is not already in progress
     */
    ...
    /**
     * Add the specified module to the end of the recovery module list. There is
     * no way to specify relative ordering of recovery modules with respect to
     * modules loaded via the property file.
     *
     * @param module
    ...
    public final Vector<RecoveryModule> getModules() {
        ...
        // dynamic modifications to the list
    ...
    /**
     * wait for the required backoff period or less if the scanning status or
     * scan mode changes
     *
    ...
    /**
     * wait for the required recovery period or less if the scanning status or
     * scan mode changes
     *
     * <b>Caveats:</b> this must only be called when synchronized on
     * {@link PeriodicRecovery#_stateLock} and when _currentStatus is INACTIVE
     * and _currentMode is ENABLED
     */
    ...
    /**
     * wait for the initial required recovery delay or less if the scanning
     * status or scan mode changes
     */
    ...
    /**
     * start performing a scan continuing to completion unless we are
     * terminating
     *
     * <b>Caveats:</b> this must only be called when _currentStatus is SCANNING.
     * on return _currentStatus is always still SCANNING
     */
    ...
    private void doWorkInternal() {
        ...
        // n.b. this works on a copy of the modules list so it is not affected
        // by
        // dynamic updates in the middle of a scan, ensuring first+second pass
        // happen
        ...
        while (modules.hasMoreElements()) {
            ...
            // we need to ensure we use the class loader context of the recovery
            // module while we are executing
            // its methods
        ...
        synchronized (_stateLock) {
            ...
            // we carry on scanning even if scanning is SUSPENDED because the
            // suspending thread
            // might be waiting on us to complete and we don't want to risk
            // deadlocking it by waiting
            // here for a resume.
            // if we have been TERMINATED we bail out now
            // n.b. if we give up here the caller is responsible for clearing
            // the active scan
    ...
    /**
     * install the classloader associated with some specific recovery module as
     * the current thread's class loader
     *
     * this avoids a problem where the background periodic recovery thread can
     * see the same class as the recovery module's class loader, specifically
     * where a the recovery module resides in a sar (e.g. the XTS code). If
     * class with name "A" is loaded via the background thread class loader as
     * A' and used to create instance a' then a cast expression in th erecovery
     * code of the form (A)a' will try to resolve a' against version A'' loaded
     * via the sar loader and get a class cast exception.
     *
     * @param rm
     *            the recovery module whose class loader is to be installed as
     *            the new thread class loader
     ...
     *
     * @param cl
     *            the class loader to be set as the current thread class loader
     */
    ...
    /**
     * Load recovery modules prior to starting to recovery. These are loaded in
     ...
     * specified in the recovery manager property file or added dynamically by
     * calls to addModule
     */
    ...
    /**
     * Time in seconds for which the periodic recovery thread waits to start
     * initial scan.
     */
    private int _periodicRecoveryInitilizationOffset = 0;

    /**
     * default value for _backoffPeriod if not specified via
     ...
     *
     * Note: if we start and stop the service then changes to the timeouts won't
     * be reflected. We will need to modify this eventually.
     */
