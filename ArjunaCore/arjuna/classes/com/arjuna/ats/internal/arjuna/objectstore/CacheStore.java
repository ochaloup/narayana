...
public class CacheStore extends HashedStore {
    ...
    protected boolean remove_state(Uid objUid, String name, int ft) throws ObjectStoreException {
        ...
        /*
         * Check to see if there's a state on disk. If there is, then we still need to
         * add a remove work item to the cache queue.
         */

        int fileState = currentState(objUid, name);

        if ((fileState != StateStatus.OS_UNKNOWN) || (status == AsyncStore.IN_USE)) {
    ...
    protected boolean write_state(Uid objUid, String tName, OutputObjectState state, int ft)
        ...
        /*
         * If there is already a write operation in the cache for exactly this state and
         * type, then remove it and any corresponding remove_state there might be. This
         * is because write_state overwrites the state rather than appending some
         * operational work. If we used an appender log then obviously this isn't
         * appropriate.
         */
    ...
    /**
     * Before we look at the disk let's look in the state cache first for the state,
     * just in case it hasn't been written out to persistent store yet.
     */
...
class ShutdownThread extends Thread {
    ...
    public void run() {
        ...
        synchronized (CacheStore._storeManager._activeLock) {
            /*
             * We don't want to exit the VM if the worker thread is currently writing to the
             * disk. That would be very bad :-(! So, just check that the thread isn't
             * actively doing work.
             */
...
//keep priority same as app. threads
class AsyncStore extends Thread {
    ...
    public final void flush() {
        /*
         * Do it this way because by the time we get here the daemon thread has been
    ...
    public final boolean addWork(CacheStore store, int workType, Uid objUid, String tName, OutputObjectState state,
            int ft) {
        /*
         * If the cache is full already, then wait until it has drained. We sit in a
         * while/do loop because many threads could be blocked and only one may get a
         * chance to add something to the queue before it goes full again.
         */
    ...
    /**
     * Remove any item in the cache that operates on this state (must be identical
     * uid and file type (shadowed, hidden etc.) This could potentially leave us
     * with states on disk that should have been deleted but weren't because a crash
     * happened before we could do that. Crash recovery should fix this up later
     * though.
     */
    ...
    public void run() {
        ...
        while (!_terminated) {
            synchronized (_activeLock) {
                ...
                /*
                 * We've drained the queue, so notify any blocked threads so that they get a
                 * chance to add something to the queue again.
                 */
    ...
    private final void doWork() {
        ...
        if ((_work != null) && !_work.removed) {
            /*
             * Should write any errors to a persistent log so that an admin tool can pick up
             * the pieces later.
             */
    ...
    private final boolean cacheIsFull() {
        synchronized (_workList) {
            if ((_currentCacheSize >= _maximumCacheSize) || (_removedItems >= _maximumRemovedItems)
    ...
    private static final int _defaultRemovedItems = arjPropertyManager.getObjectStoreEnvironmentBean()
            .getCacheStoreRemovedItems();

    private static final int _defaultWorkItems = arjPropertyManager.getObjectStoreEnvironmentBean()
            .getCacheStoreWorkItems();

    private static final int _defaultScanPeriod = arjPropertyManager.getObjectStoreEnvironmentBean()
