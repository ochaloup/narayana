...
public class JDBCObjectStoreTest {
    ...
    public void testJDBCImple() throws SQLException, ObjectStoreException, Exception {
        ...
        assertTrue(imple.write_state(new Uid(), "tName", new OutputObjectState(), StateStatus.OS_UNCOMMITTED,
                "tableName"));

        try {
            byte[] buff = new byte[imple.getMaxStateSize() + 1];

            assertTrue(imple.write_state(new Uid(), "tName", new OutputObjectState(new Uid(), "tName", buff),
                    StateStatus.OS_UNCOMMITTED, "tableName"));
    ...
    public void testOracleDriver() throws SQLException, ObjectStoreException {
        ...
        try {
            assertTrue(drvr.write_state(new Uid(), "foobar", new OutputObjectState(new Uid(), "foobar", buff),
                    StateStatus.OS_COMMITTED, "mytable"));
