...
//        }
    ...
    /**
     * Create an atomic action with two participants, one of which will generate a
     * heuristic during phase 2. The test will move the heuristic back into the
     * prepared state and trigger recovery to replay phase 2. The test then asserts
     * that the corresponding MBeans have been unregistered.
     *
    ...
    /**
     * Similar to aaReplayTest except that the whole transaction record is removed
     * from the object store (instead of replaying the record that generates a
     * heuristic).
     *
     * @throws Exception if test fails unexpectedly
     */
    ...
    public void aaTest(boolean replay) throws Exception {
        ...
        CrashRecord[] recs = { new CrashRecord(CrashRecord.CrashLocation.NoCrash, CrashRecord.CrashType.Normal),
                new CrashRecord(CrashRecord.CrashLocation.CrashInCommit, CrashRecord.CrashType.HeuristicHazard) };

        // register CrashRecord record type so that it is persisted in the object store
        // correctly
        ...
        // and there should be one MBean corresponding to the CrashRecord that got the
        // heuristic:
        int recCount = 0;
        for (CrashRecord rec : recs) {
        ...
        if (!replay) {
            ...
            /*
             * prompt the recovery manager to replay the record that was moved off the
             * heuristic list and back onto the prepared list
             */
            rcm.scan();
        }

        /*
         * Since the recovery scan (or explicit remove request) will have successfully
         * removed the record from the object store another probe should cause the MBean
         * representing the record to be unregistered
         */
        osb.probe();
