...
public class PerformanceTest extends BaseTest {
    ...
    public void measureThroughput() throws Exception {
        ...
        System.out.printf(
                "%s:  Test performance: %d calls/sec (%d invocations using %d threads with %d errors. Total time %d ms)%n",
                info, opts.getThroughput(), opts.getNumberOfCalls(), opts.getThreadCount(), opts.getErrorCount(),
                opts.getTotalMillis());

        Assert.assertEquals(0, opts.getErrorCount());

        boolean correct = PerformanceProfileStore.checkPerformance(getClass().getName() + "_measureThroughput_" + info,
                opts.getThroughput(), true);
    ...
    private class RTSWorker implements WorkerWorkload<String> {
        ...
        private void runEmptyTxn(TxSupport txn) throws Exception {
             ...
             * 
             * for (int i = 0; i < 100; i++) { TxSupport tx = new TxSupport();
             * tx.startTx(); txns.add(tx); }
             * 
             * Thread.sleep(240000); for (TxSupport tx : txns) { tx.commitTx();
             * } txns.clear();
             */
