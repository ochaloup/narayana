...
public class SpecTest extends BaseTest {
    ...
    public void testTransactionUrls() throws Exception {
        ...
        /*
         * 196Performing a POST on the transaction-manager URI with header as
         * shown 197below will start a new transaction with a default timeout. A
         * successful invocation will return 201 198and the Location header MUST
         * contain the URI of the newly created transaction resource, which
         * 199we will refer to as transaction-coordinator in the rest of this
         * specification. At least two related 200URLs MUST also be returned,
         * one for the terminator of the transaction to use (typically referred
         * 201to as the client) and one used for registering durable
         * participation in the transaction (typically 202referred to as the
         * server). These are referred to as the transaction-terminator and
         * transaction- 203enlistment URIs, respectively. Although uniform URL
         * structures are used in the examples, these 204linked URLs can be of
         * arbitrary format.
         */
        ...
        /*
         * 228Performing a HEAD on the transaction-coordinator URI MUST return
         * the same link information.
         */
         ...
         * have committed and aborted. The link MAY contain a 266media type hint
         * with value “application/txstatusext+xml”.
         */
        Collection<String> txns = txn.getTransactions(TxMediaType.TX_LIST_MEDIA_TYPE);

        // assert that the returned list of transactions contains the new
        // transaction:
         ...
         * 272current status of the transaction, as described later. TODO Check
         * which other status's we test for
         */
        ...
        /*
         * 285Performing a DELETE on any of the transaction-coordinator or
         * transaction-enlistment URIs 286/transaction-coordinator URIs will
         * return a 403.
         */
        txn.httpRequest(new int[]{HttpURLConnection.HTTP_FORBIDDEN}, txn.getTxnUri(), "DELETE", null);

        /*
         * 292The client can PUT one of the following to the
         * transaction-terminator URI /transaction-
         * 293coordinator/1234/terminator in order to control the outcome of the
         * transaction; anything else 294MUST return a 400 (unless the
         * terminator and transaction URLs are the same in which case GET
        ...
        for (HTTP_METHOD method : HTTP_METHOD.values()) {
            // if sameUrls then the terminator url will support both GET and PUT
            // otherwise just PUT is supported
        ...
        /*
         * 295 ... Performing a PUT as shown below 296will trigger the commit of
         * the transaction. Upon termination, the resource and all associated
         * 297resources are implicitly deleted. For any subsequent PUT
         * invocation, such as due to a 298timeout/retry, then an implementation
         * MAY return 410 if the implementation records information 299about
         * transactions that have rolled back, (not necessary for presumed
         * rollback semantics) but at 300a minimum MUST return 404. The invoker
         * can assume this was a rollback. In order for an 301interested party
         * to know for sure the outcome of a transaction then it MUST be
         * registered as a 302participant with the transaction coordinator.
         */
    ...
    public void testTransactionTimeout() throws Exception {
        ...
        /*
         * 241Performing a POST on the transaction-manager URI as shown below
         * will start a new transaction 242with the specified timeout in
         * milliseconds.
         */
        txn.startTx(1000);
         ...
         * transactions that have rolled back, (not necessary for presumed
         * rollback semantics) but at 300a minimum MUST return 404. The invoker
         * can assume this was a rollback. In order for an
         * 
         * and
         * 
         * 311... If the transaction no longer exists then 312an implementation
         * MAY return 410 if the implementation records information about
         * transactions 313that have rolled back, (not necessary for presumed
         * rollback semantics) but at a minimum MUST 314return 404.
         */
        try {
    ...
    /*
     * TODO there is no test for the following 316The state of the transaction
     * MUST be TransactionActive for this operation to succeed. If the
     * 317transaction is in an invalid state for the operation then the
     * implementation MUST return a 412 318status code. Otherwise the
     * implementation MAY return 200 or 202 codes. In the latter case the
     * 319Location header SHOULD contain a URI upon which a GET may be performed
     * to obtain the 320transaction outcome. It is implementation dependent as
     * to how long this URI will remain valid. 321Once removed by an
     * implementation then 410 MUST be returned.
     */
    ...
    public void testRollback() throws Exception {
        ...
        /*
         * 323The transaction may be told to rollback with the following PUT
         * request:
         */
        Assert.assertEquals(TxStatusMediaType.TX_ROLLEDBACK, txn.rollbackTx());

        /*
         * 296... Upon termination, the resource and all associated 297resources
         * are implicitly deleted. For any subsequent PUT invocation, such as
         * due to a 298timeout/retry, then an implementation MAY return 410 if
         * the implementation records information 299about transactions that
         * have rolled back, (not necessary for presumed rollback semantics) but
         * at 300a minimum MUST return 404. The invoker can assume this was a
         * rollback.
         */
        try {
            Assert.assertEquals(TxStatusMediaType.TX_ROLLEDBACK, txn.rollbackTx());
    ...
    public void testRollbackOnlyWithCommit() throws Exception {
        ...
        /*
         * committing a transaction marked rollback only should automatically
    ...
    public void testEnlistResource() throws Exception {
        ...
        /*
         * make sure we can't POST to the terminator URL - we already tested
         * this but not with enlisted participants 292The client can PUT one of
         * the following to the transaction-terminator URI /transaction-
         * 293coordinator/1234/terminator in order to control the outcome of the
         * transaction; anything else 294MUST return a 400 (unless the
         * terminator and transaction URLs are the same in which case GET
         * 295would return the transaction status as described previously)
         */
        try {
            txn.httpRequest(new int[]{HttpURLConnection.HTTP_BAD_REQUEST}, txn.getTerminatorURI(), "POST",
    ...
    /*
     * This test starts a transaction enlists a participant and then asks the
     * participant to tell the coordinator to terminate it on a different url.
     * Arrange for the transaction to rollback due to a timeout. This condition
     * tests an edge case that could cause the coordinator not to delete
    ...
    public void testRecoveryUrlIsRemovedAfterCompletion() throws Exception {
        ...
        String recoveryUrl = txn.httpRequest(new int[]{HttpURLConnection.HTTP_OK}, pId1 + "&query=recoveryUrl", "GET",
        ...
        String newPid = txn.httpRequest(new int[]{HttpURLConnection.HTTP_OK},
                pId1 + "&query=move&arg=101&register=true&twoPhaseAware=false", "GET", TxMediaType.PLAIN_MEDIA_TYPE);

        newPid = PURL + "?pId=" + newPid;
        // make sure the TransactionalResource has forgotten about the original
        // pId
        ...
        // ask the transaction links using the recovery URL. Since the txn has
        // completed the
    ...
    public void testHeuristic() throws Exception {
         ...
         */
        String[] pUrl = {PURL, PURL + "?fault=H_ROLLBACK", // this participant
                                                            // will produce a
                                                            // heuristic
                PURL,};
        ...
        // ask the dummy TransactionalResource for the terminator and
        // participant urls:
        content = txn.httpRequest(new int[]{HttpURLConnection.HTTP_OK}, PURL + "?pId=" + work[1], "GET",
                TxMediaType.PLAIN_MEDIA_TYPE);
        ...
        // (see the test testHeuristicWithForget for how to get the Transaction
        // Manager to
        // tell the participant to forget it)
        txn.httpRequest(new int[]{HttpURLConnection.HTTP_OK, HttpURLConnection.HTTP_NO_CONTENT}, pParticipant, "DELETE",
                null);
    ...
    public void testHeuristicWithForget() throws Exception {
        TxSupport txn = new TxSupport();
        String[] pUrl = {PURL + "?fault=H_ROLLBACK", PURL,};
        String[] work = new String[pUrl.length];
        ...
        /*
         * the commit request should produce a heuristic but since the first
         * participant generates a heuristic rollback the transaction manager
         * (TM) can decide to rollback the second participant. Hence both
         * participants will be consistent and therefore the TM will tell the
         * first participant that it is OK to forget the heuristic
         */

        String content = txn.commitTx();
        Assert.assertEquals(TxStatusMediaType.TX_H_ROLLBACK, content);

        // the participant terminator should have gone even though it got a
        // heuristic
        txn.httpRequest(new int[]{HttpURLConnection.HTTP_NOT_FOUND}, PURL + "?pId=" + work[0], "GET",
                TxMediaType.PLAIN_MEDIA_TYPE);
    }
    ...
    public void testSpec6() throws Exception {
        ...
        String[] pUrl = {PURL, PURL + "?fault=CDELAY", PURL,};

        // start another transaction
        txn.startTx();

        // enlist transactional participants (one of which will delay during
        // commit)
        for (String url : pUrl)
    ...
    public void testHalt() throws Exception {
        ...
        String[] pUrl = {PURL, PURL + "?fault=HALT", PURL,};
    ...
    public void testParticipantStatus() throws Exception {
        TxSupport txn = new TxSupport();
        String[] pUrls = {PURL + "?fault=CDELAY", PURL,};
        ...
        String content = txn.httpRequest(new int[]{HttpURLConnection.HTTP_OK}, PURL + "?pId=" + work[0], "GET",
                TxMediaType.PLAIN_MEDIA_TYPE);
        ...
        /*
         * 427 Performing a GET on the /participant-resource URL MUST return the
         * current status of the 428 participant
         */

        /*
         * wait long enough for the background thread to commit the transaction.
         * Note one of the participants will delay during the commit phase. Wait
         * long enough for the prepare to be called on each participant but not
         * too long that the participant commit calls will complete @see
         * TransactionalResource#terminate where the delay is 3000 ms
         */
        Thread.sleep(500);
        ...
        // the commit on the first participant is delayed so asking for its
        // status should return prepared:
        content = txn.httpRequest(new int[]{HttpURLConnection.HTTP_OK}, pParticipant, "GET",
                TxMediaType.TX_STATUS_MEDIA_TYPE);

        // on some CI machines the GET request is delayed until the outstanding
        // coordinator termination call completes
        // (it worked on all machines prior to the Grizzly upgrade) so, for now,
        // test for both TX_PREPARED or TX_COMMITTED
        ...
        // content.equals(TxStatusMediaType.TX_PREPARED) ||
        // content.equals(TxStatusMediaType.TX_COMMITTED));
    }
    ...
    public void testCannotEnlistDuring2PC() throws Exception {
        ...
        /*
         * 376If the transaction is not TransactionActive when registration is
         * attempted, then the implementation 377MUST return a 412 status code.
         * If the implementation has seen this participant URI before then it
         * 378MUST return 400. Otherwise the operation is considered a success
         * and the implementation 379MUST return 201 and SHOULD use the Location
         * header to give a participant specific URI that 380the participant MAY
         * use later during prepare or for recovery purposes.
         */
        String[] pUrls = {PURL + "?fault=PDELAY", PURL,};
        String[] work = new String[pUrls.length];
        ...
        String enlistUrls = txn.httpRequest(new int[]{HttpURLConnection.HTTP_OK}, PURL + "?pId=" + work[0], "GET",
                TxMediaType.PLAIN_MEDIA_TYPE);
        // and test that enlisting a participant a second time generates a 400
        // code:
        txn.httpRequest(new int[]{HttpURLConnection.HTTP_BAD_REQUEST}, txn.getDurableParticipantEnlistmentURI(), "POST",
                TxMediaType.POST_MEDIA_TYPE, enlistUrls);
        ...
        Thread.sleep(1000);
        // the transaction should now be prepared so it should be too late to
     ...
     * 
     * @param notifyRecovery
     *            the resource will tell the coordinator that it should be
     *            called on a different URI
     * @param twoPhaseAware
     *            if true the participant should be two Phase aware
     * @throws Exception
     */
    public void recovery(boolean notifyRecovery, boolean twoPhaseAware) throws Exception {
        ...
        String newPid = txn.httpRequest(new int[]{HttpURLConnection.HTTP_OK},
                pId1 + "&query=move&arg=101&register=" + notifyQuery + "&twoPhaseAware=" + twoPhaseAware, "GET",
                TxMediaType.PLAIN_MEDIA_TYPE);
        ...
        if (notifyRecovery) {
            ...
            String status = txn.httpRequest(new int[]{HttpURLConnection.HTTP_OK}, pId2 + "&query=status", "GET",
                    TxMediaType.PLAIN_MEDIA_TYPE);
            ...
            // the participant that moved should not have had prepare or commit
            // called on it so expect 204
            txn.httpRequest(new int[]{HttpURLConnection.HTTP_NO_CONTENT}, newPid + "&query=status", "GET",
                    TxMediaType.PLAIN_MEDIA_TYPE);
    ...
    @Test // recovery of two phase aware participants without notifying the
            // coordinator that a participant moved
    public void testRecoveryURLTwoPhaseAwareWithoutNotification() throws Exception {
        recovery(false, true);
    }

    @Test // recovery of two phase aware participants where the participant
            // tells the coordinator it moved
    public void testRecoveryURLTwoPhaseAwareWithNotification() throws Exception {
        recovery(true, true);
    }

    @Test // recovery of two phase unaware participants without notifying the
            // coordinator that a participant moved
    public void testRecoveryURLTwoPhaseUnawareWithoutNotification() throws Exception {
        recovery(false, false);
    }

    @Test // recovery of two phase unaware participants where the participant
            // tells the coordinator it moved
