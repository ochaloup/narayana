 ...
 * 
 * @deprecated The new class will be updated to extend Exception instead of
 *             Error
 */
@Deprecated // The new class will be updated to extend Exception instead of
            // Error
public class HttpResponseException extends Error {
    ...
    public HttpResponseException(Throwable cause, String body, int[] expectedResponses, int actualResponse) {
        this(cause, body, (expectedResponses != null && expectedResponses.length != 0 ? expectedResponses[0] : -1),
                actualResponse);
    }
