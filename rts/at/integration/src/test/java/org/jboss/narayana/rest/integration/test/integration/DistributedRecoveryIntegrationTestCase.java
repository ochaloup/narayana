...
public class DistributedRecoveryIntegrationTestCase extends AbstractIntegrationTestCase {
    ...
    protected static final String TRANSACTION_MANAGER_URL = FIRST_BASE_URL
    ...
    public static WebArchive getFirstDeployment() {
        return ShrinkWrap.create(WebArchive.class, FIRST_DEPLOYMENT_NAME + ".war").addClass(LoggingParticipant.class)
                .addClass(TestParticipantDeserializer.class).addClass(TransactionalService.class)
    ...
    public static WebArchive getSecondDeployment() {
        return ShrinkWrap.create(WebArchive.class, SECOND_DEPLOYMENT_NAME + ".war").addClass(LoggingParticipant.class)
                .addClass(TestParticipantDeserializer.class).addClass(TransactionalService.class)
                .addClass(RestATManagementResource.class).addAsWebInfResource(new File("web.xml"), "web.xml")
                .addAsManifestResource(new StringAsset(DEPENDENCIES), "MANIFEST.MF");
    }
    ...
    private void enlistParticipant(final String enlistmentUrl, final String deploymentUrl, final Vote vote) {
        final Response response = ClientBuilder.newClient()
                ...
                .queryParam("participantEnlistmentUrl", enlistmentUrl).queryParam("vote", vote.getClass().getName())
                .request().post(null);
    ...
    private void registerDeserializer(final String deploymentUrl) {
        final Response response = ClientBuilder.newClient()
                .target(deploymentUrl + "/" + TransactionalService.PATH_SEGMENT).request().put(null);

        Assert.assertEquals(204, response.getStatus());
    }

    private JSONArray getParticipantsInformation(final String deploymentUrl) {
        final String response = ClientBuilder.newClient().target(deploymentUrl + "/"
                + RestATManagementResource.BASE_URL_SEGMENT + "/" + RestATManagementResource.PARTICIPANTS_URL_SEGMENT)
