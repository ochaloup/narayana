...
public class NarayanaLRARecoveryTest {
    ...
    public void extractCoordinatorFromUsualLRAId() {
        ...
        Assert.assertEquals(null, coordinatorUri.getQuery());
        Assert.assertEquals(null, coordinatorUri.getFragment());
        Assert.assertEquals("http://localhost:8080/lra-coordinator", coordinatorUri.toASCIIString());
    }

    @Test
    public void extractCoordinatorWithMultipleCoordinatorPaths() {
    ...
    @Test
    public void extractCoordinatorWithNoCoordinatorPath() {
        URI lraId = URI.create("https://localhost:8080/some-strange-path");
        URI coordinatorUri = NarayanaLRARecovery.extractLRACoordinatorUri(lraId);
        Assert.assertEquals("https://localhost:8080/some-strange-path", coordinatorUri.toASCIIString());
    }
}
