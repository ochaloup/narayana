    ...
    public OperationResult invokeOperation(String name, Configuration params) {
        ...
        if (name.equals("setStatus")) {
            try {
                ...
                // Object rv = getBean().getOperation(name).invoke(new Object[]
                // {params.getSimpleValue("status", "PREPARED")});

                res.setSimpleResult("Operation succeeded");
            } catch (Exception e) {
                res.setErrorMessage(
                        "Operation failed: " + (e.getMessage() == null ? e.getClass().getName() : e.getMessage()));
            }
        }
