...
/**
 * A discovery class for transaction managers - there should generally be only
 * one
 */
public class TransactionEngineDiscovery implements ResourceDiscoveryComponent<MBeanResourceComponent> {
    ...
    public Set<DiscoveredResourceDetails> discoverResources(ResourceDiscoveryContext<MBeanResourceComponent> context)
        ...
        String version = "0.2"; // context.getDefaultPluginConfiguration().getSimpleValue("Version",
                                // "0.1");
        String description = context.getDefaultPluginConfiguration().getSimpleValue("description",
                "JBossTS Transaction Management");

        DiscoveredResourceDetails res = new DiscoveredResourceDetails(context.getResourceType(), key, name, version,
