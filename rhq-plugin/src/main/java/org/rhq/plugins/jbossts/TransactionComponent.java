...
public class TransactionComponent extends BaseComponent implements DeleteResourceFacet {
    /*
     * There are two ways to delete the transaction, via: 1) invokeOperation, or
     * 2) deleteResource But deleteResource does not provide any way of
     * notifying the caller that the operation failed and the rhq console would
     * then be missing the transaction (until the next update). So, until it's
     * fixed, record whether or not it has already been deleted
     */
    ...
    public void deleteResource() throws Exception {
        try {
            ...
            // TODO doing this produces a stack trace on the rhq console window
            // - ask the rhq team to provide an error report
            // throw new Exception("Unable to remove transaction: " +
            // e.getMessage());
        }
    }
    ...
    /**
     * Transaction participants are represented by MBeans whose ObjectName is
     * prefixed by the MBean name of the transaction. Using this fact this
     * method performs an MBean query to select participants of this transaction
     * only
     * 
