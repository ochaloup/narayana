...
/**
 * ANT task to enable an ANT script to build up a classpath on the fly.
 *
...
public class ClasspathBuilder extends org.apache.tools.ant.Task {
     ...
     *
     * @param filename
     *            The filename of the file to store the classpath in
     */
    ...
    /**
     * Set the clear flag. If this flag is set (true) then clear the contents of
     * the classpath build file.
     *
     * @param clear
     *            True or False
     */
     ...
     *
     * @param property
     *            The property to store the classpath in
     */
    ...
    public boolean putClasspathInProperty(String[] filename, String property) {
        ...
        /**
         * Open classpath file and read each line into a string buffer putting
         * the necessary classpath separator between each element.
         */
        for (int filenameCount = 0; filenameCount < filename.length; filenameCount++) {
            try {
                BufferedReader in = new BufferedReader(
                        new InputStreamReader(new FileInputStream(filename[filenameCount])));
                String inLine = null;
    ...
    public void execute() throws BuildException {
        ...
        if (_paths.size() > 0) {
            try {
                for (int filenameCount = 0; filenameCount < _filename.length; filenameCount++) {
                    BufferedWriter out = new BufferedWriter(
                            new OutputStreamWriter(new FileOutputStream(_filename[filenameCount], !_clear)));
            ...
            /**
             * If a request to clear the file has been made but no entries have
             * been given then we need to delete the classpath builder file.
             */
            for (int filenameCount = 0; filenameCount < _filename.length; filenameCount++) {
