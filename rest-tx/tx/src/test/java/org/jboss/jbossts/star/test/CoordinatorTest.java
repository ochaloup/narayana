...
public class CoordinatorTest extends BaseTest {
    ...
    public void testCommitInvalidTx() throws IOException {
        ...
        // we know that the terminator URL ends in terminate (by reading the
        // code for the provider) - modify to produce an invalid one
        terminator = terminator.replace("/terminate", "_dead/terminate");
        // an attempt to commit on this URL should fail:
        txn.httpRequest(new int[]{HttpURLConnection.HTTP_NOT_FOUND}, terminator, "PUT", TxSupport.STATUS_MEDIA_TYPE,
