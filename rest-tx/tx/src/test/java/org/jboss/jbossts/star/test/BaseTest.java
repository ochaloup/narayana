...
public class BaseTest {
    ...
    public static class TransactionalResource {
        ...
        public Work makeWork(String id, String txId, String enlistUrl, String recoveryUrl, String fault) {
            String pURI = PURL + '/' + id;
            String terminator = new StringBuilder().append(pURI).append('/').append(txId).append("/terminate")
                    .toString();
            String participant = new StringBuilder().append(pURI).append('/').append(txId).append("/terminator")
                    .toString();
        ...
        private String moveParticipant(Work work, String nid, String register) {
            ...
            if ("true".equals(register))
                new TxSupport().httpRequest(new int[]{HttpURLConnection.HTTP_OK}, work.recoveryUrl, "PUT",
                        TxSupport.POST_MEDIA_TYPE, work.pUrls, null);
        ...
        public String enlist(@QueryParam("pId") @DefaultValue("") String pId,
                @QueryParam("fault") @DefaultValue("") String fault, String enlistUrl) throws IOException {
            Map<String, String> links = new HashMap<String, String>();
            Work work = faults.get(pId);

            if (work == null)
                work = makeWork(Integer.toString(++pid), enlistUrl.substring(enlistUrl.lastIndexOf('/') + 1), enlistUrl,
                        null, fault);
            ...
            try {
                new TxSupport().httpRequest(new int[]{HttpURLConnection.HTTP_CREATED}, enlistUrl, "POST",
                        TxSupport.POST_MEDIA_TYPE, work.pUrls, links);
