...
public class RESTRecord extends AbstractRecord {
    ...
    private int statusToOutcome(String status) {
        ...
        /*
         * else if (TxSupport.PREPARING.equals(status)) return
         * TwoPhaseOutcome.PREPARE_NOTOK; else if
         * (TxSupport.RUNNING.equals(status)) return
         * TwoPhaseOutcome.FINISH_ERROR; else if
         * (TxSupport.UNKNOWN.equals(status)) return
         * TwoPhaseOutcome.FINISH_ERROR; else if
         * (TxSupport.ABORTING.equals(status)) return
         * TwoPhaseOutcome.FINISH_ERROR; else if
         * (TxSupport.COMMITTING.equals(status)) return
    ...
    public int topLevelPrepare() {
        ...
        try {
            status = TxSupport
                    .getStatus(new TxSupport().httpRequest(new int[]{HttpURLConnection.HTTP_OK}, this.terminateUrl,
                            "PUT", TxSupport.STATUS_MEDIA_TYPE, TxSupport.toStatusContent(TxSupport.PREPARED), null));
    ...
    public int topLevelAbort() {
        ...
        try {
            status = TxSupport
                    .getStatus(new TxSupport().httpRequest(new int[]{HttpURLConnection.HTTP_OK}, this.terminateUrl,
                            "PUT", TxSupport.STATUS_MEDIA_TYPE, TxSupport.toStatusContent(TxSupport.ABORTED), null));
        } catch (HttpResponseException e) {
    ...
    private int doCommit(String nextState) {
        ...
        try {
            ...
            if (!TxSupport.isReadOnly(status)) {
                txs = new TxSupport();
                String body = txs.httpRequest(new int[]{HttpURLConnection.HTTP_OK}, this.terminateUrl, "PUT",
                        TxSupport.STATUS_MEDIA_TYPE, TxSupport.toStatusContent(nextState), null);
     ...
     * has happened perform a GET on the recovery terminateUrl which returns the
    ...
    private boolean hasParticipantMoved() {
        try {
            ...
            // get the latest participant terminateUrl by probing the recovery
            // terminateUrl:
            String content = new TxSupport().httpRequest(new int[]{HttpURLConnection.HTTP_OK}, recoveryUrl, "GET",
