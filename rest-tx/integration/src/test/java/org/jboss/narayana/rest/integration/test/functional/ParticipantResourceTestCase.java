...
public final class ParticipantResourceTestCase {
    ...
    private void registerParticipant(final Uid participantId, final Participant participant)
            throws MalformedURLException {
        ParticipantInformation participantInformation = new ParticipantInformation(participantId, "", BASE_URL,
                participant);
    ...
    private ClientResponse<String> prepareParticipant(final Uid participantId) throws Exception {
        return new ClientRequest(PARTICIPANT_URL + "/" + participantId)
                .body(TxMediaType.TX_STATUS_MEDIA_TYPE, TxSupport.toStatusContent(TxStatus.TransactionPrepared.name()))
                .put(String.class);
    }

    private ClientResponse<String> commitParticipant(final Uid participantId) throws Exception {
        return new ClientRequest(PARTICIPANT_URL + "/" + participantId)
                .body(TxMediaType.TX_STATUS_MEDIA_TYPE, TxSupport.toStatusContent(TxStatus.TransactionCommitted.name()))
                .put(String.class);
    }
