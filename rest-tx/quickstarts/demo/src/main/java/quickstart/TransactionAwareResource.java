...
/**
 * An example of how a REST resource can act as a participant in a REST Atomic
 * transaction. For a complete implementation of a participant please refer to
 * the test suite, in particular the inner class:
 * org.jboss.jbossts.star.test.BaseTest$TransactionalResource which implements
 * all the responsibilities of a participant
 *
 * The example sends a service request which is handled by the method
 * incrementCounters. The request includes the URL for registering durable
 * participants within the transaction. This naive implementation assumes every
 * request with a valid enlistment URL is a request a new unit of transactional
 * work and enlists a new URL into the transaction. Thus if a client makes two
 * http requests to the method incrementCounters then the participant is
 * enlisted twice into the transaction but with different completion URLs. This
 * facilitates the demonstration of 2 phase commit processing.
 */
...
public class TransactionAwareResource {
    ...
    public Response incrementCounters(@Context UriInfo info,
        ...
        if (w != null) {
            ...
            response = Response.ok("non transactional request. Value=" + counters[index].incrementAndGet()).build();
        } else {
            ...
            System.out.println(
                    "proxies: " + System.getProperty("http.proxyHost") + ':' + System.getProperty("http.proxyPort"));

            try {
                new TxSupport().httpRequest(new int[]{HttpURLConnection.HTTP_CREATED}, enlistUrl, "POST",
                ...
                if (e.getActualResponse() == 404)
                    ...
                    return Response.status(e.getActualResponse()).entity("Transaction Manager service is unavailable")
                            .build();
            }
    ...
    public Response readCounters(@QueryParam("wId") @DefaultValue("0") String wid) {
        ...
        if (w == null)
            return Response.ok("counter[0]=" + counters[0].get() + "<br/>counter[1]=" + counters[1].get()).build();
        else
            return Response.ok("counter[" + w.index + "]=" + w.counter).build();
    }

    /*
     * this method handles PUT requests to the url that the participant gave to
     * the REST Atomic Transactions implementation (in the someServiceRequest
     * method). This is the endpoint that the transaction manager interacts with
     * when it needs participants to prepare/commit/rollback their transactional
     * work.
     */
    @PUT
    @Path("{wId}/terminate")
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        System.out.println("JAX-RS container waiting for requests on " + HOST + ":" + PORT + " (for 1000 seconds) ...");
