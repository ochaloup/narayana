...
/**
 * Base class that each transaction prodcut should subclass. The main method
 * controls test execution - if any of the default behaviours supplied by this
 * base class are not appropriate each product should override that behaviour.
 * Please consult subclasses for examples.
 */
public abstract class Product {
    ...
    /**
     * Some products require their own wrappers around javax.sql.DataSource This
     * method may be overridden to support such a product
     * 
     * @param name
     *            a name for the resource
     * @param props
     *            set of properties to set on the underlying resource manager
     ...
     * 
     * @param db
     *            the database name that was extracted from the connect URL
    ...
    protected void executeSql(Connection c, String sql, boolean ignoreErrors) throws SQLException {
        ...
        try {
            ...
            if (log.isDebugEnabled())
                log.debug("executeSql() " + e.getMessage());
        }
    }
    ...
    private static void initialize(String[] args) throws IllegalArgumentException {
        ...
        /*
         * if (urls.size() == 0) throw new
         * RuntimeException("Please specify one or more jdbc connect urls using the syntax "
         * + DB_URL_PROP + "=<connect url>");
         * 
         * if (users == null || passwords == null || users.length !=
         * passwords.length || users.length < urls.size()) throw new
         * RuntimeException("The number of db urls, users and passwords are not equal"
         * );
         */
        int i = 0;
        ...
        /*
         * StringBuilder sb = new StringBuilder(); for (String db : dbs) { if
         * (sb.length() != 0) sb.append(':');
         * 
         * sb.append(db); } System.getProperties().put("jdbc.drivers",
         * sb.toString());
         */
            ...
            for (String clazz : pprop.split(",")) {
                ...
                try {
                    ...
                    for (String type : dbs) {
                        if (!p.supportsDb(type)) {
                            if (DbWrapper.isWarnEnabled(log))
                                log.warn("Product " + p.getName() + " does not support db type " + type);
                            canRun = false;
                        }
                    }
                } catch (IllegalArgumentException e) {
                    if (DbWrapper.isWarnEnabled(log))
                        log.warn("Error initializing product " + clazz + ": " + e.getMessage());
                    // e.printStackTrace();
                }
                ...
                if (canRun) {
                    try {
                        ...
                        if (DbWrapper.isWarnEnabled(log))
                            log.warn("Product " + p.getName() + ": JMX error: " + e.getMessage());
                    }
                }
            }
        }
    ...
    private void runTest() {
        if (log.isDebugEnabled())
            log.debug("Testing product " + getName());
            ...
            try {
                ...
                for (int i = 0; i < THRCOUNT.length; i++) {
                    for (int j = 0; j < TXCOUNT.length; j++) {
                        ...
                        // csv[i + 1][prodIndex + 2] = (float) tr.getAvg() /
                        // TXCOUNT[j];
    ...
    private PerfTest getTest(final String name, final int txCount) {
        return new PerfTest() {
            ...
            public void run() {
                try {
                    if (log.isInfoEnabled())
                        log.info("Test " + Thread.currentThread().getName() + " is starting ...");
                ...
                if (log.isInfoEnabled())
                    log.info("Test " + Thread.currentThread().getName() + " has finished");
            }
        };
    }
    ...
    private void writeResults(TestRunner tr, int txCount) throws SQLException {
         ...
         * 
         * for (int i = 0; i < sql.length; i++) { Connection c =
         * adminDs[i].getConnection(); executeStatement(c, sql[i], getName(),
         * types.toString(), tr.getPassCount(), tr.getFailCount(), tr.getMax(),
         * tr.getMin(), tr.getAvg(), txCount); c.commit(); c.close(); }
         */
        ...
        results.format("%1$-32s %2$-16s %3$-8d %4$-8d %5$-8d %6$-8d %7$-8d %8$-8d %9$-9d\n", // %10$-32s\n",
                                                                                                // //%9$7.2f\n",
                getName(), types.toString(), tr.getPassCount(), tr.getFailCount(), tr.getMax(), tr.getMin(),
                tr.getAvg(), txCount, tp); // , tr.getTimes()); //(float)
                                            // tr.getAvg() / txCount);
        ...
        try {
            ...
            if (!exists) {
                results.format("%1$-32s %2$-16s %3$-8s %4$-8s %5$-8s %6$-8s %7$-8s %8$-8s %9$8s\n", "Product",
                        "Database(s)", "Pass", "Fail", "Max", "Min", "Average", "Count", "tx/sec");
                results.format("%1$-32s %2$-16s %3$-8s %4$-8s %5$-8s %6$-8s %7$-8s %8$-8s %9$8s\n", "------",
                        "-----------", "----", "----", "---", "---", "-------", "-----", "------");
            }
        } catch (FileNotFoundException e) {
