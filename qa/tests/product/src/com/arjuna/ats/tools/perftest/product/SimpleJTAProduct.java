...
public class SimpleJTAProduct extends Product {
    ...
    // WARNING putting a schema qualifier here means simpleJTA won't be able to
    // find its logs
    // see org.simplejta.tm.log.JDBCTransactionLog
    private static final String SJTA_CREATE1_SQL = "create table app.sjta_transactions ("
            + "    tid bigint generated always as identity," + "    tmid varchar(30)," + "    formatid int,"
            + "    gtid varchar(64) for bit data," + "    bqual varchar(64) for bit data," + "    state smallint" + ")";
    private static final String SJTA_CREATE2_SQL = "create table app.sjta_transaction_branches (" + "       tid bigint,"
            + "       bid int," + "       formatid int," + "       gtid varchar(64) for bit data,"
    ...
    protected void init(Connection c, Properties props) throws SQLException {
        ...
        simpleJtaProps.setProperty(org.simplejta.tm.SimpleTransactionManager.STM_TMGR_ID, TMID);
        ...
        simpleJtaProps.setProperty(org.simplejta.tm.log.TransactionLogFactory.STM_TLOG_URL, url); // url);
                                                                                                    // //DERBY_URL_PROP));
        simpleJtaProps.setProperty(org.simplejta.tm.log.TransactionLogFactory.STM_TLOG_USER, user);
        simpleJtaProps.setProperty(org.simplejta.tm.log.TransactionLogFactory.STM_TLOG_PASSWORD, pass);
    ...
    protected DataSource createDataSource(String name, Properties props) throws SQLException {
        ...
        if (DbWrapper.isDerby(type))
            ...
            return new org.simplejta.tm.datasource.SimplePostgreSqlXADataSource(TMID, url, user, pass); // TODO
                                                                                                        // url
                                                                                                        // ->
                                                                                                        // properties
            ...
            return new org.simplejta.tm.datasource.SimpleMsSqlServerXADataSource(TMID, url, user, pass); // TODO
                                                                                                            // url
                                                                                                            // ->
                                                                                                            // properties
        else
            return null; // new
                            // org.simplejta.tm.datasource.SimpleXADataSource(TMID,
                            // "need a typeid", url,user, pass);
    }
