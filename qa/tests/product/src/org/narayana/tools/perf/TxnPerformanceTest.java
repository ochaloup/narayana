...
public class TxnPerformanceTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        int threads = 100;
        String opt;
        String products[] = {"org.narayana.tools.perf.BitronixWorkerTask", "org.narayana.tools.perf.AtomikosWorkerTask",
                "org.narayana.tools.perf.JotmWorkerTask", "org.narayana.tools.perf.NarayanaWorkerTask",};

        if (args.length != 0) {
            String[] opts = args[0].trim().split("\\s+");
            /*
             * for (int i = 0; i < opts.length; i++) { if
             * (opts[i].startsWith("-")) { opt = opts[i++];
             * 
             * if (i < opts.length) { if ("-p".equals(opt)) products =
             * opts[i].split(","); else if ("-i".equals(opt)) iterations =
             * Integer.valueOf(opts[i]); else if ("-t".equals(opt)) threads =
             * Integer.valueOf(opts[i]); else fatal("Syntax error: "); } else {
             * fatal("Syntax error: "); } } else { fatal("Syntax error: "); } }
             */
    ...
    private static WorkerTask newWorker(String className, CyclicBarrier cyclicBarrier, AtomicInteger count,
            int batch_size) throws Exception {

        Class classDef = Class.forName(className);
        Constructor constructor = classDef
                .getDeclaredConstructor(new Class[]{CyclicBarrier.class, AtomicInteger.class, int.class});
    ...
    private static TaskResult testLoop(String workerClassName, int iterations, int threads) throws Exception {
        ...
        CyclicBarrier cyclicBarrier = new CyclicBarrier(nThreads + 1); // workers
                                                                        // +
                                                                        // self
    ...
    static class TaskResult {
        ...
        public String toString() {
            ...
            sb.append(product).append(": ").append((1000.0 / ((1.0 * duration_ms) / iterations)))
                    .append(" tx / second");
