...
public class JDBCProfileStore {
    ...
    public static String getTableName(String username, String suffix) throws UnknownHostException {
        ...
        // previously this would have been: username + "_" + suffix as in "DROP
        // TABLE " + databaseUser + "_InfoTable");
        String value = username + "_" + getLocalHostNameForTables() + "_" + suffix;
        // in addition to the problems with the valid characters, there are
        // issues with the max length.
        // for oracle it's 30, but we stick a 4 char suffix on indexes, so the
        // table name should not be more than 26
        // in certain cases this may mean we wind up with non-uniq names, which
        // is a pain.
        if (value.length() > 26) {
    ...
    private static String getLocalHostNameForTables() throws UnknownHostException {
        ...
        hostName = stripHostName(hostName); // strip to local portion, force
                                            // lower case.
        hostName = hostName.replace("-", "_"); // some db's don't like hyphens
                                                // in identifiers
        return hostName;
    }
    ...
    private static void loadProfile() throws Exception {
        if (_profile == null) {
            ...
            if (!file.exists()) {
                ...
                if (isIPv6()) // placeholder for using a different config for
                                // IPv6 testing
