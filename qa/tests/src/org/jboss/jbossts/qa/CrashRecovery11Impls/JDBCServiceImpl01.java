...
public class JDBCServiceImpl01 implements BeforeCrashServiceOperations {
        ...
        try {
            try {
                ...
                statement.executeUpdate(
                        "UPDATE " + tableName + " SET VALUE=" + value + " WHERE Name='" + _rowName + "'");
        ...
        try {
            try {
                ...
                ResultSet resultSet = statement
                        .executeQuery("SELECT Value FROM " + tableName + " WHERE Name = '" + _rowName + "'");
                resultSet.next();
    ...
    public void setStartCrashAbstractRecordAction(CrashBehavior action) throws InvocationException {
        try {
            try {
                if (action == CrashBehavior.CrashBehaviorCrashInCommit) {
                    _isCorrect = _isCorrect && (BasicAction.Current().add(new StartCrashAbstractRecordImpl(
                            StartCrashAbstractRecordImpl.CRASH_IN_COMMIT)) == AddOutcome.AR_ADDED);
                } else if (action == CrashBehavior.CrashBehaviorCrashInPrepare) {
                    _isCorrect = _isCorrect && (BasicAction.Current().add(new StartCrashAbstractRecordImpl(
                            StartCrashAbstractRecordImpl.CRASH_IN_PREPARE)) == AddOutcome.AR_ADDED);
                }
            } catch (Exception exception) {
    ...
    public void setEndCrashAbstractRecordAction(CrashBehavior action) throws InvocationException {
        try {
            try {
                if (action == CrashBehavior.CrashBehaviorCrashInCommit) {
                    _isCorrect = _isCorrect && (BasicAction.Current().add(new EndCrashAbstractRecordImpl(
                            EndCrashAbstractRecordImpl.CRASH_IN_COMMIT)) == AddOutcome.AR_ADDED);
                } else if (action == CrashBehavior.CrashBehaviorCrashInPrepare) {
                    _isCorrect = _isCorrect && (BasicAction.Current().add(new EndCrashAbstractRecordImpl(
