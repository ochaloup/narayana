...
public class WorkerClient001 extends BaseTestClient {
    ...
    public void Test() {
        try {
            ...
            // set up lockmanager records - if newLock use the implementation
            // with new lock per iteration
            BasicLockRecord[] mLockRecordList = _newLock
                    ? new BasicLockRecord[mNumberOfResources]
                    : new BasicLockRecord2[mNumberOfResources];
            ...
            for (int i = 0; i < mNumberOfResources; i++) {
                ...
                if (result > mPercent) {
                    Debug("resource " + i + " final value is incorrect: value =" + mLockRecordList[i].getValue()
                            + " we expected = " + expectedValue[i] + " does not fall into the " + (mPercent * 100)
                            + "% margin");
                    mCorrect = false;
                }
            }
