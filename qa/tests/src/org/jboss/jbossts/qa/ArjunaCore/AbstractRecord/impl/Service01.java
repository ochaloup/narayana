...
/**
 * This service class is the container that we will use to emulate how abstract
 * records are to be used by the transaction servcie.
 */
public class Service01 {
    /**
     * Constructor that will set up the number of abstract records that are
     * going to ber used in each transaction.
     */
    ...
    /**
     * simple method used to create the abstract records and enlist them into
     * the current running transaction. if no transaction is running the method
    ...
    /**
     * passing in true to this operation will force the records to be enlisted
     * into a new transaction nesting the new transaction within any other
     * running transaction.
     */
    ...
    /**
     * main body of work will be performed here and any sub transactions that
     * are currently running will be commited on completion.
     */
    ...
    public boolean checkAbortOper() {
        ...
        for (int i = 0; i < mNumberOfResources; i++) {
            ...
            if (mAbstractRecordList[i].getValue() != mMaxIteration) {
                qautil.debug("whilst checking the " + i + " resource the getvalue was: "
                        + mAbstractRecordList[i].getValue() + " and we expected: " + mMaxIteration);
                return false;
            }
            if (mNest) {
    ...
    public boolean checkCommitOper() {
        ...
        for (int i = 0; i < mNumberOfResources; i++) {
            ...
            if (mNest) {
                ...
                if (mAbstractRecordList[i].getNC() != 1) {
                    qautil.debug(
                            "nested commit value is wrong in resource " + i + " " + mAbstractRecordList[i].getNC());
                    return false;
                }
            } else {
                qautil.qadebug("normal check");
