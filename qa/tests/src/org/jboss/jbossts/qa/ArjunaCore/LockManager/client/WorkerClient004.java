...
public class WorkerClient004 extends BaseTestClient {
    ...
    public void Test() {
        try {
            ...
            TXBasicLockRecord[] mLockRecordList = _newLock
                    ? new TXBasicLockRecord[mNumberOfResources]
                    : new TXBasicLockRecord2[mNumberOfResources];
            ...
            for (int i = 0; i < mNumberOfResources; i++) {
                ...
                if (result > mPercent) {
                    Debug("resource " + i + " final value is incorrect: value =" + mLockRecordList[i].getValue()
                            + " we expected = " + expectedValue[i] + " does not fall within the " + (mPercent * 100)
                            + "% margin");
                    mCorrect = false;
                }
            }
