...
/**
 * Reimplementation of the Task abstraction. This version cooperates with a task
 * reaper which times out tasks whose underlying subprocess has become wedged.
 * This can be used to ensure that the thread running a junit tests does not get
 * wedged by a badly behaving subprocess. The task reaper also provides a means
...
public class TaskImpl implements Task {
    ...
    static {
        try {
            ...
            // do property value token substitution, copy result to the actual
            // properties:
            ...
            while (propertyNames.hasMoreElements()) {
                ...
                while (matcher.find()) {
                    ...
                    String replacement = Matcher
                            .quoteReplacement(rawproperties.getProperty(group, System.getProperty(group, "")));
            ...
            // in addition to the props file, we allow properties via.
            // "additional.properties" var
    ...
    /**
     * a class implementing a main method which gets executed in a JVM running
     * in a subprocess in order to perform this task
     */
    private Class clazz;

    /**
     * identifies whether the task is a test which is expected to print Passed
     * or Failed or a service which is expected to print Ready.
     */
    private TaskType type;

    /**
     * a timeout in seconds from when the task is started after which the task
     * should be destroyed automatically by the task reaper if it has nto yet
    ...
    /**
     * tasks can only be started once. this flag gaurds against repeated start
     * attempts
     */
    private boolean started;
    /**
     * flag identifying whether or not this task's subpricess has finished
     * executing
     */
    private boolean isDone;
    /**
     * a flag which is set if the task's process times out and gets destroyed
     * before the task's subprocess has finished executing.
     */
    private boolean isTimedOut;
    /**
     * an output stream to which the contents of the task's stdout and stderr
     * are redirected.
     */
    ...
    /**
     * a thread which reads the task's merged stdout/stderr stream and
     * identifies whether or not a Passed/Failed or a Ready message has been
     * printed. the reader thread alos needs to write the output to a log file.
     */
     ...
     * 
     * @param clazz
     *            the task whose main method is to be executed in a JVM running
     *            in a subprocess
     * @param type
     *            the type of the test either PASS_FAIL or READY
     * @param out
     *            the output stream to which output from the task's process
     *            shoudl be redirected.
     * @param timeout
     *            the timeout for the task in seconds
     * @param additionalLocalCommandLineElements
     *            elements to add to the command line.
     */
    TaskImpl(String taskName, Class clazz, TaskType type, PrintStream out, int timeout,
            List<String> additionalLocalCommandLineElements) {
    ...
    /**
     * execute a type READY task in a subprocess passing no arguments to the
     * Main method of the implementing class then wait for the subprocess to
     * exit.
     */
    public void perform() {
        perform((String[]) null);
    ...
    /**
     * execute a type READY task in a subprocess passing the supplied arguments
     * to the Main method of the implementing class then wait for the subprocess
     * to exit.
     * 
     * @param params
     *            arguments to supply to the main method of the implementing
     *            class
     */
    public void perform(String... params) {
        ...
        synchronized (this) {
            ...
            try {
                ...
                // !!!! we cannot do this as we cannot be sure the spawned task
                // does not
                // interleave its stdout and stderr streams. if it does then
                // this messes up detection
                // of Ready, Passed and Failed output lines since they may get
                // mingled with lines
                // from the error stream. !!!!
                // redirect errors to stdout -- avoids getting wedged when error
                // output stream becomes full
            ...
            // ok, we have started so register with the task reaper -- need to
            // synchronize so we can set
            // started atomically

            System.out.printf("%sInsert into task reaper queue with timeout %d secs%n", getTaskPrefix(),
                    timeout * 1000);
        ...
        try {
            // create an error stream reader to merge error output into the
            // output file
            ...
            System.out.printf("%sReached end of input passed=%b failed=%b%n", getTaskPrefix(), printedPassed,
                    printedFailed);
        } catch (Exception e) {
        ...
        // we cannot really protect properly against races here because
        // process.waitFor takes its
        // own lock before then suspending so there will always be a window
        // here. luckily process.destroy()
        // which gets called by the reaper is idempotent, meaning the only thing
        // we risk is that the task
        // reaper may call destroy and register a stalled thread even though we
        // get through the waitFor
        // call.
        try {
            process.waitFor();
        } catch (Exception e) {
        ...
        // ok, now ensure we sort out the race between the reaper task and this
        // one
        synchronized (this) {
            if (!isTimedOut) {
                System.out.printf("%s perform removing task from reaper%n", getTaskPrefix());
                // we got through the waitFor and relocked this before we could
                // be timed out so remove the
                // task from the reaper list
                TaskReaper.getReaper().remove(this);
            } else {
                System.out.printf("%s perform timed out%n", getTaskPrefix());
            }
            // setting this will forestall any pending attempt to timeout this
            // task
            isDone = true;
    ...
    /**
     * execute a type PASS_FAIL or type READY task asynchronously in a
     * subprocess passing no arguments to the Main method of the implementing
     * class. if the task type is READY do not return until it has printed
     * Ready.
     */
    public void start() {
        start((String[]) null);
    ...
    /**
     * execute a type PASS_FAIL or type READY task asynchronously in a
     * subprocess passing the supplied arguments to the Main method of the
     * implementing class. if the task type is READY do not return until it has
     * printed Ready.
     * 
     * @param params
     *            arguments to supply to the main method of the implementing
    ...
    public void start(String... params) {
        ...
        synchronized (this) {
            ...
            System.out.printf("%s Task started, insert into task reaper queue with timeout %d secs%n", getTaskPrefix(),
                    timeout * 1000);
    ...
    /**
     * check that a type PASS_FAIL task which was started asynchronously has
     * printed either Passed or Failed and exited cleanly, waiting if it has not
     * yet completed and asserting if either condition fails.
     */
    public void waitFor() {
        ...
        // the taskReaderThread will throw up if it did nto get a clean finish
        // or get a Passed and no Failed
    ...
    public void terminate() {
        ...
        if (taskReaderThread != null) {
            // tell the reader not to throw a wobbly before we destroy the
            // process
    ...
    /**
     * ensure that there are no tasks left running, clearing out all such tasks
     * and asserting if this is not the case
     */
     ...
     * 
     * @return true if the process was timed out and false if it exited before
     *         we got there
     */
    boolean timeout() {
        synchronized (this) {
            ...
            out.flush();
            createThreadDumps();
            // we timed out before the process managed to complete so kill it
            // now
            // n.b. since this closes the process stdout we can be sure that the
            // task
    ...
    /**
     * construct an executable command line to execute the supplied java class
     * with the arguments in params, substituting those in the form $(...) with
    ...
    /**
     * construct a copy of the supplied argument String array replacing all
     * terms in the form $(...) with corresponding values derived from the
     * properties file
     * 
     * @param params
     *            the original arguments to be copied
    ...
    /**
     * a thread created whenever a task is started asynchronously to forward
     * output from the task's process to an output stream and to detect printing
     * of Ready, Passed and Failed ouptut lines.
     */
    private class TaskReaderThread extends Thread {
        ...
        /**
         * called by the test thread under Task.start() to ensure that a type
         * READY task's process has printed a Ready output line before returning
         * from the start call.
         */
        public void blockingWaitForReady() {
            synchronized (printedReady) {
                // test of shutdown ensures we exit this loop if we get
            ...
            System.out.printf("%s TaskReader printedReady=%b shutdown=%b%n", getTaskPrefix(), id, printedReady.get(),
                    shutdown.get());
        ...
        /**
         * called by the test thread to ensure that a type PASS_FAIL or READY
         * task's process has exited and closed its output stream cleanly,
         * thereby causing the reader thread to exit cleanly.
         */
        ...
        /**
         * called by the test thread to ensure that a type PASS_FAIL task's
         * process has printed a Passed output line and has not printed a Failed
         * output line.
         */
        ...
        /**
         * create a task reader thread defaulting the prefix to "Line: " and the
         * output stream to
         *
         */
        ...
        public void run() {
            ...
            try {
                ...
                // // if the process is explicitly destroyed we can see an
                // IOException because the output
                // // stream gets closed under the call to readLine(). shutdown
                // is set befofe destroy
                // // is called so only trace the exception if we are not shut
                // down
                // if (shutdown.get()) {
                // return;
                // }
                out.printf("%s TaskReaderThread : exception before shutdown %s%n", getTaskPrefix(), e.getMessage());
                System.out.printf("%s TaskReaderThread : exception before shutdown %s%n", getTaskPrefix(),
                        e.getMessage());
    ...
    /**
     * a thread created whenever a task is started asynchronously to forward
     * error output from the task's process to an output stream.
     */
    private class TaskErrorReaderThread extends Thread {
        ...
        public void run() {
            ...
            try {
                ...
                // if the process is explicitly destroyed we can see an
                // IOException because the output
                // stream gets closed under the call to readLine(). shutdown is
                // set befofe destroy
                // is called so only trace the exception if we are not shut down
                if (shutdown.get()) {
                    return;
                }
                out.printf("%s TaskErrorReaderThread : exception before shutdown %s%n", getTaskPrefix(),
                        e.getMessage());
                e.printStackTrace(out);
            } finally {
                try {
