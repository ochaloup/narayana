...
public class Test03 implements ClientAction {
    // the longest time to wait in millis before declaring a test a failed
    // (overridable)
    ...
    public boolean execute(ASTestConfig config, Map<String, String> params) {
        ...
        try {
            ...
            // switch class loaders since a custom ant task runs with a
            // different loader from the loader
            ...
            if (expectFailure) {
                // this test may halt the VM so make sure the transaction log is
                // empty
                // before starting the test - then the pass/fail check is simply
                // to
                // test whether or not the log is empty (see recoverUids()
                // below).
                try {
    ...
    // count how many pending transaction branches there are in the transaction
    // log
    ...
    private boolean recoverUids() throws IOException {
        ...
        // the test failed to recover some uids - clear them out ready for the
        // next test
        try {
            store.clearXids(storeType);
        } catch (Exception e) {
