...
/**
 * Ant task for capturing common configuration for a group of tests defined in
 * an ant build script.
 */
public class ASTestConfig extends Task {
    // arbitary handle for refering to the unique instance of the task
    // configuration
    ...
    public void execute() throws BuildException {
        ...
        if (productDir == null)
            throw new BuildException("Please specify the location of the product directory via the \""
                    + PRODUCT_DIR_PROP + "\" system property");
        ...
        // add this to the ant project so that other tasks are able to locate
        // this
        ...
        // read in any failure specifications so that an ant target may look
        // them up
     ...
     *
     * @param javaHome
     *            Path to the the java installation
     */
    ...
    /**
     * JBoss dist to start. The default is to use the AS installed with the
     * product. If no AS can be found then the environment variable JBOSS_HOME
     * is used
     * 
     * @param jbossHome
     *            Path to the the jboss installation
     */
     ...
     *
     * @param serverName
     *            the server providing the context
     ...
     *
     * @param serverName
     *            the name of the server whose path is being sought
     ...
     * @throws IOException
     *             If another process is already using a port required by this
     *             instance
     * @see org.jboss.jbossas.servermanager.ServerController#startServer(org.jboss.jbossas.servermanager.Server,
     *      org.jboss.jbossas.servermanager.ServerManager)
     */
    public void startServer(String serverName) throws IOException {
        ...
        // !server.isRunning() and !server.isStopped() implies that some other
        // entity terminated the server
        if (!server.isRunning())
            manager.startServer(serverName);
    }

    /**
     * @see org.jboss.jbossas.servermanager.ServerController#stopServer(org.jboss.jbossas.servermanager.Server,
     *      org.jboss.jbossas.servermanager.ServerManager)
     */
    ...
    /**
     * Stop a group of servers. Any error stopping a server will logged and then
     * the next one in the list will be tried.
     *
     * @param servers
     *            the names of the servers to be stopped
     ...
     *
     * @param serverName
     *            the server whose bindings are being configured
     * @throws TransformerException
     *             error parsing the server bindings xml file
     * @throws IOException
     *             error whilst locating the bindings file in the file system
     * @throws SAXException
     *             error parsing the server bindings xml file
     * @throws ParserConfigurationException
     *             error parsing the server bindings xml file
     */
    public void configureServerBinding(String serverName)
        ...
        // if bindingName is set then it is safe to check that the AS is up via
        // the Naming service port
        // BTW the AS5 team has changed the behavior of the naming service port
        // - it is opened before
        // all the services have been configured so any target deployments may
        // not have been initialised
        // when the server is declared as running. Instead we force the server
        // to use a web server port to
        ...
        System.out.println("Using: " + Context.PROVIDER_URL + " = jnp://" + server.getHost() + ':' + server.getRmiPort()
