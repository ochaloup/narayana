...
public class BeforeCrashServiceImpl01 implements BeforeCrashServiceOperations {
    ...
    public void setup_oper(ResourceBehavior[] resource_behaviors) {
        ...
        for (int index = 0; index < resource_behaviors.length; index++) {
            try {
                resourceImpl[index] = new ResourceImpl01(_serviceNumber, _objectNumber, index,
                        resource_behaviors[index]);
                ...
                recoveryCoordinator[index] = OTS.current().get_control().get_coordinator()
                        .register_resource(resource[index]);

                System.out.println("BeforeCrashServiceImpl01: storing IOR \"RecoveryCoordinator_" + _serviceNumber + "_"
