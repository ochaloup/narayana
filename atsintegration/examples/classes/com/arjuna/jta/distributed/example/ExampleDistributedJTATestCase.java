...
public class ExampleDistributedJTATestCase {
    ...
    public static void setup() throws SecurityException, NoSuchMethodException, InstantiationException,
        ...
        for (int i = 0; i < localServers.length; i++) {
            ...
            IsolatableServersClassLoader classLoaderForTransactionManager = new IsolatableServersClassLoader(null,
                    ExampleDistributedJTATestCase.class.getPackage().getName(), contextClassLoader);
            IsolatableServersClassLoader classLoader = new IsolatableServersClassLoader(
                    ExampleDistributedJTATestCase.class.getPackage().getName(), null, classLoaderForTransactionManager);
            localServers[i] = (LocalServer) classLoader
                    .loadClass("com.arjuna.jta.distributed.example.server.impl.ServerImpl").newInstance();
     ...
     */
    @Test
    public void testMigrateTransaction()
            throws NotSupportedException, SystemException, IllegalStateException, RollbackException, XAException,
        ...
        List<String> nodesToFlowTo = new LinkedList<String>(
                Arrays.asList(new String[]{"2000", "3000", "2000", "1000", "2000", "3000", "1000", "3000"}));
        ...
        if (!nodesToFlowTo.isEmpty()) {
            ...
            if (status == Status.STATUS_ACTIVE) {
                ...
                int remainingTimeout = (int) (((TransactionTimeoutConfiguration) transactionManager)
                        .getTimeLeftBeforeTransactionTimeout(false) / 1000);
                ...
                DataReturnedFromRemoteServer dataReturnedFromRemoteServer = propagateTransaction(nodesToFlowTo,
                        remainingTimeout, currentXid);
                ...
                // subordinate
                transaction.enlistResource(originalServer.generateProxyXAResource(nextServerNodeName,
                ...
                transaction.registerSynchronization(
                        originalServer.generateProxySynchronization(nextServerNodeName, currentXid));
     ...
     * @throws IOException
     */
    private DataReturnedFromRemoteServer propagateTransaction(List<String> nodesToFlowTo, int remainingTimeout,
            Xid toMigrate) throws RollbackException, IllegalStateException, XAException, SystemException,
        ...
        if (!nodesToFlowTo.isEmpty()) {
            ...
            if (status == Status.STATUS_ACTIVE) {
                ...
                remainingTimeout = (int) (((TransactionTimeoutConfiguration) transactionManager)
                        .getTimeLeftBeforeTransactionTimeout(false) / 1000);
                // Get the XID to propagate
                ...
                if (dataReturnedFromRemoteServer.getRemoteXidCreated() != null) {
                    ...
                    transaction.enlistResource(currentServer.generateProxyXAResource(nextServerNodeName,
                            dataReturnedFromRemoteServer.getRemoteXidCreated()));
                    // Register a sync
                    transaction.registerSynchronization(
